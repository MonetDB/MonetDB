...
transformCoordSeq(int idx, int coordinatesNum, projPJ proj4_src, projPJ proj4_dst, const GEOSCoordSequence *gcs_old, GEOSCoordSeq gcs_new)
{
	...
	if (!GEOSCoordSeq_getX(gcs_old, idx, &x) ||
		...
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""Couldn't get coordinates");
	...
	if (*errorNum != 0) {
		if (coordinatesNum > 2)
			throw(MAL, "geom.Transform", "SQLSTATE ----- !""Couldn't transform point (%f %f %f): %s\n", x, y, z, pj_strerrno(*errorNum));
		else
			throw(MAL, "geom.Transform", "SQLSTATE ----- !""Couldn't transform point (%f %f): %s\n", x, y, pj_strerrno(*errorNum));
	}
	...
	if (!GEOSCoordSeq_setX(gcs_new, idx, x) ||
		...
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""Couldn't set coordinates");
...
transformPoint(GEOSGeometry **transformedGeometry, const GEOSGeometry *geosGeometry, projPJ proj4_src, projPJ proj4_dst)
{
	...
	*transformedGeometry = NULL;
	...
	if (gcs_old == NULL)
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
...
transformLine(GEOSCoordSeq *gcs_new, const GEOSGeometry *geosGeometry, projPJ proj4_src, projPJ proj4_dst)
{
	...
	if (!GEOSCoordSeq_getSize(gcs_old, &pointsNum))
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSCoordSeq_getSize failed");

	/* create the coordinates sequence for the transformed geometry */
	*gcs_new = GEOSCoordSeq_create(pointsNum, coordinatesNum);
	if (*gcs_new == NULL)
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
...
transformLineString(GEOSGeometry **transformedGeometry, const GEOSGeometry *geosGeometry, projPJ proj4_src, projPJ proj4_dst)
{
	...
	*transformedGeometry = GEOSGeom_createLineString(coordSeq);
	if (*transformedGeometry == NULL) {
		...
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGeom_createLineString failed");
	}
...
transformPolygon(GEOSGeometry **transformedGeometry, const GEOSGeometry *geosGeometry, projPJ proj4_src, projPJ proj4_dst, int srid)
{
	...
	if (exteriorRingGeometry == NULL) {
		*transformedGeometry = NULL;
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	}
	...
	if (numInteriorRings == -1) {
		*transformedGeometry = NULL;
		...
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGetInteriorRingN failed.");
	}
	...
	*transformedGeometry = GEOSGeom_createPolygon(transformedExteriorRingGeometry, transformedInteriorRingGeometries, numInteriorRings);
	if (*transformedGeometry == NULL) {
		for (i = 0; i < numInteriorRings; i++)
			GEOSGeom_destroy(transformedInteriorRingGeometries[i]);
		ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGeom_createPolygon failed");
	}
	GDKfree(transformedInteriorRingGeometries);
	GEOSGeom_destroy(transformedExteriorRingGeometry);
...
transformMultiGeometry(GEOSGeometry **transformedGeometry, const GEOSGeometry *geosGeometry, projPJ proj4_src, projPJ proj4_dst, int srid, int geometryType)
{
	...
	for (i = 0; i < geometriesNum; i++) {
		if ((multiGeometry = GEOSGetGeometryN(geosGeometry, i)) == NULL)
			ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGetGeometryN failed");
		else if ((subGeometryType = GEOSGeomTypeId(multiGeometry) + 1) == 0)
			ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGeomTypeId failed");
		else {
			...
			case wkbPolygon_mdb:
				...
				break;
			default:
				ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""Unknown geometry type");
				break;
			}
		}
	...
	*transformedGeometry = GEOSGeom_createCollection(geometryType - 1, transformedMultiGeometries, geometriesNum);
	if (*transformedGeometry == NULL) {
		for (i = 0; i < geometriesNum; i++)
			GEOSGeom_destroy(transformedMultiGeometries[i]);
		ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""GEOSGeom_createCollection failed");
	}
	GDKfree(transformedMultiGeometries);
...
wkbTransform(wkb **transformedWKB, wkb **geomWKB, int *srid_src, int *srid_dst, char **proj4_src_str, char **proj4_dst_str)
{
	...
	*transformedWKB = NULL;
	...
	(void) proj4_src_str;
	(void) proj4_dst_str;
	throw(MAL, "geom.Transform", "SQLSTATE ----- !""Function Not Implemented");
#else
	...
	if (strcmp(*proj4_src_str, "null") == 0)
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""Could not find in spatial_ref_sys srid %d\n", *srid_src);
	...
	if (proj4_src == NULL || proj4_dst == NULL) {
		...
		if (proj4_dst)
			pj_free(proj4_dst);
		throw(MAL, "geom.Transform", "SQLSTATE ----- !""pj_init failed");
	}
	...
	if (ret == MAL_SUCCEED && transformedGeosGeometry) {
		...
		if ((*transformedWKB = geos2wkb(transformedGeosGeometry)) == NULL)
			ret = createException(MAL, "geom.Transform", "SQLSTATE ----- !""geos2wkb failed");
...
forceDimCoordSeq(int idx, int coordinatesNum, int dim, const GEOSCoordSequence *gcs_old, GEOSCoordSeq gcs_new)
{
	...
	//get the coordinates
	if (!GEOSCoordSeq_getX(gcs_old, idx, &x))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_getX failed");
	if (!GEOSCoordSeq_getY(gcs_old, idx, &y))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_getY failed");
	if (coordinatesNum > 2 && dim > 2 &&	//read it only if needed (dim >2)
	    !GEOSCoordSeq_getZ(gcs_old, idx, &z))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_getZ failed");

	//create the new coordinates
	if (!GEOSCoordSeq_setX(gcs_new, idx, x))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_setX failed");
	if (!GEOSCoordSeq_setY(gcs_new, idx, y))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_setY failed");
	if (dim > 2)
		if (!GEOSCoordSeq_setZ(gcs_new, idx, z))
			throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_setZ failed");
	return MAL_SUCCEED;
}
...
forceDimPoint(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	if (gcs_old == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
	}

	/* create the coordinates sequence for the translated geometry */
	if ((gcs_new = GEOSCoordSeq_create(1, dim)) == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
	}
	...
	*outGeometry = GEOSGeom_createPoint(gcs_new);
	if (*outGeometry == NULL) {
		GEOSCoordSeq_destroy(gcs_new);
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_createPoint failed");
	}
...
forceDimLineString(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	/* get the number of points in the geometry */
	if (!GEOSCoordSeq_getSize(gcs_old, &pointsNum))
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSCoordSeq_getSize failed");
	...
	*outGeometry = GEOSGeom_createLineString(gcs_new);
	if (*outGeometry == NULL) {
		GEOSCoordSeq_destroy(gcs_new);
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_createLineString failed");
	}
...
forceDimLinearRing(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	*outGeometry = GEOSGeom_createLinearRing(gcs_new);
	if (*outGeometry == NULL) {
		GEOSCoordSeq_destroy(gcs_new);
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_createLinearRing failed");
	}
...
forceDimPolygon(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	if (!exteriorRingGeometry) {
		*outGeometry = NULL;
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	}
	...
	if (numInteriorRings == -1) {
		*outGeometry = NULL;
		GEOSGeom_destroy(transformedExteriorRingGeometry);
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGetInteriorRingN failed.");
	}
	...
	*outGeometry = GEOSGeom_createPolygon(transformedExteriorRingGeometry, transformedInteriorRingGeometries, numInteriorRings);
	if (*outGeometry == NULL) {
		for (i = 0; i < numInteriorRings; i++)
			GEOSGeom_destroy(transformedInteriorRingGeometries[i]);
		ret = createException(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_createPolygon failed");
	}
	GDKfree(transformedInteriorRingGeometries);
	GEOSGeom_destroy(transformedExteriorRingGeometry);
...
forceDimMultiGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	*outGeometry = GEOSGeom_createCollection(GEOSGeomTypeId(geosGeometry), transformedMultiGeometries, geometriesNum);
	if (*outGeometry == NULL) {
		for (i = 0; i < geometriesNum; i++)
			GEOSGeom_destroy(transformedMultiGeometries[i]);
		err = createException(MAL, "geom.ForceDim", "SQLSTATE ----- !""GEOSGeom_createCollection failed");
	}
	GDKfree(transformedMultiGeometries);
...
forceDimGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, int dim)
{
	...
	default:
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
wkbForceDim(wkb **outWKB, wkb **geomWKB, int *dim)
{
	...
	if (geosGeometry == NULL) {
		*outWKB = NULL;
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	*outWKB = geos2wkb(outGeometry);
	...
	if (*outWKB == NULL)
		throw(MAL, "geom.ForceDim", "SQLSTATE ----- !""geos2wkb failed");
...
segmentizePoint(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry)
{
	...
	if ((gcs_old = GEOSGeom_getCoordSeq(geosGeometry)) == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
	}
	//create a copy of it
	if ((gcs_new = GEOSCoordSeq_clone(gcs_old)) == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_clone failed");
	}
	//create the geometry from the coordinates sequence
	*outGeometry = GEOSGeom_createPoint(gcs_new);
	if (*outGeometry == NULL) {
		GEOSCoordSeq_destroy(gcs_new);
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGeom_createPoint failed");
	}
...
segmentizeLineString(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double sz, int isRing)
{
	...
	if (!GEOSCoordSeq_getSize(gcs_old, &pointsNum)) {
		*outGeometry = NULL;
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_getSize failed");
	}
	...
	if (!GEOSCoordSeq_getX(gcs_old, 0, &xCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_getX failed");
		goto bailout;
	}
	if (!GEOSCoordSeq_getY(gcs_old, 0, &yCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_getY failed");
		goto bailout;
	}
	if (coordinatesNum > 2 && !GEOSCoordSeq_getZ(gcs_old, 0, &zCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_getZ failed");
		goto bailout;
	}
...
//fprintf(stderr, "Adding %d\n", additionalPoints);
	...
	if ((gcs_new = GEOSCoordSeq_create(pointsNum + additionalPoints, coordinatesNum)) == NULL) {
		*outGeometry = NULL;
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
		goto bailout;
	}
	//add the first point
	if (!GEOSCoordSeq_setX(gcs_new, 0, xCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_setX failed");
		GEOSCoordSeq_destroy(gcs_new);
		goto bailout;
	}
	if (!GEOSCoordSeq_setY(gcs_new, 0, yCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_setY failed");
		GEOSCoordSeq_destroy(gcs_new);
		goto bailout;
	}
	if (coordinatesNum > 2 && !GEOSCoordSeq_setZ(gcs_new, 0, zCoords_org[0])) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSCoordSeq_setZ failed");
		GEOSCoordSeq_destroy(gcs_new);
		goto bailout;
	}
...
//fprintf(stderr, "OLD : (%f, %f, %f) vs (%f, %f, %f) = %f\n", xl, yl, zl, xCoords_org[i], yCoords_org[i], zCoords_org[i], dist);
	...
	if (*outGeometry == NULL) {
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGeom_%s failed", isRing ? "LinearRing" : "LineString");
		GEOSCoordSeq_destroy(gcs_new);
	}
...
segmentizePolygon(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double sz)
{
	...
	if (exteriorRingGeometry == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	}
	...
	if (numInteriorRings == -1) {
		*outGeometry = NULL;
		GEOSGeom_destroy(transformedExteriorRingGeometry);
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGetInteriorRingN failed.");
	}
	...
	*outGeometry = GEOSGeom_createPolygon(transformedExteriorRingGeometry, transformedInteriorRingGeometries, numInteriorRings);
	if (*outGeometry == NULL) {
		for (i = 0; i < numInteriorRings; i++)
			GEOSGeom_destroy(transformedInteriorRingGeometries[i]);
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGeom_createPolygon failed");
	}
	GDKfree(transformedInteriorRingGeometries);
	GEOSGeom_destroy(transformedExteriorRingGeometry);
...
segmentizeMultiGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double sz)
{
	...
	*outGeometry = GEOSGeom_createCollection(GEOSGeomTypeId(geosGeometry), transformedMultiGeometries, geometriesNum);
	if (*outGeometry == NULL) {
		for (i = 0; i < geometriesNum; i++)
			GEOSGeom_destroy(transformedMultiGeometries[i]);
		err = createException(MAL, "geom.Segmentize", "SQLSTATE ----- !""GEOSGeom_createCollection failed");
	}
	GDKfree(transformedMultiGeometries);
...
segmentizeGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double sz)
{
	...
	default:
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
wkbSegmentize(wkb **outWKB, wkb **geomWKB, dbl *sz)
{
	...
	if (geosGeometry == NULL) {
		*outWKB = NULL;
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	*outWKB = geos2wkb(outGeometry);
	...
	if (*outWKB == NULL)
		throw(MAL, "geom.Segmentize", "SQLSTATE ----- !""geos2wkb failed");
...
translateCoordSeq(int idx, int coordinatesNum, double dx, double dy, double dz, const GEOSCoordSequence *gcs_old, GEOSCoordSeq gcs_new)
{
	...
	//get the coordinates
	if (!GEOSCoordSeq_getX(gcs_old, idx, &x))
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_getX failed");
	if (!GEOSCoordSeq_getY(gcs_old, idx, &y))
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_getY failed");
	if (coordinatesNum > 2)
		if (!GEOSCoordSeq_getZ(gcs_old, idx, &z))
			throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_getZ failed");

	//create new coordinates moved by dx, dy, dz
	if (!GEOSCoordSeq_setX(gcs_new, idx, (x + dx)))
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_setX failed");
	if (!GEOSCoordSeq_setY(gcs_new, idx, (y + dy)))
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_setY failed");
	if (coordinatesNum > 2)
		if (!GEOSCoordSeq_setZ(gcs_new, idx, (z + dz)))
			throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_setZ failed");
...
translatePoint(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	if (gcs_old == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
	}
	...
	if (gcs_new == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
	}
	...
	*outGeometry = GEOSGeom_createPoint(gcs_new);
	if (*outGeometry == NULL) {
		err = createException(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_createPoint failed");
		GEOSCoordSeq_destroy(gcs_new);
	}
...
translateLineString(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	*outGeometry = GEOSGeom_createLineString(gcs_new);
	if (*outGeometry == NULL) {
		err = createException(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_createLineString failed");
		GEOSCoordSeq_destroy(gcs_new);
	}
...
translateLinearRing(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	*outGeometry = GEOSGeom_createLinearRing(gcs_new);
	if (*outGeometry == NULL) {
		err = createException(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_createLinearRing failed");
		GEOSCoordSeq_destroy(gcs_new);
	}
...
translatePolygon(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	if (exteriorRingGeometry == NULL) {
		*outGeometry = NULL;
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	}
	...
	if (numInteriorRings == -1) {
		*outGeometry = NULL;
		GEOSGeom_destroy(transformedExteriorRingGeometry);
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGetInteriorRingN failed.");
	}
	...
	*outGeometry = GEOSGeom_createPolygon(transformedExteriorRingGeometry, transformedInteriorRingGeometries, numInteriorRings);
	if (*outGeometry == NULL) {
		for (i = 0; i < numInteriorRings; i++)
			GEOSGeom_destroy(transformedInteriorRingGeometries[i]);
		err = createException(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_createPolygon failed");
	}
	GDKfree(transformedInteriorRingGeometries);
	GEOSGeom_destroy(transformedExteriorRingGeometry);
...
translateMultiGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	*outGeometry = GEOSGeom_createCollection(GEOSGeomTypeId(geosGeometry), transformedMultiGeometries, geometriesNum);
	if (*outGeometry == NULL) {
		for (i = 0; i < geometriesNum; i++)
			GEOSGeom_destroy(transformedMultiGeometries[i]);
		err = createException(MAL, "geom.Translate", "SQLSTATE ----- !""GEOSGeom_createCollection failed");
	}
	GDKfree(transformedMultiGeometries);
...
translateGeometry(GEOSGeometry **outGeometry, const GEOSGeometry *geosGeometry, double dx, double dy, double dz)
{
	...
	default:
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
wkbTranslate(wkb **outWKB, wkb **geomWKB, dbl *dx, dbl *dy, dbl *dz)
{
	...
	if (geosGeometry == NULL) {
		*outWKB = NULL;
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	*outWKB = geos2wkb(outGeometry);
	...
	if (*outWKB == NULL)
		throw(MAL, "geom.Translate", "SQLSTATE ----- !""geos2wkb failed");
...
wkbDelaunayTriangles(wkb **outWKB, wkb **geomWKB, dbl *tolerance, int *flag)
{
	...
	if (outGeometry == NULL) {
		*outWKB = NULL;
		throw(MAL, "geom.DelaunayTriangles", "SQLSTATE ----- !""GEOSDelaunayTriangulation failed");
	}
	...
	*outWKB = geos2wkb(outGeometry);
	...
	if (*outWKB == NULL)
		throw(MAL, "geom.DelaunayTriangles", "SQLSTATE ----- !""geos2wkb failed");
...
wkbPointOnSurface(wkb **resWKB, wkb **geomWKB)
{
	...
	if (geosGeometry == NULL) {
		*resWKB = NULL;
		throw(MAL, "geom.PointOnSurface", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if (resGeosGeometry == NULL) {
		*resWKB = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.PointOnSurface", "SQLSTATE ----- !""GEOSPointOnSurface failed");
	}
	...
	*resWKB = geos2wkb(resGeosGeometry);
	...
	if (*resWKB == NULL)
		throw(MAL, "geom.PointOnSurface", "SQLSTATE ----- !""geos2wkb failed");
...
dumpGeometriesSingle(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, unsigned int *lvl, const char *path)
{
	...
	if (singleWKB == NULL)
		throw(MAL, "geom.Dump", "SQLSTATE ----- !""geos2wkb failed");
	...
	if (BUNappend(idBAT, newPath, TRUE) != GDK_SUCCEED ||
		...
		err = createException(MAL, "geom.Dump", "SQLSTATE ----- !""BUNappend failed");
...
dumpGeometriesMulti(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, const char *path)
{
	...
	for (i = 0; i < geometriesNum; i++) {
		...
		if (multiGeometry == NULL) {
			err = createException(MAL, "geom.Dump", "SQLSTATE ----- !""GEOSGetGeometryN failed");
			break;
		}
		lvl++;
...
dumpGeometriesGeometry(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, const char *path)
{
	...
	default:
		throw(MAL, "geom.Dump", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
wkbDump(bat *idBAT_id, bat *geomBAT_id, wkb **geomWKB)
{
	...
	if (wkb_isnil(*geomWKB)) {
		...
		if ((idBAT = COLnew(0, TYPE_str, 0, TRANSIENT)) == NULL) {
			*idBAT_id = bat_nil;
			throw(MAL, "geom.DumpPoints", "SQLSTATE ----- !""Error creating new BAT");
		}
	...
	if ((idBAT = COLnew(0, TYPE_str, geometriesNum, TRANSIENT)) == NULL) {
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.Dump", "SQLSTATE ----- !""Error creating new BAT");
	}
...
dumpPointsLineString(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, const char *path)
{
	...
	for (i = 0; i < pointsNum && err == MAL_SUCCEED; i++) {
		...
		if (pointGeometry == NULL)
			throw(MAL, "geom.DumpPoints", "SQLSTATE ----- !""GEOSGeomGetPointN failed");
...
dumpPointsPolygon(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, unsigned int *lvl, const char *path)
{
	...
	exteriorRingGeometry = GEOSGetExteriorRing(geosGeometry);
	if (exteriorRingGeometry == NULL)
		throw(MAL, "geom.DumpPoints", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	...
	numInteriorRings = GEOSGetNumInteriorRings(geosGeometry);
	if (numInteriorRings == -1)
		throw(MAL, "geom.DumpPoints", "SQLSTATE ----- !""GEOSGetNumInteriorRings failed");
...
dumpPointsGeometry(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, const char *path)
{
	...
	default:
		throw(MAL, "geom.DumpPoints", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
geom_2_geom(wkb **resWKB, wkb **valueWKB, int *columnType, int *columnSRID)
{
	...
	if (geosGeometry == NULL)
		throw(MAL, "calc.wkb", "SQLSTATE ----- !""wkb2geos failed");
	...
	if (valueSRID != *columnSRID || valueType != *columnType) {
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "calc.wkb", "SQLSTATE ----- !""column needs geometry(%d, %d) and value is geometry(%d, %d)\n", *columnType, *columnSRID, valueType, valueSRID);
	}
	...
	*resWKB = geos2wkb(geosGeometry);
	...
	if (*resWKB == NULL)
		throw(MAL, "calc.wkb", "SQLSTATE ----- !""geos2wkb failed");
...
wkbFROMSTR_withSRID(char *geomWKT, int *len, wkb **geomWKB, int srid, size_t *nread)
{
	...
	*geomWKB = NULL;
	...
	if (WKT_reader == NULL) {
		if (geomWKT_original)
			GDKfree(geomWKT);
		throw(MAL, "wkb.FromText", "SQLSTATE ----- !""GEOSWKTReader_create failed");
	}
	...
	if (geosGeometry == NULL) {
		if (geomWKT_original)
			GDKfree(geomWKT);
		throw(MAL, "wkb.FromText", "SQLSTATE ----- !""GEOSWKTReader_read failed");
	}
	...
	if (GEOSGeomTypeId(geosGeometry) == -1) {
		if (geomWKT_original)
			GDKfree(geomWKT);
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "wkb.FromText", "SQLSTATE ----- !""GEOSGeomTypeId failed");
	}
	...
	*geomWKB = geos2wkb(geosGeometry);
	...
	if (*geomWKB == NULL) {
		if (geomWKT_original)
			GDKfree(geomWKT);
		throw(MAL, "wkb.FromText", "SQLSTATE ----- !""geos2wkb failed");
	}
...
wkbFromBinary(wkb **geomWKB, char **inStr)
{
	...
	if (strLength & 1)
		throw(MAL, "geom.FromBinary", "SQLSTATE ----- !""odd length input string");
	...
	for (i = 0; i < strLength; i += 2) {
		...
		if (firstHalf == -1 || secondHalf == -1) {
			GDKfree(w);
			throw(MAL, "geom.FromBinary", "SQLSTATE ----- !""incorrectly formatted input string");
		}
...
wkbFromText(wkb **geomWKB, str *geomWKT, int *srid, int *tpe)
{
	...
	*geomWKB = NULL;
	...
	te += (te > 2);
	if (*tpe > 0 && te != *tpe)
		throw(SQL, "wkb.FromText", "SQLSTATE ----- !""Geometry not type '%d: %s' but '%d: %s' instead", *tpe, geom_type2str(*tpe, 0), te, geom_type2str(te, 0));
	throw(MAL, "wkb.FromText", "SQLSTATE ----- !""%s", "cannot parse string");
}
...
wkbAsText(char **txt, wkb **geomWKB, int *withSRID)
{
	...
	if (wkbTOSTR(&wkt, &len, *geomWKB) == 0)
		throw(MAL, "geom.AsText", "SQLSTATE ----- !""Failed to create Text from Well Known Format");
	...
	*geomWKB = NULL;
	...
	if (*flag == 0) {
		...
		if (externalGeometry == NULL) {
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""Error in wkb2geos");
			goto bailout;
		}
		...
		if (coordSeq_external == NULL) {
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""GEOSCoordSeq_clone failed");
			goto bailout;
		}
		...
		if (linearRingExternalGeometry == NULL) {
			...
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""GEOSGeom_createLinearRing failed");
			goto bailout;
		}
		...
		if (finalGeometry == NULL) {
			...
			GDKfree(internalGeometries);
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""Error creating Polygon from LinearRing");
			goto bailout;
		}
		GDKfree(internalGeometries);
		...
		if (GEOSisValid(finalGeometry) != 1) {
			...
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""The provided MultiLineString does not create a valid Polygon");
			goto bailout;
		}
		...
		*geomWKB = geos2wkb(finalGeometry);
		GEOSGeom_destroy(finalGeometry);
		if (*geomWKB == NULL)
			ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""geos2wkb failed");
	} else if (*flag == 1) {
		ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""Multipolygon from string has not been defined");
	} else {
		ret = createException(MAL, "geom.MLineStringToPolygon", "SQLSTATE ----- !""Unknown flag");
	}
...
wkbMakePoint(wkb **out, dbl *x, dbl *y, dbl *z, dbl *m, int *zmFlag)
{
	...
	case 10:		/* x, y, z */
		...
		break;
	case 11:		/* x, y, z, m */
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !""POINTZM is not supported");
	default:
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !"ILLEGAL_ARGUMENT);
	}

	if (seq == NULL)
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
	...
	if (!GEOSCoordSeq_setOrdinate(seq, 0, 0, *x) ||
		...
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !""GEOSCoordSeq_setOrdinate failed");
	}

	if ((geosGeometry = GEOSGeom_createPoint(seq)) == NULL) {
		GEOSCoordSeq_destroy(seq);
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !""Failed to create GEOSGeometry from the coordinates");
	}
	...
	*out = geos2wkb(geosGeometry);
	...
	if (wkb_isnil(*out)) {
		GDKfree(*out);
		*out = NULL;
		throw(MAL, "geom.MakePoint", "SQLSTATE ----- !""Failed to create WKB from GEOSGeometry");
	}
...
wkbBasicInt(int *out, wkb *geom, int (*func) (const GEOSGeometry *), const char *name)
{
	...
	if ((geosGeometry = wkb2geos(geom)) == NULL)
		throw(MAL, name, "SQLSTATE ----- !""wkb2geos failed");
	...
	*out = (*func) (geosGeometry);
	...
	if (GDKerrbuf && GDKerrbuf[0]) {
		...
		ret = createException(MAL, name, "SQLSTATE ----- !""%s", GDKerrbuf);
...
wkbSetSRID(wkb **resultGeomWKB, wkb **geomWKB, int *srid)
{
	...
	if ((geosGeometry = wkb2geos(*geomWKB)) == NULL)
		throw(MAL, "geom.setSRID", "SQLSTATE ----- !""wkb2geos failed");
	...
	*resultGeomWKB = geos2wkb(geosGeometry);
	...
	if (*resultGeomWKB == NULL)
		throw(MAL, "geom.setSRID", "SQLSTATE ----- !""geos2wkb failed");
...
wkbGetCoordinate(dbl *out, wkb **geom, int *dimNum)
{
	...
	if (geosGeometry == NULL) {
		*out = dbl_nil;
		throw(MAL, "geom.GetCoordinate", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if ((GEOSGeomTypeId(geosGeometry) + 1) != wkbPoint_mdb) {
		...
		if ((err = wkbAsText(&geomSTR, geom, NULL)) != MAL_SUCCEED)
			return err;
		err = createException(MAL, "geom.GetCoordinate", "SQLSTATE ----- !""Geometry %s not a Point", geomSTR);
		GDKfree(geomSTR);
		return err;
	}
	...
	if (gcs == NULL) {
		err = createException(MAL, "geom.GetCoordinate", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
	} else if (!GEOSCoordSeq_getOrdinate(gcs, 0, *dimNum, out))
		err = createException(MAL, "geom.GetCoordinate", "SQLSTATE ----- !""GEOSCoordSeq_getOrdinate failed");
	GEOSGeom_destroy(geosGeometry);
...
wkbBasic(wkb **out, wkb **geom, GEOSGeometry *(*func) (const GEOSGeometry *), const char *name)
{
	...
	if ((outGeometry = (*func) (geosGeometry)) == NULL) {
		err = createException(MAL, name, "SQLSTATE ----- !""GEOS%s failed", name + 5);
	} else {
...
wkbEnvelopeFromCoordinates(wkb **out, dbl *xmin, dbl *ymin, dbl *xmax, dbl *ymax, int *srid)
{
	...
	if ((coordSeq = GEOSCoordSeq_create(5, 2)) == NULL)
		throw(MAL, "geom.MakeEnvelope", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
	...
	if (!GEOSCoordSeq_setX(coordSeq, 0, *xmin) ||
		...
		GEOSCoordSeq_destroy(coordSeq);
		throw(MAL, "geom.MakeEnvelope", "SQLSTATE ----- !""GEOSCoordSeq_setX/Y failed");
	}
	...
	if (linearRingGeometry == NULL) {
		...
		GEOSCoordSeq_destroy(coordSeq);
		throw(MAL, "geom.MakeEnvelope", "SQLSTATE ----- !""Error creating LinearRing from coordinates");
	}

	geosGeometry = GEOSGeom_createPolygon(linearRingGeometry, NULL, 0);
	if (geosGeometry == NULL) {
		GEOSGeom_destroy(linearRingGeometry);
		throw(MAL, "geom.MakeEnvelope", "SQLSTATE ----- !""Error creating Polygon from LinearRing");
	}
...
wkbMakePolygon(wkb **out, wkb **external, bat *internalBAT_id, int *srid)
{
	...
	if ((GEOSGeomTypeId(externalGeometry) + 1) != wkbLineString_mdb) {
		*out = NULL;
		GEOSGeom_destroy(externalGeometry);
		throw(MAL, "geom.Polygon", "SQLSTATE ----- !""Geometries should be LineString");
	}
	...
	if (!closed) {
		*out = NULL;
		GEOSGeom_destroy(externalGeometry);
		throw(MAL, "geom.Polygon", "SQLSTATE ----- !""LineString should be closed");
	}
	...
	if (coordSeq_copy == NULL)
		throw(MAL, "geom.Polygon", "SQLSTATE ----- !""GEOSCoordSeq_clone failed");
	...
	if (linearRingGeometry == NULL) {
		...
		throw(MAL, "geom.Polygon", "SQLSTATE ----- !""GEOSGeom_createLinearRing failed");
	}
	...
	if (internalBAT_id == NULL) {
		...
		if (geosGeometry == NULL) {
			*out = NULL;
			GEOSGeom_destroy(linearRingGeometry);
			throw(MAL, "geom.Polygon", "SQLSTATE ----- !""Error creating Polygon from LinearRing");
		}
	} else {
...
wkbMakeLine(wkb **out, wkb **geom1WKB, wkb **geom2WKB)
{
	...
	*out = NULL;
	...
	if (!geom1Geometry) {
		*out = NULL;
		throw(MAL, "geom.MakeLine", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if (GEOSGetSRID(geom1Geometry) != GEOSGetSRID(geom2Geometry)) {
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""Geometries of different SRID");
		goto bailout;
	}
	...
	if (GEOSGeomTypeId(geom1Geometry) + 1 != wkbPoint_mdb &&
		...
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""Geometries should be Point or LineString");
		goto bailout;
	}
	...
	if ((geom1CoordSeq = GEOSGeom_getCoordSeq(geom1Geometry)) == NULL ||
		...
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");
		goto bailout;
	}
	...
	if (!GEOSCoordSeq_getDimensions(geom1CoordSeq, &geom1Dimension) ||
		...
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSGeom_getDimensions failed");
		goto bailout;
	}
	if (geom1Dimension != geom2Dimension) {
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""Geometries should be of the same dimension");
		goto bailout;
	}
	...
	if (!GEOSCoordSeq_getSize(geom1CoordSeq, &geom1Size) ||
		...
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSGeom_getSize failed");
		goto bailout;
	}
	//create the coordSeq for the new geometry
	if ((outCoordSeq = GEOSCoordSeq_create(geom1Size + geom2Size, geom1Dimension)) == NULL) {
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSCoordSeq_create failed");
		goto bailout;
	}
	for (i = 0; i < geom1Size; i++) {
		...
		if (!GEOSCoordSeq_setX(outCoordSeq, i, x) ||
			...
			err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSCoordSeq_set[XY] failed");
			goto bailout;
		}
		if (geom1Dimension > 2) {
			GEOSCoordSeq_getZ(geom1CoordSeq, i, &z);
			if (!GEOSCoordSeq_setZ(outCoordSeq, i, z)) {
				err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSCoordSeq_setZ failed");
				goto bailout;
			}
		}
	}
	...
	if ((outGeometry = GEOSGeom_createLineString(outCoordSeq)) == NULL) {
		err = createException(MAL, "geom.MakeLine", "SQLSTATE ----- !""GEOSGeom_createLineString failed");
		goto bailout;
	}
	outCoordSeq = NULL;
...
wkbMakeLineAggr(wkb **outWKB, bat *inBAT_id)
{
	...
	if ((inBAT = BATdescriptor(*inBAT_id)) == NULL) {
		throw(MAL, "geom.MakeLine", "SQLSTATE ----- !"RUNTIME_OBJECT_MISSING);
	}
...
wkbBorderPoint(wkb **out, wkb **geom, GEOSGeometry *(*func) (const GEOSGeometry *), const char *name)
{
	...
	*out = NULL;
	...
	if (GEOSGeomTypeId(geosGeometry) != GEOS_LINESTRING) {
		err = createException(MAL, name, "SQLSTATE ----- !""Geometry not a LineString");
	} else {
		new = (*func) (geosGeometry);
		if (new == NULL) {
			err = createException(MAL, name, "SQLSTATE ----- !""GEOSGeomGet%s failed", name + 5);
		} else {
			*out = geos2wkb(new);
...
numPointsLineString(unsigned int *out, const GEOSGeometry *geosGeometry)
{
	...
	if (coordSeq == NULL)
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""GEOSGeom_getCoordSeq failed");

	/* get the number of points in the geometry */
	if (!GEOSCoordSeq_getSize(coordSeq, out)) {
		*out = int_nil;
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""GEOSGeomGetNumPoints failed");
	}
...
numPointsPolygon(unsigned int *out, const GEOSGeometry *geosGeometry)
{
	...
	if (!exteriorRingGeometry) {
		*out = int_nil;
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	}
	//get the points in the exterior ring
	...
	if (numInteriorRings == -1) {
		*out = int_nil;
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""GEOSGetNumInteriorRings failed");
	}
	// iterate over the interiorRing and transform each one of them
	for (i = 0; i < numInteriorRings; i++) {
...
numPointsGeometry(unsigned int *out, const GEOSGeometry *geosGeometry)
{
	...
	default:
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""%s Unknown geometry type", geom_type2str(geometryType, 0));
	}
}
...
wkbNumPoints(int *out, wkb **geom, int *check)
{
	...
	if (geosGeometry == NULL) {
		*out = int_nil;
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if (*check && geometryType != wkbLineString_mdb) {
		*out = int_nil;
		...
		if ((err = wkbAsText(&geomSTR, geom, NULL)) == MAL_SUCCEED) {
			err = createException(MAL, "geom.NumPoints", "SQLSTATE ----- !""Geometry %s not a LineString", geomSTR);
			GDKfree(geomSTR);
		}
		return err;
	}
	...
	if (pointsNum > INT_MAX) {
		GEOSGeom_destroy(geosGeometry);
		*out = int_nil;
		throw(MAL, "geom.NumPoints", "SQLSTATE ----- !""Overflow");
	}
...
wkbPointN(wkb **out, wkb **geom, int *n)
{
	...
	if (geosGeometry == NULL) {
		*out = NULL;
		throw(MAL, "geom.PointN", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (GEOSGeomTypeId(geosGeometry) != GEOS_LINESTRING) {
		*out = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.PointN", "SQLSTATE ----- !""Geometry not a LineString");
	}
	...
	if (rN == -1) {
		*out = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.PointN", "SQLSTATE ----- !""GEOSGeomGetNumPoints failed");
	}

	if (rN <= *n || *n < 0) {
		*out = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.PointN", "SQLSTATE ----- !""Unable to retrieve point %d (not enough points)", *n);
	}

	if ((new = GEOSGeomGetPointN(geosGeometry, *n)) == NULL) {
		err = createException(MAL, "geom.PointN", "SQLSTATE ----- !""GEOSGeomGetPointN failed");
	} else {
...
wkbExteriorRing(wkb **exteriorRingWKB, wkb **geom)
{
	...
	if (geosGeometry == NULL) {
		*exteriorRingWKB = NULL;
		throw(MAL, "geom.ExteriorRing", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (GEOSGeomTypeId(geosGeometry) != GEOS_POLYGON) {
		*exteriorRingWKB = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.ExteriorRing", "SQLSTATE ----- !""Geometry not a Polygon");

	}
	/* get the exterior ring of the geometry */
	if ((exteriorRingGeometry = GEOSGetExteriorRing(geosGeometry)) == NULL)
		err = createException(MAL, "geom.ExteriorRing", "SQLSTATE ----- !""GEOSGetExteriorRing failed");
	else {
...
wkbInteriorRingN(wkb **interiorRingWKB, wkb **geom, int *ringNum)
{
	...
	*interiorRingWKB = NULL;
	...
	if (geosGeometry == NULL) {
		*interiorRingWKB = NULL;
		throw(MAL, "geom.InteriorRingN", "SQLSTATE ----- !""wkb2geos failed");
	}

	if ((GEOSGeomTypeId(geosGeometry) + 1) != wkbPolygon_mdb) {
		*interiorRingWKB = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.InteriorRingN", "SQLSTATE ----- !""Geometry not a Polygon");
	...
	if (rN == -1) {
		*interiorRingWKB = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.InteriorRingN", "SQLSTATE ----- !""GEOSGetInteriorRingN failed.");
	}
	...
	if ((interiorRingGeometry = GEOSGetInteriorRingN(geosGeometry, *ringNum - 1)) == NULL) {
		err = createException(MAL, "geom.InteriorRingN", "SQLSTATE ----- !""GEOSGetInteriorRingN failed");
	} else {
		/* get the wkb representation of it */
...
wkbInteriorRings(wkba **geomArray, wkb **geomWKB)
{
	...
	geosGeometry = wkb2geos(*geomWKB);
	if (geosGeometry == NULL) {
		throw(MAL, "geom.InteriorRings", "SQLSTATE ----- !""Error in wkb2geos");
	}

	if ((GEOSGeomTypeId(geosGeometry) + 1) != wkbPolygon_mdb) {
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.interiorRings", "SQLSTATE ----- !""Geometry not a Polygon");
	...
	*geomArray = GDKmalloc(wkba_size(interiorRingsNum));
	...
	for (i = 0; i < interiorRingsNum; i++) {
		...
		if (interiorRingGeometry == NULL) {
			...
			GEOSGeom_destroy(geosGeometry);
			*geomArray = NULL;
			throw(MAL, "geom.InteriorRings", "SQLSTATE ----- !""GEOSGetInteriorRingN failed");
		}
		// get the wkb representation of it
...
wkbNumRings(int *out, wkb **geom, int *exteriorRing)
{
	...
	if (geosGeometry == NULL)
		throw(MAL, "geom.NumRings", "SQLSTATE ----- !""Problem converting WKB to GEOS");
...
geosIsClosed(bit *out, const GEOSGeometry *geosGeometry)
{
	...
	*out = bit_nil;
	...
	switch (geometryType) {
	case -1:
		throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""GEOSGeomTypeId failed");
	case wkbPoint_mdb:
	case wkbPolygon_mdb:
	case wkbMultiPoint_mdb:
	case wkbMultiPolygon_mdb:
	...
	case wkbLineString_mdb:
		...
		if ((i = GEOSisClosed(geosGeometry)) == 2)
			throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""GEOSisClosed failed");
		*out = i;
		break;
	case wkbMultiLineString_mdb:
	case wkbGeometryCollection_mdb:
		...
		geometriesNum = GEOSGetNumGeometries(geosGeometry);
		if (geometriesNum < 0)
			throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""GEOSGetNumGeometries failed");

		for (i = 0; i < geometriesNum; i++) {
			const GEOSGeometry *gN = GEOSGetGeometryN(geosGeometry, i);
			if (!gN)
				throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""GEOSGetGeometryN failed");
		...
		break;
	default:
		throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""Unknown geometry type");
	}
...
wkbIsClosed(bit *out, wkb **geomWKB)
{
	...
	geosGeometry = wkb2geos(*geomWKB);
	if (geosGeometry == NULL)
		throw(MAL, "geom.IsClosed", "SQLSTATE ----- !""wkb2geos failed");
...
wkbIsValidReason(char **reason, wkb **geomWKB)
{
	...
	geosGeometry = wkb2geos(*geomWKB);
	if (geosGeometry == NULL)
		throw(MAL, "geom.IsValidReason", "SQLSTATE ----- !""wkb2geom failed");
	...
	if (GEOSReason == NULL)
		throw(MAL, "geom.IsValidReason", "SQLSTATE ----- !""GEOSisValidReason failed");
...
wkbIsValidDetail(char **out, wkb **geom)
{
	...
	if ((geosGeometry = wkb2geos(*geom)) == NULL) {
		*out = NULL;
		throw(MAL, "geom.IsValidDetail", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if (res == 2) {
		throw(MAL, "geom.IsValidDetail", "SQLSTATE ----- !""GEOSisValidDetail failed");
	}
...
wkbArea(dbl *out, wkb **geomWKB)
{
	...
	if (geosGeometry == NULL) {
		*out = dbl_nil;
		throw(MAL, "geom.Area", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (!GEOSArea(geosGeometry, out)) {
		GEOSGeom_destroy(geosGeometry);
		*out = dbl_nil;
		throw(MAL, "geom.Area", "SQLSTATE ----- !""GEOSArea failed");
	}
...
wkbDistance(dbl *out, wkb **a, wkb **b)
{
	...
	if (ga == NULL || gb == NULL) {
		...
		if (gb)
			GEOSGeom_destroy(gb);
		*out = dbl_nil;
		throw(MAL, "geom.Distance", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (GEOSGetSRID(ga) != GEOSGetSRID(gb)) {
		err = createException(MAL, "geom.Distance", "SQLSTATE ----- !""Geometries of different SRID");
	} else if (!GEOSDistance(ga, gb, out)) {
		err = createException(MAL, "geom.Distance", "SQLSTATE ----- !""GEOSDistance failed");
	}
...
wkbLength(dbl *out, wkb **a)
{
	...
	if (geosGeometry == NULL) {
		throw(MAL, "geom.Length", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (!GEOSLength(geosGeometry, out))
		err = createException(MAL, "geom.Length", "SQLSTATE ----- !""GEOSLength failed");
...
wkbConvexHull(wkb **out, wkb **geom)
{
	...
	if ((convexHullGeometry = GEOSConvexHull(geosGeometry)) == NULL) {
		...
		*out = geos2wkb(convexHullGeometry);
		...
		if (*out == NULL)
			ret = createException(MAL, "geom.ConvexHull", "SQLSTATE ----- !""geos2wkb failed");
	}
	GEOSGeom_destroy(geosGeometry);
...
wkbanalysis(wkb **out, wkb **geom1WKB, wkb **geom2WKB, GEOSGeometry *(*func) (const GEOSGeometry *, const GEOSGeometry *), const char *name)
{
	...
	//make sure the geometries are of the same srid
	if (GEOSGetSRID(geom1Geometry) != GEOSGetSRID(geom2Geometry)) {
		err = createException(MAL, name, "SQLSTATE ----- !""Geometries of different SRID");
...
wkbUnionAggr(wkb **outWKB, bat *inBAT_id)
{
	...
	if (!(inBAT = BATdescriptor(*inBAT_id))) {
		throw(MAL, "geom.Union", "SQLSTATE ----- !""Problem retrieving BATs");
	}
...
wkbBuffer(wkb **out, wkb **geom, dbl *distance)
{
	...
	geosGeometry = wkb2geos(*geom);
	if (geosGeometry == NULL) {
		throw(MAL, "geom.Buffer", "SQLSTATE ----- !""wkb2geos failed");
	}

	if ((new = GEOSBuffer(geosGeometry, *distance, 18)) == NULL) {
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.Buffer", "SQLSTATE ----- !""GEOSBuffer failed");
	}
	*out = geos2wkb(new);
	GEOSGeom_destroy(new);
	GEOSGeom_destroy(geosGeometry);
...
wkbspatial(bit *out, wkb **geomWKB_a, wkb **geomWKB_b, char (*func) (const GEOSGeometry *, const GEOSGeometry *), const char *name)
{
	...
	if (GEOSGetSRID(geosGeometry_a) != GEOSGetSRID(geosGeometry_b)) {
		GEOSGeom_destroy(geosGeometry_a);
		GEOSGeom_destroy(geosGeometry_b);
		throw(MAL, name, "SQLSTATE ----- !""Geometries of different SRID");
	}
	...
	if (res == 2)
		throw(MAL, name, "SQLSTATE ----- !""GEOS%s failed", name + 5);
...
wkbRelate(bit *out, wkb **geomWKB_a, wkb **geomWKB_b, str *pattern)
{
	...
	if (geosGeometry_a == NULL || geosGeometry_b == NULL) {
		...
		if (geosGeometry_b)
			GEOSGeom_destroy(geosGeometry_b);
		throw(MAL, "geom.RelatePattern", "SQLSTATE ----- !""wkb2geos failed");
	}

	if (GEOSGetSRID(geosGeometry_a) != GEOSGetSRID(geosGeometry_b)) {
		GEOSGeom_destroy(geosGeometry_a);
		GEOSGeom_destroy(geosGeometry_b);
		throw(MAL, "geom.RelatePattern", "SQLSTATE ----- !""Geometries of different SRID");
	}
	...
	if (res == 2)
		throw(MAL, "geom.RelatePattern", "SQLSTATE ----- !""GEOSRelatePattern failed");
...
wkbGeometryN(wkb **out, wkb **geom, const int *geometryNum)
{
	...
	if (geosGeometry == NULL) {
		*out = NULL;
		throw(MAL, "geom.GeometryN", "SQLSTATE ----- !""wkb2geos failed");
	}
	...
	if (geometriesNum < 0) {
		*out = NULL;
		GEOSGeom_destroy(geosGeometry);
		throw(MAL, "geom.GeometryN", "SQLSTATE ----- !""GEOSGetNumGeometries failed");
	}
...
wkbNumGeometries(int *out, wkb **geom)
{
	...
	if (geosGeometry == NULL) {
		*out = int_nil;
		throw(MAL, "geom.NumGeometries", "SQLSTATE ----- !""wkb2geos failed");
	}
...
wkbMBR(mbr **geomMBR, wkb **geomWKB)
{
	...
	if (geosGeometry == NULL) {
		*geomMBR = NULL;
		throw(MAL, "geom.MBR", "SQLSTATE ----- !""Problem converting GEOS to WKB");
	}
	...
	*geomMBR = mbrFromGeos(geosGeometry);
	...
	if (*geomMBR == NULL || mbr_isnil(*geomMBR)) {
		GDKfree(*geomMBR);
		*geomMBR = NULL;
		throw(MAL, "wkb.mbr", "SQLSTATE ----- !""Failed to create mbr");
	}
...
wkbBox2D(mbr **box, wkb **point1, wkb **point2)
{
	...
	if (GEOSGeomTypeId(point1_geom) + 1 != wkbPoint_mdb ||
		...
		err = createException(MAL, "geom.MakeBox2D", "SQLSTATE ----- !""Geometries should be points");
		...
		err = createException(MAL, "geom.MakeBox2D", "SQLSTATE ----- !""Error in reading the points' coordinates");
	} else {
		//Assign the coordinates. Ensure that they are in correct order
		*box = GDKmalloc(sizeof(mbr));
		if (*box == NULL) {
			err = createException(MAL, "SQLSTATE ----- !""geom.MakeBox2D", MAL_MALLOC_FAIL);
		} else {
...
wkbCoordinateFromMBR(dbl *coordinateValue, mbr **geomMBR, int *coordinateIdx)
{
	...
	case 4:
		*coordinateValue = (*geomMBR)->ymax;
		break;
	default:
		throw(MAL, "geom.coordinateFromMBR", "SQLSTATE ----- !""Unrecognized coordinateIdx: %d\n", *coordinateIdx);
	}
...
mbrFromString(mbr **w, str *src)
{
	...
	ex = createException(MAL, "mbr.FromString", "SQLSTATE ----- !""%s", errbuf);
...
pnpoly(int *out, int nvert, dbl *vx, dbl *vy, bat *point_x, bat *point_y)
{
	...
	if ((bpx = BATdescriptor(*point_x)) == NULL) {
		throw(MAL, "geom.point", "SQLSTATE ----- !"RUNTIME_OBJECT_MISSING);
	}
	...
	if (bpx->hseqbase != bpy->hseqbase || BATcount(bpx) != BATcount(bpy)) {
		...
		throw(MAL, "geom.point", "SQLSTATE ----- !""both point bats must have dense and aligned heads");
	}
