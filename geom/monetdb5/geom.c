...
dumpPointsPolygon(BAT *idBAT, BAT *geomBAT, const GEOSGeometry *geosGeometry, unsigned int *lvl, unsigned *num_points, const char *path)
{
	...
	if ( (newPath = GDKmalloc(pathLength + lvlDigitsNum + extraLength + 1)) == NULL) {
...
wkbPolygonize_(wkb** outWKB, wkb** geom1, wkb** geom2){
    ...
    if ( (multiGeometry = GDKzalloc(sizeof(GEOSGeometry*) * (geos1Num + geos2Num))) == NULL) {
        GEOSGeom_destroy(geos1Geometry);
        GEOSGeom_destroy(geos2Geometry);
        *outWKB = NULL;
...
wkbFROMSTR_withSRID(char *geomWKT, int *len, wkb **geomWKB, int srid, size_t *nread)
{
	...
	*geomWKB = NULL;
	...
	if (strncasecmp(geomWKT, polyhedralSurface, strlen(polyhedralSurface)) == 0) {
		...
		if ( (geomWKT = GDKmalloc(sizeOfInfo + strlen(multiPolygon) + 1)) == NULL)
            throw(MAL, "wkb.FromText", MAL_MALLOC_FAIL);
...
static str
wkbaFROMSTR_withSRID(int *res, char *fromStr, int *len, wkba **toArray, int srid)
{
	int items, i;
	size_t skipBytes = 0;
...
wkbMLineStringToPolygon(wkb **geomWKB, str *geomWKT, int *srid, int *flag)
{
	...
	*geomWKB = NULL;
	...
	if (*flag == 0) {
		...
		GEOSGeom externalGeometry, linearRingExternalGeometry, *internalGeometries = NULL, finalGeometry;
...
wkbBasicBoolean(bit *out, wkb **geom, char (*func) (const GEOSGeometry *), const char *name)
{
	...
	if (GDKerrbuf && GDKerrbuf[0]) {
		//create an exception with this name
		msg = createException(MAL, name, "%s", GDKerrbuf);
	...
	if (ret == 2)
		throw(MAL, name, "GEOSis%s failed: %s", name + 7, msg);
...
wkbCollectAppend(wkb **out, wkb **geom1WKB, wkb **geom2WKB)
{
    ...
    if ( !empty && (GEOSGetSRID(geom1Geometry) != GEOSGetSRID(geom2Geometry))) {
        ...
        if ( (geomGeometries = (GEOSGeometry **) GDKmalloc(sizeof(GEOSGeometry *)*(num_geoms+1))) == NULL) {
            GEOSGeom_destroy(geom1Geometry);
            GEOSGeom_destroy(geom2Geometry);
            throw(MAL, "geom.Collect", MAL_MALLOC_FAIL);
        }
...
mbrFROMSTR(char *src, int *len, mbr **atom)
{
	...
	if (*len < (int) sizeof(mbr)) {
		...
		if ( (*atom = GDKmalloc(*len = sizeof(mbr))) == NULL) {
            //throw(MAL, "geom.mbrFROMSTR", MAL_MALLOC_FAIL);
            return 0;
        }
	}
	if (nil) {
...
wkbaTOSTR(char **toStr, int *len, wkba *fromArray)
{
	...
	char **partialStrs;
	char *nilStr = "nil";
	char *toStrPtr = NULL, *itemsNumStr = NULL;
    
    if ( (itemsNumStr = GDKmalloc((itemsNumDigits + 1) * sizeof(char))) == NULL) {
        //throw(MAL, "geom.wkbaTOSTR", MAL_MALLOC_FAIL);
        return 0;
    }
	...
	if ( (partialStrs = GDKzalloc(items * sizeof(char **))) == NULL) {
        GDKfree(itemsNumStr);
        //throw(MAL, "geom.wkbaTOSTR", MAL_MALLOC_FAIL);
        return 0;
    }
	...
	if (*len < (int) dataSize + 3 || *toStr == NULL) {
		GDKfree(*toStr);
		if ( (*toStr = GDKmalloc(*len = (int) dataSize + 3)) == NULL) {	/* plus quotes + termination character */
            //throw(MAL, "geom.mbrFROMSTR", MAL_MALLOC_FAIL);
            return 0;
        }
	}
...
{
    int res;
	wkbaFROMSTR_withSRID(&res, fromStr, len, toArray, 0);
    return res;
}
...
static str
WKBWKBtoBITsubjoin_intern_light(bat *lres, bat *rres, bat *lid, bat *rid, char (*func) (const GEOSGeometry *, const GEOSGeometry *), const char *name)
{
	BAT *xl, *xr, *bl, *br;
	oid lo, ro;
	...
	/*iterator over the BATs*/
	lBAT_iter = bat_iterator(bl);
	if ( (rBAT_iters = (BATiter*) GDKmalloc(sizeof(BATiter)*numThreads)) == NULL) {
		BBPunfix(*lid);
		BBPunfix(*rid);
		BBPunfix(xl->batCacheid);
		BBPunfix(xr->batCacheid);
...
ContainsXYZsubjoin_intern(bat *lres, bat *rres, bat *lid, bat *xid, bat*yid, bat *zid, int *srid, const char* name)
{
    ...
    BATloop(bl, pl, ql) {
        ...
        for (j = 0; j < BATcount(bx); j++) {
            ...
            if ( (err = pnpoly_(&outs[j], (int) verts->nvert, verts->vert_x, verts->vert_y, verts->nholes, verts->holes_x, verts->holes_y, verts->holes_n, x, y)) != MAL_SUCCEED) {
                msg = err;
#ifdef OPENMP
                #pragma omp cancelregion
#else
