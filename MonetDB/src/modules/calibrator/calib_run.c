...
void loads_nodelay(char **P, caliblng *j, caliblng *best)
{
	register char **p = P;
...
void loads_delay(char **P, caliblng *j, caliblng *best)
{
	register char **p = P;
	caliblng tries, i, time;
...
caliblng loads(char *array, caliblng range, caliblng stride, caliblng MHz, FILE *fp, int delay)
{
	register char **p = 0;
...
*/
	...
	*p = &(array[k]);
	...
	if(delay)	loads_delay  (p, &j, &best);
	else		loads_nodelay(p, &j, &best);
...
caliblng** runCache(char *array, caliblng maxrange, caliblng minstride, caliblng MHz, FILE *fp, caliblng *maxstride)
{
	...
	if (!(*maxstride)) {
		...
		if ((range*1.25) <= maxrange) range *= 1.25;
		do {
			stride *= 2;
			last = time;
		...
		#ifdef EPSILON1
		} while (((fabs(time - last) / (calibdbl)time) > EPSILON1) && (stride <= (maxrange / 2)));
		#endif
		#ifdef EPSILON3
...
caliblng** runTLB(char *array, caliblng maxrange, caliblng minstride, caliblng shift, caliblng mincachelines, caliblng MHz, FILE *fp, caliblng *maxstride)
{
		...
		do {
			...
			if (!time)
				ErrXit("runTLB: 'loads(%x(array), %ld(range), %ld(stride), %ld(MHz), 0(fp), 0(delay))` returned elapsed time of 0us",
					array, range, stride, MHz);
		#ifdef EPSILON1
		} while ((((fabs(time - last) / (calibdbl)time) > EPSILON1) || (stride < (pgsz / 1))) && (range <= (maxrange / 2)));	
	/*	} while (((fabs(time - last) / (calibdbl)time) > EPSILON1) && (range <= (maxrange / 2)));	*/
		#endif
		#ifdef EPSILON3
...
caliblng** runAsso(char *array, caliblng maxrange, caliblng minstride, caliblng shift, caliblng mincachelines, caliblng MHz, FILE *fp, caliblng *maxstride)
{
	caliblng	i, x, y, z, stride, minspots, maxspots, p;
	caliblng	range = maxrange, s = minstride / 2, spots = mincachelines / 2;
	calibdbl	f = 0.25;
...
-*/
	...
	if (!(*maxstride)) {
		do {
			...
			time = loads(array, range, stride, MHz, 0, 0);
			if (!time)
				ErrXit("runAsso: 'loads(%x(array), %ld(range), %ld(stride), %ld(MHz), 0(fp), 0(delay))` returned elapsed time of 0us",
					array, range, stride, MHz);
		#ifdef EPSILON1
		} while ((((fabs(time - last) / (calibdbl)time) > EPSILON1) || (stride < (pgsz / 1))) && (range <= (maxrange / 2)));	
	/*	} while (((fabs(time - last) / (calibdbl)time) > EPSILON1) && (range <= (maxrange / 2)));	*/
...
*/
				...
				for (s = *maxstride, x = 2; s >= minstride; s /= 2, x++) {
					...
					if (stride <= (maxrange/spots)) {
						...
						if (fp) {
							fprintf(fp, " %06ld %05.1f %05.1f"
								,result[ymax[x]][x]		/* elapsed time [microseconds] */
