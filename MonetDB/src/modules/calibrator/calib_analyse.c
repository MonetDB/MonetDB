...
cacheInfo* analyzeCache(caliblng **result1, caliblng **result2, caliblng MHz)
{
	...
	for (y = 1; y <= yy ; y++) {
		...
		for (x = 1; x <= xx; x++) {
			...
			if (draft[1].linesize[level] && last[a] && (range == draft[1].size[level])) {
				#ifdef EPSILON1
				if ((fabs(time1 - last[a]) / (calibdbl)time1) < EPSILON1) {
				#endif
				#ifdef EPSILON3
			...
			if (draft[2].linesize[level] && last[0] && lastrange && (lastrange == draft[2].size[level])) {
				#ifdef EPSILON1
				if ((fabs(time1 - last[0]) / (calibdbl)time1) < EPSILON1) {
				#endif
				#ifdef EPSILON3
			...
			#ifdef EPSILON2
			if ((x == 1) && (!draft[2].linesize[level]) && ((last[0] && ((FABS(time1 - last[LENPLATEAU - 1]) / (calibdbl)last[LENPLATEAU - 1]) > EPSILON2)) || (y == yy))) {
			#endif
			#ifdef EPSILON4
			...
			if ((x == 1) && (a < LENPLATEAU) && (!last[0])) {
				#ifdef EPSILON2
				if ((FABS(time1 - last[LENPLATEAU - 1]) / (calibdbl)last[LENPLATEAU - 1]) < EPSILON2) {
				#endif
				#ifdef EPSILON4
...
TLBinfo* analyzeTLB(caliblng **result1, caliblng **result2, caliblng shift, caliblng mincachelines, caliblng MHz)
{
...
*/
			if (draft[1].pagesize[level] && last[a] && (spots == draft[1].entries[level])) {
				#ifdef EPSILON1
				if (((fabs(time1 - last[a]) / (calibdbl)time1) < EPSILON1) || (stride >= result1[0][1])) {
				#endif
				#ifdef EPSILON3
			...
			if (draft[2].pagesize[level] && last[0] && lastspots && (lastspots == draft[2].entries[level])) {
				#ifdef EPSILON1
				if (((fabs(time1 - last[0]) / (calibdbl)time1) < EPSILON1) || (stride >= result1[0][1])) {
				#endif
				#ifdef EPSILON3
			...
			#ifdef EPSILON2
			if ((top) && (!draft[2].pagesize[level]) && ((last[0] && ((FABS(time1 - last[LENPLATEAU - 1]) / (calibdbl)last[LENPLATEAU - 1]) > EPSILON2)) || limit)) {
			#endif
			#ifdef EPSILON4
			if ((top) && (!draft[2].pagesize[level]) && ((last[0] && (FABS(CYperIt(time1) - CYperIt(last[LENPLATEAU - 1])) >= EPSILON4)) || limit)) {
			#endif
...
AssoInfo* analyzeAsso(caliblng **result1, caliblng **result2, caliblng shift, caliblng minTLBentries, caliblng maxLevels, caliblng MHz)
{
