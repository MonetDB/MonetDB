...
int allTrue(void)
{
    int i;
    for(i=0; i < condSP; i++)
	if (!condStack[i].defined) return 0;
    return 1;
}
    


Def *NwDef(CmdCode dir, int mod, int sec, int lino)
{
Def*	d;
 ...
 */
  ...
  */
	case Module:
		...
		d->d_cmd= cmd;
		d->d_blk= blk;
		if (!Hide()) IndexEntry(0, d->d_cmd,d->d_mod, d->d_sec);
		lastdir = Continue;
	    }
	    break;
	case Section:
