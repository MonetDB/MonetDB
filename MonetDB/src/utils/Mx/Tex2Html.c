...
void initltab(int lt, int p1, int p2, int p3, int p4, int p5, int p6)
{
	...
	if (p3) { ltab[lt][2]=p3; ltab[lt-'A'+'a'][2] = p3+32; }
	if (p4) { ltab[lt][3]=p4; ltab[lt-'A'+'a'][3] = p4+32; }
	if (p5) { ltab[lt][4]=p5; ltab[lt-'A'+'a'][4] = p5+32; }
	if (p6) { ltab[lt][5]=p6; ltab[lt-'A'+'a'][5] = p6+32; }
}
...
#define output(s,p) { sprintf(out,s,p); out+=strlen(out); }
	...
	*bibtexfile = 0;
	...
	for(i=0; i<nrefs; i++) {
		...
		if ((p=bib_entry("month", hit)) != 0) {
			...
			else if (!strcmp(p,"may")) outputNArg(", May")
			else if (!strcmp(p,"jun")) outputNArg(", June")
			else if (!strcmp(p,"jul")) outputNArg(", July")
			else if (!strcmp(p,"aug")) outputNArg(", August")
			else if (!strcmp(p,"sep")) outputNArg(", September")
			else if (!strcmp(p,"oct")) outputNArg(", October")
...
int translate_text(char *p1, char *p2, char **stack, char *nextstack)
{
	...
	if (table && (printed_column <= table_column)) { 
		...
		ofile_printf("<td align=%s colspan=%d><font size=\"-1\">",
			p5, width);
		printed_column++;
	}
	...
	if (*p1 != '\\') {
		switch(*p1) {
		case '~': ofile_puts("&#160"); break;
		case '$': if (math) {
mathoff:			math=0; ofile_puts("</i>");
			  } else {
mathon:				math=1; ofile_puts("<i>");
			  } break;
		case '&': if (table) {
			  	/* tab code in a table */
	...
	case '/':  p1++; continue;
	case 'c':  if (p1[1]) break; /* require block: disallow e.g. '\cite' */
		   k++;
	case '`':  k++;
	case '\'': k++;
	case '^':  k++;
	...
	for(i=TYPE_UNKNOWN; i >= TYPE_HLINE; i--) 
		if ((strncmp(p1, keyword[i],len)==0) && 
                    (len == strlen(keyword[i]))) break;
	if (p1[len]=='*') len++; /* 'XXX*' equals to 'XXX' */ 
	p = p1; p1 += len;
	...
	if (i < 0) {
		...
		if (table && (i == TYPE_HLINE)) {
			if (printed_column > 1) {
				...
				if (table > printed_column) {
					ofile_printf(
		"<td colspan=%d><font size=\"+1\"> </font></td>", 
						table-printed_column);
				}
				ofile_printf( "</tr>\n<tr>");
				printed_column=0; 	
			}
			...
			while(opt_start < opt_end) *opt_start++ = 0;
			ofile_puts(stackptr);
			ofile_puts("</b>\n<dd>");
		} else {
		...
		if (!*p1) { 
			...
			if (i <= TYPE_THANKS) {
			    if (i == TYPE_THANKS) ofile_puts("<br>[");
			    else ofile_puts("[");
		...
		*p3 = 0; if (i>=0) ofile_putc(',');
		...
		if (i == TYPE_VERBATIM) {
			...
			break;
		} else if (i == TYPE_DESC) indesclist = 1;
	} else if (i < TYPE_UNKNOWN) {
		/* kill the two renewcommand arguments */
		char *s = p1;
		while(s < latexend && *s) s++;
		s++; /* skip { */
		while(s < latexend && *s) *s++ = 0;
		s++; /* skip } */
		s++; /* skip { */
		while(s < latexend && *s) *s++ = 0;
	} else {
