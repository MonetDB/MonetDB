...
Ientry *ie;
	...
	if WWWMODE { 
		...
		for(i=0, ie = it->it_entrys; i<it->it_nentry; i++, ie++) { 
			if (ie->ie_sec == 0) { 
				...
				PrNum(ie->ie_mod); PrCmd("_0_0\">"); 
				PrCmd(ie->ie_name); 
				PrCmd("</a></h3>\n<ul><li>\n");
			} else {
				PrCmd("  [<a href=\"#mod_");
				PrNum(ie->ie_mod); PrCmd("_");
				PrNum(ie->ie_sec); PrCmd("_0\">"); 
				PrCmd(ie->ie_name); PrCmd("</a>]\n");
			}
		}
		if (i) PrCmd("</ul>\n");
	...
	for( ie= it->it_entrys; ie < it->it_entrys + it->it_nentry; ie++ ){
		if( ie->ie_sec == 0 ) {
			if TEXMODE PrCmd("\\makebox[3 em][l]{\\bf ");
			PrNum(ie->ie_mod);
			if TEXMODE PrCmd("}{\\bf ");
			else	PrCmd("\t");
			PrText(ie->ie_name);
			...
			if TEXMODE PrCmd("\\hspace*{2 em}\\ ");
			if TEXMODE PrCmd("\\makebox[2 em][l]{");
			else PrCmd("\t");
			PrNum(ie->ie_mod);
			PrChr('.');
			PrNum(ie->ie_sec);
	...
	if TEXMODE PrCmd("\\end{flushleft}");
	else	PrCmd(".XE\n.PX\n");
