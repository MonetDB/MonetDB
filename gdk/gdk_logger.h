...
gdk_export gdk_return log_tflush(logger *lg, ulng log_file_id, ulng commit_ts, unsigned int* commit_queue_number); /* Flush the WAL to disk using group commit */
gdk_export gdk_return log_tcommit(logger *lg, ulng commit_ts, unsigned int commit_queue_number);
