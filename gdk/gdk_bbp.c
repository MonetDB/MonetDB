...
BBPcheckbats(unsigned bbpversion)
{
	...
	for (bat bid = 1, size = (bat) ATOMIC_GET(&BBPsize); bid < size; bid++) {
		...
		if (b->theap->free > 0) {
			if (GDKfilepath(path, sizeof(path), 0, BATDIR, b->theap->filename, NULL) != GDK_SUCCEED)
				return GDK_FAIL;
		...
		if (b->tvheap != NULL && b->tvheap->free > 0) {
			if (GDKfilepath(path, sizeof(path), 0, BATDIR, BBP_physical(b->batCacheid), "theap") != GDK_SUCCEED)
				return GDK_FAIL;
			if (MT_stat(path, &statb) < 0) {
	...
	for (i = 0; i < MAXFARMS; i++) {
		if (BBPfarms[i].roles == 0) {
			...
			if ((rolemask & 1) == 0 && dirname != NULL) {
				 ...
				 * database */
				if (GDKfilepath(bbpdir, sizeof(bbpdir), i, BATDIR, "BBP", "dir") != GDK_SUCCEED) {
					return GDK_FAIL;
				}
				...
				if (GDKfilepath(bbpdir, sizeof(bbpdir), i, BAKDIR, "BBP", "dir") != GDK_SUCCEED) {
					return GDK_FAIL;
				}
				if (MT_stat(bbpdir, &st) != -1 || errno != ENOENT) {
					if (logerror)
...
BBPinit(bool allow_hge_upgrade, bool no_manager)
{
	...
	if (!GDKinmemory(0)) {
		char bbpdirstr[MAXPATH], backupbbpdirstr[MAXPATH];
		...
		if (MT_stat(backupbbpdirstr, &st) == 0) {
			...
			if (MT_stat(backupbbpdirstr, &st) < 0) {
				...
				TRC_DEBUG(IO, "reverting to dir saved in BBP.bak.\n");
...
#ifdef GDKLIBRARY_JSON
	if (bbpversion <= GDKLIBRARY_JSON && !GDKinmemory(0)) {
		...
		/* create signal file that we need to upgrade stored
		 * json strings. This will be performed by an upgrade
		 * function in the GDK that will be called at the end of
		 * the json module initialization with a callback that
		 * actually knows how to perform the upgrade. */
	...
	for (i = 0; i < MAXFARMS && BBPfarms[i].dirname != NULL; i++) {
		...
		if (j == i) {
			...
			if (GDKfilepath(d, sizeof(d), i, NULL, BATDIR, NULL) != GDK_SUCCEED) {
				return GDK_FAIL;
			}
	...
	if (BBP_status(bid) & BBPSAVING) {
		...
		if (lock) {
			while (BBP_status(bid) & BBPSAVING)
				MT_cond_wait(&GDKswapCond(bid), &GDKswapLock(bid));
			MT_lock_unset(&GDKswapLock(bid));
		} else {
...
file_move(int farmid, const char *srcdir, const char *dstdir, const char *name, const char *ext)
{
	if (GDKmove(farmid, srcdir, name, ext, dstdir, name, ext, false) == GDK_SUCCEED) {
		...
		if (GDKfilepath(path, sizeof(path), farmid, srcdir, name, ext) != GDK_SUCCEED)
			return GDK_FAIL;
		if (MT_stat(path, &st)) {
...
file_exists(int farmid, const char *dir, const char *name, const char *ext)
{
	...
	if (GDKfilepath(path, sizeof(path), farmid, dir, name, ext) == GDK_SUCCEED) {
		ret = MT_stat(path, &st);
...
BBPprepare(bool subcommit)
{
	...
	if (backup_files == 0) {
		...
		if (ret != GDK_SUCCEED)
			return ret;
		char bakdirpath[MAXPATH];

		if (GDKfilepath(bakdirpath, sizeof(bakdirpath), 0, NULL, BAKDIR, NULL) != GDK_SUCCEED) {
			return GDK_FAIL;
		}
	...
	if (start_subcommit) {
		...
		char subdirpath[MAXPATH];

		if (GDKfilepath(subdirpath, sizeof(subdirpath), 0, NULL, SUBDIR, NULL) != GDK_SUCCEED) {
			return GDK_FAIL;
		}
...
do_backup(Heap *h, bool dirty, bool subcommit)
{
	...
	if (h->storage != STORE_MMAP) {
		...
		if (GDKfilepath(srcdir, sizeof(srcdir), NOFARM, BATDIR, h->filename, NULL) != GDK_SUCCEED)
			return GDK_FAIL;
...
BBPcheckHeap(Heap *h)
{
	...
	if (s)
		...
		s = h->filename;
	if (GDKfilepath(path, sizeof(path), 0, BAKDIR, s, NULL) != GDK_SUCCEED)
		return;
	if (MT_stat(path, &statb) < 0) {
		if (GDKfilepath(path, sizeof(path), 0, BATDIR, h->filename, NULL) != GDK_SUCCEED)
			return;
		if (MT_stat(path, &statb) < 0) {
			GDKsyserror("cannot stat file %s (expected size %zu)\n",
				    path, h->free);
...
BBPsync(int cnt, const bat *restrict subcommit, const BUN *restrict sizes, lng logno)
{
	gdk_return ret = GDK_SUCCEED;
	lng t0 = 0, t1 = 0;
	char bakdir[MAXPATH], deldir[MAXPATH];
	...
	for (int idx = 1; ret == GDK_SUCCEED && idx < cnt; idx++) {
		...
		if (BBP_status(i) & BBPPERSISTENT) {
			...
			if (ret == GDK_SUCCEED && b && size != 0) {
				...
				if (lock) {
					MT_lock_set(&GDKswapLock(i));
					while (BBP_status(i) & (BBPSAVING|BBPUNLOADING))
						MT_cond_wait(&GDKswapCond(i), &GDKswapLock(i));
				} else {
...
{
	const char *p;
	char dstpath[MAXPATH], path2[MAXPATH];
	gdk_return ret = GDK_SUCCEED;
	...
	if ((p = strrchr(name, '.')) != NULL && strcmp(p, ".kill") == 0) {
		...
		if (GDKfilepath(path2, sizeof(path2), farmid, srcdir, name, NULL) != GDK_SUCCEED) {
			return GDK_FAIL;
		}
		if (MT_remove(path2) != 0) {
			ret = GDK_FAIL;
			GDKsyserror("force_move: remove(%s)\n", path2);
		}
		return ret;
	...
	if (ret != GDK_SUCCEED) {
		 ...
		 * doesn't exist */
		if (GDKfilepath(dstpath, sizeof(dstpath), farmid, dstdir, name, NULL) != GDK_SUCCEED ||
		    GDKfilepath(path2, sizeof(path2), farmid, srcdir, name, NULL) != GDK_SUCCEED) {
			return GDK_FAIL;
		}
...
BBPrecover(int farmid)
{
	...
	if (GDKfilepath(bakdirpath, sizeof(bakdirpath), farmid, NULL, BAKDIR, NULL) != GDK_SUCCEED ||
	    GDKfilepath(leftdirpath, sizeof(leftdirpath), farmid, NULL, LEFTDIR, NULL) != GDK_SUCCEED) {
		return GDK_FAIL;
	}
	...
	while ((dent = readdir(dirp)) != NULL) {
		...
		if (q == dent->d_name) {
			if (strcmp(dent->d_name, ".") == 0 ||
				...
				continue;
			if (GDKfilepath(fn, sizeof(fn), farmid, BAKDIR, dent->d_name, NULL) == GDK_SUCCEED) {
	...
	if (dirseen && ret == GDK_SUCCEED) {	/* we have a saved BBP.dir; it should be moved back!! */
		...
		if (GDKfilepath(fn, sizeof(fn), farmid, BATDIR, "BBP", "dir") != GDK_SUCCEED) {
			ret = GDK_FAIL;
		} else {
...
BBPrecover_subdir(void)
{
	...
	if (ret == GDK_SUCCEED) {
		...
		if (backup_dir == 2) {
			TRC_DEBUG(IO, "%s%cBBP.dir had disappeared!\n", SUBDIR, DIR_SEP);
			backup_dir = 0;
		}
	}
	TRC_DEBUG(IO, "end = %d\n", (int) ret);
...
BBPtmlockFinish(void)
{
	if (!GDKinmemory(0) &&
	     ...
	     * external programs */
	    GDKfilepath(lockfile, sizeof(lockfile), 0, NULL, ".tm_lock", NULL) == GDK_SUCCEED) {
...
BBPprintinfo(FILE *outf)
{
	...
	if (!BBPtrytmlock(1000)) {
		fprintf(outf,
			"BBP is currently locked, so no BAT information\n");
		return;
	}
	...
	if (bats[1][1][1][1][1].nr > 0)
		fprintf(outf,
			"fix, dirty, persistent, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][1][1][1].nr,
			bats[1][1][1][1][1].vmsz,
	...
	if (bats[1][1][1][1][0].nr > 0)
		fprintf(outf,
			"fix, dirty, persistent, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][1][1][0].nr,
			bats[1][1][1][1][0].vmsz,
	...
	if (bats[1][1][1][0][1].nr > 0)
		fprintf(outf,
			"fix, dirty, persistent, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][1][0][1].nr,
			bats[1][1][1][0][1].vmsz,
	...
	if (bats[1][1][1][0][0].nr > 0)
		fprintf(outf,
			"fix, dirty, persistent, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][1][0][0].nr,
			bats[1][1][1][0][0].vmsz,
	...
	if (bats[1][1][0][1][1].nr > 0)
		fprintf(outf,
			"fix, dirty, transient, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][0][1][1].nr,
			bats[1][1][0][1][1].vmsz,
	...
	if (bats[1][1][0][1][0].nr > 0)
		fprintf(outf,
			"fix, dirty, transient, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][0][1][0].nr,
			bats[1][1][0][1][0].vmsz,
	...
	if (bats[1][1][0][0][1].nr > 0)
		fprintf(outf,
			"fix, dirty, transient, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][0][0][1].nr,
			bats[1][1][0][0][1].vmsz,
	...
	if (bats[1][1][0][0][0].nr > 0)
		fprintf(outf,
			"fix, dirty, transient, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][1][0][0][0].nr,
			bats[1][1][0][0][0].vmsz,
	...
	if (bats[1][0][1][1][1].nr > 0)
		fprintf(outf,
			"fix, clean, persistent, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][1][1][1].nr,
			bats[1][0][1][1][1].vmsz,
	...
	if (bats[1][0][1][1][0].nr > 0)
		fprintf(outf,
			"fix, clean, persistent, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][1][1][0].nr,
			bats[1][0][1][1][0].vmsz,
	...
	if (bats[1][0][1][0][1].nr > 0)
		fprintf(outf,
			"fix, clean, persistent, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][1][0][1].nr,
			bats[1][0][1][0][1].vmsz,
	...
	if (bats[1][0][1][0][0].nr > 0)
		fprintf(outf,
			"fix, clean, persistent, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][1][0][0].nr,
			bats[1][0][1][0][0].vmsz,
	...
	if (bats[1][0][0][1][1].nr > 0)
		fprintf(outf,
			"fix, clean, transient, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][0][1][1].nr,
			bats[1][0][0][1][1].vmsz,
	...
	if (bats[1][0][0][1][0].nr > 0)
		fprintf(outf,
			"fix, clean, transient, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][0][1][0].nr,
			bats[1][0][0][1][0].vmsz,
	...
	if (bats[1][0][0][0][1].nr > 0)
		fprintf(outf,
			"fix, clean, transient, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][0][0][1].nr,
			bats[1][0][0][0][1].vmsz,
	...
	if (bats[1][0][0][0][0].nr > 0)
		fprintf(outf,
			"fix, clean, transient, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[1][0][0][0][0].nr,
			bats[1][0][0][0][0].vmsz,
	...
	if (bats[0][1][1][1][1].nr > 0)
		fprintf(outf,
			"no fix, dirty, persistent, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][1][1][1].nr,
			bats[0][1][1][1][1].vmsz,
	...
	if (bats[0][1][1][1][0].nr > 0)
		fprintf(outf,
			"no fix, dirty, persistent, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][1][1][0].nr,
			bats[0][1][1][1][0].vmsz,
	...
	if (bats[0][1][1][0][1].nr > 0)
		fprintf(outf,
			"no fix, dirty, persistent, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][1][0][1].nr,
			bats[0][1][1][0][1].vmsz,
	...
	if (bats[0][1][1][0][0].nr > 0)
		fprintf(outf,
			"no fix, dirty, persistent, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][1][0][0].nr,
			bats[0][1][1][0][0].vmsz,
	...
	if (bats[0][1][0][1][1].nr > 0)
		fprintf(outf,
			"no fix, dirty, transient, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][0][1][1].nr,
			bats[0][1][0][1][1].vmsz,
	...
	if (bats[0][1][0][1][0].nr > 0)
		fprintf(outf,
			"no fix, dirty, transient, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][0][1][0].nr,
			bats[0][1][0][1][0].vmsz,
	...
	if (bats[0][1][0][0][1].nr > 0)
		fprintf(outf,
			"no fix, dirty, transient, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][0][0][1].nr,
			bats[0][1][0][0][1].vmsz,
	...
	if (bats[0][1][0][0][0].nr > 0)
		fprintf(outf,
			"no fix, dirty, transient, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][1][0][0][0].nr,
			bats[0][1][0][0][0].vmsz,
	...
	if (bats[0][0][1][1][1].nr > 0)
		fprintf(outf,
			"no fix, clean, persistent, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][1][1][1].nr,
			bats[0][0][1][1][1].vmsz,
	...
	if (bats[0][0][1][1][0].nr > 0)
		fprintf(outf,
			"no fix, clean, persistent, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][1][1][0].nr,
			bats[0][0][1][1][0].vmsz,
	...
	if (bats[0][0][1][0][1].nr > 0)
		fprintf(outf,
			"no fix, clean, persistent, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][1][0][1].nr,
			bats[0][0][1][0][1].vmsz,
	...
	if (bats[0][0][1][0][0].nr > 0)
		fprintf(outf,
			"no fix, clean, persistent, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][1][0][0].nr,
			bats[0][0][1][0][0].vmsz,
	...
	if (bats[0][0][0][1][1].nr > 0)
		fprintf(outf,
			"no fix, clean, transient, loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][0][1][1].nr,
			bats[0][0][0][1][1].vmsz,
	...
	if (bats[0][0][0][1][0].nr > 0)
		fprintf(outf,
			"no fix, clean, transient, loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][0][1][0].nr,
			bats[0][0][0][1][0].vmsz,
	...
	if (bats[0][0][0][0][1].nr > 0)
		fprintf(outf,
			"no fix, clean, transient, not loaded, hot: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][0][0][1].nr,
			bats[0][0][0][0][1].vmsz,
	...
	if (bats[0][0][0][0][0].nr > 0)
		fprintf(outf,
			"no fix, clean, transient, not loaded, cold: %d bats, %zu%s virtual, %zu%s malloc\n",
			bats[0][0][0][0][0].nr,
			bats[0][0][0][0][0].vmsz,
	...
	fprintf(outf,
		"%d bats total, %d in use, %" PRIu32
		" free bats in common shared list\n",
		sz - 1, nbats, nfree);
	if (nskip > 0)
		fprintf(outf,
			"%d bat slots unaccounted for because of locking\n",
			nskip);
}
