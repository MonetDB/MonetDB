			...
			if (tpe == TYPE_msk) {
				if (r) {
					...
					for (; res == LOG_OK && snr > 0; snr-=cnt) {
						...
						if (!mnstr_readIntArray(lg->input_log, rbuf, cnt))
							res = LOG_ERR;
					}
				}
			} else {
				if (!ATOMvarsized(tpe)) {
					size_t cnt = 0, snr = (size_t)nr;
					size_t tlen = rbufsize/ATOMsize(tpe), ntlen = rbufsize;
					assert(tlen);
					/* read in chunks of max
					 * BUFSIZE/width rows */
					for (; res == LOG_OK && snr > 0; snr-=cnt) {
						cnt = snr>tlen?tlen:snr;
						void *t = rt(rbuf, &ntlen, lg->input_log, cnt);
...
{
	gdk_return res = GDK_SUCCEED;
	rotation_lock(lg); /*protects against potential concurrent write in log_tflush*/
	if (lg->current->drops > 100000 && ATOMIC_GET(&lg->current->end) > 0 && lg->saved_id+1 == lg->id) {
		lg->id++;
		log_close_output(lg);
		/* start new file */
...
check_rotation_conditions(logger *lg) {
	...
	return
		...
		(p > log_large || (((lng) ATOMIC_GET(&lg->current->end) * 1024) > log_large));
}
...
log_tflush(logger* lg, ulng log_file_id, ulng commit_ts) {
	...
	if (lg->flushnow) {
		...
		assert(ATOMIC_GET(&lg->current->end) ==ATOMIC_GET(&lg->current->pend));
		lg->flushnow = 0;
		if (log_commit(lg) == GDK_SUCCEED)
