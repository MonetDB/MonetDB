...
/* Do a binary search for the first/last occurrence of v between lo
 * and hi (lo inclusive, hi not inclusive) in rvals/rvars, using rcand
 * as a candidate list.
...
binsearch(const oid *rcand, oid offset,
	  int type, const void *rvals, const char *rvars,
	  int rwidth, BUN lo, BUN hi, const void *v,
			...
			if (v == NULL) {
				...
				if (lscan < (BUN) (lcandend - lcand)) {
					if (lvals) {
						if (lordering * cmp(VALUE(l, lcand[lscan] - l->hseqbase), v) < 0) {
							nlx = binsearch(lcand, l->hseqbase, l->ttype, lvals, lvars, lwidth, lscan, (BUN) (lcandend - lcand), v, cmp, lordering, 0);
						}
					} else {
						lval = (oid) ((lng) *(const oid*)v - loff);
						if (lordering > 0 ? lcand[lscan] < lval : lcand[lscan] > lval) {
							nlx = binsearch(NULL, 0, TYPE_oid, (const char *) lcand, NULL, SIZEOF_OID, 0, lcandend - lcand, &lval, cmp, 1, 0);
						}
					}
					lcand += nlx;
					if (lcand == lcandend)
				...
				if (lscan + lstart < lend) {
					if (lordering * cmp(VALUE(l, lstart + lscan),
							    v) < 0) {
						nlx = lstart;
						lstart = binsearch(NULL, 0, l->ttype, lvals, lvars, lwidth, lstart + lscan, lend, v, cmp, lordering, 0);
						nlx = lstart - nlx;
						if (lstart == lend)
							v = NULL;
					}
			...
			if (rcand) {
				...
				if (rvals) {
					if (rscan < (BUN) (rcandend - rcand) &&
						 ...
						 * in r: use binary
						 * search */
						rcand += binsearch(rcand, r->hseqbase, r->ttype, rvals, rvars, rwidth, rscan, (BUN) (rcandend - rcand), v, cmp, rordering, 0);
					} else {
						/* scan r for v */
					...
					if (rcand < rcandend &&
						...
						if (r->tkey) {
							 ...
							 * find the end */
							nr = binsearch(rcand, r->hseqbase, r->ttype, rvals, rvars, rwidth, rscan, (BUN) (rcandend - rcand), v, cmp, rordering, 1);
							rcand += nr;
						} else {
							/* scan r for end of
							 * range */
					...
					if (rscan < (BUN) (rcandend - rcand) &&
						...
						rcand += binsearch(NULL, 0, TYPE_oid, (const char *) rcand, NULL, SIZEOF_OID, rscan, rcandend - rcand, &rval, cmp, 1, 0);
					} else {
						while (rcand < rcandend &&
				...
				if (rstart + rscan < rend &&
					 ...
					 * in r: use binary
					 * search */
					rstart = binsearch(NULL, 0, r->ttype, rvals, rvars, rwidth, rstart + rscan, rend, v, cmp, rordering, 0);
				} else {
					/* scan r for v */
					while (rstart < rend &&
				...
				if (rstart < rend &&
					...
					if (r->tkey) {
						...
						nr = binsearch(NULL, 0, r->ttype, rvals, rvars, rwidth, rstart + rscan, rend, v, cmp, rordering, 1);
						nr -= rstart;
						rstart += nr;
					} else {
						/* scan r for end of range */
			...
			if (rcand) {
				...
				if (rvals) {
					if (rscan < (BUN) (rcandend - rcand) &&
						 ...
						 * in r: use binary
						 * search */
						rcandend = rcand + binsearch(rcand, r->hseqbase, r->ttype, rvals, rvars, rwidth, 0, (BUN) (rcandend - rcand) - rscan, v, cmp, rordering, 1);
					} else {
						/* scan r for v */
						while (rcand < rcandend &&
					...
					if (rcand < rcandend &&
						...
						if (r->tkey) {
							...
							nr = binsearch(rcand, r->hseqbase, r->ttype, rvals, rvars, rwidth, 0, (BUN) (rcandend - rcand) - rscan, v, cmp, rordering, 0);
					...
					if (rscan < (BUN) (rcandend - rcand) &&
						...
						rcandend = rcand + binsearch(NULL, 0, TYPE_oid, (const char *) rcand, NULL, SIZEOF_OID, 0, rcandend - rcand - rscan, &rval, cmp, 1, 1);
					} else {
						while (rcand < rcandend &&
				...
				if (rstart + rscan < rend &&
					 ...
					 * in r: use binary
					 * search */
					rend = binsearch(NULL, 0, r->ttype, rvals, rvars, rwidth, rstart, rend - rscan, v, cmp, rordering, 1);
				} else {
					/* scan r for v */
					while (rstart < rend &&
