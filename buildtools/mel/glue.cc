...
gen_default_atomops(ostream &o, int op, const Atom &d)
{
	...
	case OP_TOSTR:
		o << "static int\n";
		o << atom << "_tostr(char **dst, int *len, char *src)\n{\n";
		o << "   int i = 0;\n";
		o << "   unsigned char *buf = 0;\n\n";
		o << "   if (*len < " << len << " * 4 + 3) {\n";
		o << "      if (*dst && *len > 0)\n";
		o << "         GDKfree(*dst);\n";
		o << "      *dst = (char *) GDKmalloc(*len = " << len << " * 4 + 3);\n";
		o << "   }\n";
		o << "   buf = (unsigned char *) *dst;\n";
		o << "   *buf++ = '\"';\n";
		o << "   for (i = 0; i < " << len << "; i++) {\n";
		o << "      unsigned char c = (unsigned char) *src++;\n\n";
		o << "      if (GDKisprint(c) && c != '\"' && c != '\\\\')\n";
		o << "         *buf++ = c;\n";
		o << "      else {\n";
		o << "         *buf++ = '\\\\';\n";
		o << "         *buf++ = '0' + c / 64;\n";
		...
		o << "   *buf++ = '\"';\n";
		o << "   *buf = 0;\n";
		o << "   return (int) (buf - (unsigned char *) *dst);\n";
		o << "}\n\n";
		break;
	case OP_FROMSTR:
		o << "static int\n";
		o << atom << "_fromstr(char *src, int *len, char **dst)\n{\n";
		o << "   int i = 0;\n";
		o << "   unsigned char *buf = 0;\n\n";
		o << "   if (*len < " << len << ") {\n";
		o << "      if (*dst && *len > 0)\n";
		...
		o << "   }\n";
		o << "   for (i = 0, buf = (unsigned char *) src; i < " << len << "; i++,buf++) {\n";
		o << "      if (*buf == '\\\\') {\n";
		o << "         if (buf[1] < '0' || buf[1] > '3' ||\n";
		o << "             buf[2] < '0' || buf[2] > '7' ||\n";
		o << "             buf[3] < '0' || buf[3] > '7')\n";
		o << "            continue;\n";
		o << "         (*dst)[i] = buf[1] * 64 + buf[2] * 8 + buf[3];\n";
		o << "         buf += 4;\n";
		o << "      } else {\n";
		o << "         (*dst)[i] = *buf;\n";
		o << "      }\n";
		o << "   }\n";
		o << "   return (int) (buf - (unsigned char *) src);\n";
		o << "}\n\n";
		break;
	...
	case OP_HASH:
		...
		break;
	case OP_NULL:
		o << "static char " << atom <<"_nil[" << len << "] = {";
		for (int i = d.size(); i > 0; i--)
			o << "'\\201',";
		o << "};\n";
		o << "static ptr " << atom << "_null(void){\n";
