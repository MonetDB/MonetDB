...
mapi_reconnect(Mapi mid)
{
  ...
  try_again_after_redirect:
	...
	check_stream(mid, mid->from, "Connection terminated", "mapi_reconnect", (mid->blk.eos = 1, mid->error));
	...
	server = strchr(chal, ':');
	if (server == NULL) {
		mapi_setError(mid, "Challenge string is not valid", "mapi_reconnect", MERROR);
		close_connection(mid);
		return mid->error;
	}
	*server++ = '\0';
