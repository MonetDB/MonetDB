...
/*	Driver specific Setup APIs called by installer */
	EXPORT BOOL ConfigDSN(HWND hwndParent, WORD fRequest, LPCSTR lpszDriver, LPCSTR lpszAttributes);

	EXPORT BOOL ConfigTranslator(HWND hwndParent, DWORD * pvOption);

#if (ODBCVER >= 0x0250)
	EXPORT BOOL ConfigDriver(HWND hwndParent, WORD fRequest, LPCSTR lpszDriver, LPCSTR lpszArgs, LPSTR lpszMsg, WORD cbMsgMax, WORD * pcbMsgOut);
#endif
