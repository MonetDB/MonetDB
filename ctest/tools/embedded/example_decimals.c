...
main(void)
{
	...
	if ((err = monetdb_connect(&conn)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "CREATE TABLE test (b bigint, d1 decimal(15,3), m decimal(4,2), y string)", NULL, NULL, NULL)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "INSERT INTO test VALUES (42, 200042.024, 42.42, 'Hello'), (NULL, NULL, NULL, 'World')", NULL, NULL, NULL)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "SELECT b, d1, m, y FROM test; ", &result, NULL, NULL)) != NULL)
		error(err)
