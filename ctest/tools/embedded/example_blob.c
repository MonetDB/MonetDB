...
main(void)
{
	...
	if ((err = monetdb_connect(&conn)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "CREATE TABLE test (name string, data blob)", NULL, NULL)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "INSERT INTO test VALUES ('Hello', '01020308')", NULL, NULL)) != NULL)
		error(err)
	if ((err = monetdb_query(conn, "SELECT name, data FROM test; ", &result, NULL)) != NULL)
		error(err)
