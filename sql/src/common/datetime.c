...
int
parse_interval_intern(context * sql,
	...
	if (ek != sk) {
		if (*n != sep) {
			snprintf(sql->errstr, ERRSIZE,
				 _
				 ("Interval field seperator \'%c\' missing\n"),
				 sep);
			return -1;
		}
		return parse_interval_intern(sql, sign, n + 1, sk + 1, ek,
					     i);
	} else {
		return type;
	}
	       ...
	       int sign, char *str, struct dlist *pers, int *val)
{
	...
	*val = 0;
	if (sk < ek) {
		snprintf(sql->errstr, ERRSIZE,
			 _
			 ("End interval field is larger than the start field\n"));
		return -1;
	}
	if ((sk == iyear || sk == imonth) && ek > imonth) {
		snprintf(sql->errstr, ERRSIZE,
			 _
			 ("Correct interval ranges are year-month or day-seconds\n"));
		return -1;
	}
