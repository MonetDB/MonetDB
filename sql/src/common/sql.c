...
statement *column_ref( context *sql, scope *scp, symbol *column_r ){
	...
	if (dlist_length(l) == 1){
		...
		if (!c){
			statement *sc = 
				scope_bind_statement( scp, NULL, name );
			if (sc) return sc;
		}
		if (!c){
		...
		if (!c){
			statement *sc = 
				scope_bind_statement( scp, tname, cname );
			if (sc) return sc;
		}
		if (tv){
			return statement_column(c, tv); 
...
var *subquery_optname( context *sql, scope *scp, symbol *query ){
	...
	var *res = NULL;
	statement *sq = subquery(sql, nscp, query );
...
statement *scalar_exp( context *sql, scope *scp, symbol *se, statement *group,
	...
	case SQL_SELECT: {
		scope *nscp = scope_open(scp);
		statement *sq = subquery( sql, nscp, se );
...
statement *column_exp( context *sql, scope *scp, symbol *column_e, statement *group, statement *subset ){
	...
	case SQL_COLUMN:
	{
		...
		if (s && l->h->next->data.sval){
			...
			scope_add_statement( scp, s, NULL, l->h->next->data.sval);
		} 
		return s;
	} break;
	/*
...
statement *search_condition( context *sql, scope *scp, symbol *sc, 
	...
	case SQL_IN: {
		...
		if (l->h->next->type == type_list){
			...
			scope *nscp = scope_open(scp);
			statement *sq =
			       	subquery(sql, nscp, l->h->next->data.sym);
	...
	case SQL_NOT_EXISTS: {
		...
		scope *nscp = scope_open(scp);
		statement *ls = subquery(sql, nscp, lo );
