...
rel2bin_topn( mvc *sql, sql_rel *rel, list *refs)
{
	...
	if (n) {
		...
		if (order) {
		 	limit = stmt_limit(stmt_dup(order), o, l, LIMIT_DIRECTION(0,0,need_including(rel)));
		} else {
			stmt *sc = stmt_dup(n->data);
			char *cname = column_name(sc);
			char *tname = table_name(sc);
			...
			sc = column(sc);
			limit = stmt_limit(stmt_alias(sc, tname, cname), o, l, LIMIT_DIRECTION(0,0,need_including(rel)));
		}
