...
call Mlog.bat -x java -jar "%JAR%" -h %HOST% -p %MAPIPORT% -b %TSTDB% -f "%RELSRCDIR%\..\JdbcClient_create_tables.sql"

call Mlog.bat -x java -jar "%JAR%" -h %HOST% -p %MAPIPORT% -b %TSTDB% -f "%RELSRCDIR%\..\JdbcClient_inserts_selects.sql"

call Mlog.bat -x java -jar "%JAR%" -h %HOST% -p %MAPIPORT% -b %TSTDB% -d
