...
\subsection{Mknife}
\label{mknife}
A flexible graphical user interface has been designed to interact with
the MonetDB server using any of its languages. The interaction is based
on the Mapi protocol.

Its working is described in a separate document, found on the
...
explain & 'plan' & Generate the MIL query execution plan\\
	& 'history' & keep a log of the statements executed in the history table\\
