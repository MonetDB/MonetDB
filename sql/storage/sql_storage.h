...
typedef int (*append_col2_fptr) (sql_trans *tr, sql_column *c, BUN offset, BAT *offsets, void *null, void *d, BUN cnt, bool isbat, int t);
...
extern res_col *res_col_create(sql_trans *tr, res_table *t, const char *tn, const char *name, const char *typename, int digits, int scale, bool isbat, char mtype, void *v, void *null, bool cache);
...
extern int sql_trans_change_schema_authorization(sql_trans *tr, sqlid id, sqlid auth_id);
...
extern sql_sequence *create_sql_sequence(struct sqlstore *store, allocator *sa, sql_schema *s, const char *name, lng start, lng min, lng max, lng inc, lng cacheinc, bit cycle);
...
extern sql_session * sql_session_create(struct sqlstore *store, allocator *sa, int autocommit);
...
extern int sql_trans_create_dependency(sql_trans *tr, sqlid id, sqlid depend_id, sql_dependency depend_type, temp_t temp);
...
extern sql_idx *create_sql_idx(struct sqlstore *store, allocator *sa, sql_table *t, const char *nme, idx_type it);
...
extern void store_printinfo(FILE *outf, sqlstore *store)
