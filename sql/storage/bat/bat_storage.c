...
static void
lock_table(sqlstore *store, sql_table *t)
{
	(void)store;
	MT_lock_set(&t->lock);
}
...
static void
unlock_table(sqlstore *store, sql_table *t)
{
	(void)store;
...
static void
lock_column(sqlstore *store, sql_column *c)
{
	(void)store;
	MT_lock_set(&c->lock);
}
...
static void
unlock_column(sqlstore *store, sql_column *c)
{
	(void)store;
...
static void
lock_idx(sqlstore *store, sql_idx *c)
{
	(void)store;
	MT_lock_set(&c->lock);
}
...
static void
unlock_idx(sqlstore *store, sql_idx *c)
{
	(void)store;
	MT_lock_unset(&c->lock);
}
...
count_col(sql_trans *tr, sql_column *c, int access)
{
	...
	if (access == CNT_ACTIVE) {
		...
		lock_table(tr->store, c->t);
		cnt -= count_deletes_in_range(d->segs->h, tr, 0, cnt);
		unlock_table(tr->store, c->t);
		return cnt;
	}
...
static sql_delta *
bind_col_data(sql_trans *tr, sql_column *c, bool *update_conflict, bool *new)
{
	lock_column(tr->store, c);
...
{
	int res = LOG_OK;
	bool update_conflict = false, new = false;
	sql_delta *delta;
	...
	if ((delta = bind_col_data(tr, c, &update_conflict, &new)) == NULL)
...
static sql_delta *
bind_idx_data(sql_trans *tr, sql_idx *i, bool *update_conflict, bool *new)
{
	lock_idx(tr->store, i);
...
update_idx(sql_trans *tr, sql_idx * i, void *tids, void *upd, bool isbat)
{
	...
	if ((delta = bind_idx_data(tr, i, &update_conflict, &new)) == NULL)
		return update_conflict ? LOG_CONFLICT : LOG_ERR;
...
static int
delta_append_bat(sql_trans *tr, sql_delta **batp, BUN offset, BAT *offsets, BAT *i, char *storage_type)
{
	BAT *b, *oi = i;
	int err = 0;
	sql_delta *bat = *batp;
...
static int
bat_append_val(sql_delta *bat, BUN offset, BAT *offsets, void *i, BUN cnt, bool val)
{
	void *oi = i;
...
static int
delta_append_val(sql_trans *tr, sql_delta **batp, BUN offset, BAT *offsets, void *null, void *i, BUN cnt, char *storage_type, int tt)
{
	sql_delta *bat = *batp;
	...
	if (!null || bat_append_val(bat, offset, offsets, null, cnt, false) == LOG_OK)
		return bat_append_val(bat, offset, offsets, i, cnt, true);
	return LOG_ERR;
}
...
static int
append_col_execute(sql_trans *tr, sql_delta **delta, sql_column *c, BUN offset, BAT *offsets, void *null, void *incoming_data, BUN cnt, bool isbat, int tt, char *storage_type)
{
	int ok = LOG_OK;
	...
	if (isbat) {
		...
		if (BATcount(bat))
			ok = delta_append_bat(tr, delta, offset, offsets, bat, storage_type);
	} else {
		ok = delta_append_val(tr, delta, offset, offsets, null, incoming_data, cnt, storage_type, tt);
	}
	unlock_column(tr->store, c);
	return ok;
}
...
static int
append_col2(sql_trans *tr, sql_column *c, BUN offset, BAT *offsets, void *null, void *data, BUN cnt, bool isbat, int tpe)
{
	int res = LOG_OK;
	sql_delta *delta;
	bit f = false;
	...
	if (!null && c->nullmask)
		null = &f;
	if ((res = append_col_execute(tr, &delta, c, offset, offsets, null, data, cnt, isbat, tpe, c->storage_type)) != LOG_OK)
		return res;
	if (delta->cs.st == ST_DEFAULT && c->storage_type)
		res = sql_trans_alter_storage(tr, c, NULL);
	return res;
...
{
	return append_col2(tr, c, offset, offsets, NULL, data, cnt, isbat, tpe);
}

static int
append_idx_execute(sql_trans *tr, sql_delta **delta, sql_idx *c, BUN offset, BAT *offsets, void *incoming_data, BUN cnt, bool isbat, int tt, char *storage_type)
{
	int ok = LOG_OK;
...
append_idx(sql_trans *tr, sql_idx *i, BUN offset, BAT *offsets, void *data, BUN cnt, bool isbat, int tpe)
{
	...
	if ((delta = bind_idx_data(tr, i, NULL, NULL)) == NULL)
		return LOG_ERR;

	assert(delta->cs.st == ST_DEFAULT);

	res = append_idx_execute(tr, &delta, i, offset, offsets, data, cnt, isbat, tpe, NULL);
	return res;
}
...
swap_bats(sql_trans *tr, sql_column *col, BAT *bn)
{
	...
	if ((d = bind_col_data(tr, col, &update_conflict, &new)) == NULL)
		return update_conflict ? LOG_CONFLICT : LOG_ERR;
...
static int
log_segments(sql_trans *tr, segments *segs, sql_table *t)
{
	size_t total = 0;
	/* log segments */
	lock_table(tr->store, t);
	...
	for (segment *seg = segs->h; seg; seg=ATOMIC_PTR_GET(&seg->next)) {
		...
		if (seg->ts == tr->tid && seg->end-seg->start &&
			(ATOMIC_PTR_GET(&seg->next) || !seg->deleted || seg->ts != seg->oldts)) {
			if (log_segment(tr, seg, t->base.id, total) != LOG_OK) {
				return LOG_ERR;
			}
		}
		lock_table(tr->store, t);
...
clear_col(sql_trans *tr, sql_column *c, bool renew)
{
	...
	if ((delta = bind_col_data(tr, c, renew?&update_conflict:NULL, &new)) == NULL)
...
clear_idx(sql_trans *tr, sql_idx *i, bool renew)
{
	...
	if (!isTable(i->t) || (hash_index(i->type) && list_length(i->columns) <= 1) || !idx_has_column(i->type))
		return 0;
	if ((delta = bind_idx_data(tr, i, renew?&update_conflict:NULL, &new)) == NULL)
		return update_conflict ? BUN_NONE - 1 : BUN_NONE;
...
static int
commit_update_delta( sql_trans *tr, sql_change *change, sql_table* t, sql_column *c, sql_idx *i, ATOMIC_PTR_TYPE* data, int type, ulng commit_ts, ulng oldest)
{
	...
	if (t->commit_action == CA_DELETE || t->commit_action == CA_DROP) {
		...
		if (!tr->parent) {
			if (c)
				t->base.new = c->base.new = 0;
			else
				t->base.new = i->base.new = 0;
		}
		change->handled = true;
		return ok;
	}
	...
	if (!commit_ts) { /* rollback */
		...
		if (delta && !delta->cs.merged && delta->cs.ts <= oldest) {
			if (c)
				lock_column(tr->store, c); /* lock for concurrent updates (appends) */
			else
				lock_idx(tr->store, i); /* lock for concurrent updates (appends) */
...
commit_update_col( sql_trans *tr, sql_change *change, ulng commit_ts, ulng oldest)
{
	...
	return commit_update_delta(tr, change, t, c, NULL, data, type, commit_ts, oldest);
}
...
commit_update_idx( sql_trans *tr, sql_change *change, ulng commit_ts, ulng oldest)
{
	...
	return commit_update_delta(tr, change, t, NULL, i, data, type, commit_ts, oldest);
}
...
tc_gc_col( sql_store Store, sql_change *change, ulng oldest)
{
	...
	if (d && d->next) {
		...
		lock_column(store, c); /* lock for concurrent updates (appends) */
		id->nr_updates += merge_delta(d);
		unlock_column(store, c);
	}
	column_destroy(store, c);
	return 1;
}
...
tc_gc_idx( sql_store Store, sql_change *change, ulng oldest)
{
	...
	if (d && d->next) {
		...
		lock_idx(store, i); /* lock for concurrent updates (appends) */
		id->nr_updates += merge_delta(d);
		unlock_idx(store, i);
	}
	idx_destroy(store, i);
...
vacuum_col(sql_trans *tr, sql_column *c, bool force)
{
	...
	if ((d = bind_col_data(tr, c, NULL, NULL)) == NULL)
		return LOG_CONFLICT;
