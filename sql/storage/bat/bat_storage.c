...
static int
delta_append_bat(sql_trans *tr, sql_delta *bat, sqlid id, BUN offset, BAT *offsets, BAT *i, char *storage_type)
{
	BAT *b, *oi = i;
	int err = 0;
...
static int
delta_append_val(sql_trans *tr, sql_delta *bat, sqlid id, BUN offset, void *i, BUN cnt, char *storage_type, int tt)
{
	void *oi = i;
	BAT *b;
	lock_column(tr->store, id);
	...
	if (bat->cs.st == ST_DICT) {
		/* possibly a new array is returned */
		i = dict_append_val(&bat->cs, i, cnt);
		if (!i) {
			unlock_column(tr->store, id);
			return LOG_ERR;
		}
...
append_idx(sql_trans *tr, sql_idx * i, BUN offset, BAT *offsets, void *data, BUN cnt, int tpe)
{
	...
	return append_col_execute(tr, delta, i->base.id, offset, offsets, data, cnt, tpe, NULL);
}
...
col_min_value(sql_trans *tr, sql_column *c, ValPtr res)
{
	...
	if (c->storage_type) {
		...
		if (d->cs.st == ST_DICT) {
			...
			BATiter bi = bat_iterator(b);
			if (bi.minpos != BUN_NONE && d->cs.ucnt == 0 && VALinit(res, bi.type, BUNtail(bi, bi.minpos)))
				ok = 1;
			bat_iterator_end(&bi);
			bat_destroy(b);
		}
	...
	if ((d = ATOMIC_PTR_GET(&c->data)) &&
		(b = quick_descriptor(d->cs.bid)) && (u = quick_descriptor(d->cs.uvbid)) &&
		b->tminpos != BUN_NONE && BATcount(u) == 0 &&
		(b = temp_descriptor(d->cs.bid))) {
		BATiter bi = bat_iterator(b);
...
col_max_value(sql_trans *tr, sql_column *c, ValPtr res)
{
	...
	if (c->storage_type) {
		...
		if (d->cs.st == ST_DICT) {
			b = temp_descriptor(d->cs.ebid);
			BATiter bi = bat_iterator(b);
			if (bi.maxpos != BUN_NONE && d->cs.ucnt == 0 && VALinit(res, bi.type, BUNtail(bi, bi.maxpos)))
				ok = 1;
			bat_iterator_end(&bi);
			bat_destroy(b);
		}
	...
	if ((d = ATOMIC_PTR_GET(&c->data)) &&
		(b = quick_descriptor(d->cs.bid)) && (u = quick_descriptor(d->cs.uvbid)) &&
		b->tmaxpos != BUN_NONE && BATcount(u) == 0 &&
		(b = temp_descriptor(d->cs.bid))) {
		BATiter bi = bat_iterator(b);
