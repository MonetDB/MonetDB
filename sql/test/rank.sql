...
select DENSE_RANK() over () as foo, id, k from ranktest order by k, foo;
select DENSE_RANK() over (PARTITION BY id) as foo, id, k from ranktest order by k, foo;
select DENSE_RANK() over (PARTITION BY id ORDER BY id, k) as foo, id, k from ranktest order by k, foo;
select DENSE_RANK() over (ORDER BY id, k) as foo, id, k from ranktest order by k, foo;
drop table ranktest;
