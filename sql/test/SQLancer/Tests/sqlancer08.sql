...
START TRANSACTION;
CREATE TABLE "sys"."t0" ("tc0" INTERVAL DAY NOT NULL,"tc1" INTEGER NOT NULL,"tc2" UUID,CONSTRAINT "t0_tc0_pkey" PRIMARY KEY ("tc0"),CONSTRAINT "t0_tc0_unique" UNIQUE ("tc0"));
COPY 5 RECORDS INTO "sys"."t0" FROM stdin USING DELIMITERS E'\t',E'\n','"';
106435138838400.000	1862176006	eaba775d-ffbe-ba1f-b0fa-aba4d8d76971
88717851417600.000	1862176006	eaba775d-ffbe-ba1f-b0fa-aba4d8d76971
178058421590400.000	1862176006	eaba775d-ffbe-ba1f-b0fa-aba4d8d76971
107739028454400.000	1862176006	eaba775d-ffbe-ba1f-b0fa-aba4d8d76971
4002829401600.000	1862176006	eaba775d-ffbe-ba1f-b0fa-aba4d8d76971

create view v0(vc0) as (values (0.6686823));
create view v2(vc0) as (values (((((select distinct abs(0.3834286535052111588584011769853532314300537109375)) intersect all 
(select all cast("isauuid"(r'A') as bigint) where ((interval '46659371' second)<=(interval '984003396' second)))))&(+ (ifthenelse((63) not in (78, 92, 83043306), + (1318708640), 1284487639)))))) with check option;
SELECT CAST(SUM(count) AS BIGINT) FROM (SELECT ALL CAST((CAST(((24829)+(((0.9767751031140547)*(0.7479400824095245)))) AS DOUBLE)) IS NULL AS INT) as count FROM v2, t0 CROSS JOIN v0) as res;
	-- 0
ROLLBACK;
