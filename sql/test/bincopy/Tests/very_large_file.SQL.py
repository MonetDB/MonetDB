...
def write_zeroes(count: int, f: typing.BinaryIO):
    block_size = 1024 * 1024
    block = block_size * b'\x00'
    while count > 0:
...
def test_large_upload(size: int, on_clause: str):
    ...
    with pymonetdb.connect(TSTDB, port=MAPIPORT) as conn, conn.cursor() as c:
        ...
        try:
            ...
            execute('CREATE TABLE foo(i INT)')
