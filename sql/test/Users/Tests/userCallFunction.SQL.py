...
try:
    ...
    if "SELECT: no such operator 's1'.'f1'()" not in str(e):
        sys.stderr.write('Wrong error %s, expected SELECT: no such operator \'s1\'.\'f1\'()' % (str(e)))
try:
    cur1.execute('SELECT s1.f1(1);') # error, not allowed
    sys.stderr.write("Exception expected")
except pymonetdb.DatabaseError as e:
    if "SELECT: no such unary operator 's1'.'f1'(tinyint)" not in str(e):
        sys.stderr.write('Wrong error %s, expected SELECT: no such unary operator \'s1\'.\'f1\'(tinyint)' % (str(e)))
try:
    cur1.execute('CALL sys.flush_log();') # error, not allowed
    sys.stderr.write("Exception expected")
except pymonetdb.DatabaseError as e:
    if "SELECT: no such operator 'sys'.'flush_log'" not in str(e):
        sys.stderr.write('Wrong error %s, expected SELECT: no such operator \'sys\'.\'flush_log\'' % (str(e)))
cur1.close()
conn1.close()
