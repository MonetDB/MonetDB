...
server_args = ['--set', 'embedded_py=3', '--set', 'embedded_r=true']
...
with tempfile.TemporaryDirectory() as farm_dir:
    ...
    with process.server(args = server_args, mapiport='0', dbname='db1', dbfarm=os.path.join(farm_dir, 'db1'), stdin = process.PIPE, stdout = process.PIPE, stderr = process.PIPE) as s:
        ...
        cursor1.execute("SELECT CAST(myfunc1(1, 1) + myfunc2(1, 1) + myfunc3(1, 1) + myfunc4(1, 1) + myfunc5(1, 1) + myfunc6(1, 1) AS BIGINT);")
