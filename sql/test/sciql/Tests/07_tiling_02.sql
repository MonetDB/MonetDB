...
select x, y, sum(v),  avg(v),  min(v),  max(v),  count(v)            from a group by a[x-1:x+2][y-2:y+1];
select x, y, sum(v),  avg(v),  min(v),  max(v),  count(v), sum(v), v from a group by a[x-1:x+2][y-2:y+1];
select x, y, sum(v1), avg(v1), min(v1), max(v1), count(v1), v1, sum(v2), avg(v2), min(v2), max(v2), count(v2), v2 from aa group by aa[x-1:x+2][y-2:y+1];
drop array a;
drop array aa;
