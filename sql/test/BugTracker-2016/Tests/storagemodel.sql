...
select "schema", "table", "column", "type", "mode", case when typewidth <= 64 then typewidth else 99 end as typewidth, hashes, phash, imprints from storage('sys','_tables');
...
-- skip columns: location, count, columnsize, sorted.
select "schema", "table", "column", "type", "mode", case when typewidth <= 64 then typewidth else 99 end as typewidth, hashes, phash, imprints from sys.storage('sys','_tables');
