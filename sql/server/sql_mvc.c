...
mvc_commit(mvc *m, int chain, const char *name, bool enabling_auto_commit)
{
	...
	if (name && name[0] != '\0') {
		...
		if (!(m->session->schema = find_sql_schema(m->session->tr, m->session->schema_name))) {
			...
			return createException(SQL, "sql.commit", SQLSTATE(40000) "%s finished successfuly, but the session's schema could not be found on the current transaction", operation);
		}
		m->type = Q_TRANS;
	...
	*/
	...
	if (chain && sql_trans_begin(m->session) < 0)
		msg = createException(SQL, "sql.commit", SQLSTATE(40000) "%s finished successfuly, but the session's schema could not be found while starting the next transaction", operation);
	m->type = Q_TRANS;
	TRC_INFO(SQL_TRANS,
		"Commit done\n");
	return msg;
...
mvc_rollback(mvc *m, int chain, const char *name, bool disabling_auto_commit)
{
	...
	if (name && name[0] != '\0') {
		...
		if (!(m->session->schema = find_sql_schema(m->session->tr, m->session->schema_name))) {
			msg = createException(SQL, "sql.rollback", SQLSTATE(40000) "ROLLBACK: finished successfuly, but the session's schema could not be found on the current transaction");
			m->session->status = -1;
			return msg;
		}
	} else {
		...
		(void)sql_trans_end(m->session, SQL_ERR);
		if (chain && sql_trans_begin(m->session) < 0)
			msg = createException(SQL, "sql.rollback", SQLSTATE(40000) "ROLLBACK: finished successfuly, but the session's schema could not be found while starting the next transaction");
	}
	if (msg == MAL_SUCCEED)
...
mvc_release(mvc *m, const char *name)
{
	...
	m->session->tr = tr;
	if (!(m->session->schema = find_sql_schema(m->session->tr, m->session->schema_name))) {
		msg = createException(SQL, "sql.release", SQLSTATE(40000) "RELEASE: finished successfuly, but the session's schema could not be found on the current transaction");
		m->session->status = -1;
		return msg;
	}
