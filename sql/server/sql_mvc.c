...
mvc_init(int debug, store_type store, int ro, int su, backend_stack stk)
{
	...
	if (first || catalog_version) {
		...
		if (!first) {
			...
			MVC_INIT_DROP_TABLE(sid,  "streams")
...
schema_bind_type(mvc *sql, sql_schema *s, const char *name)
{
	...
	if (!t)
		return NULL;
	TRC_DEBUG(SQL_MVC, "Schema bind type: %s\n", name);
	return t;
}
...
mvc_bind_func(mvc *sql, const char *name)
{
	...
	TRC_DEBUG(SQL_MVC, "Bind function: %s\n", name);
	return t;
}
...
schema_bind_func(mvc *sql, sql_schema * s, const char *name, sql_ftype type)
{
	...
	if (!func_list)
		return NULL;
	TRC_DEBUG(SQL_MVC, "Schema bind function: %s\n", name);
	return func_list;
}
...
mvc_bind_schema(mvc *m, const char *sname)
{
	...
	if (!s)
		return NULL;
	TRC_DEBUG(SQL_MVC, "Bind schema: %s\n", sname);
	return s;
}
...
mvc_bind_column(mvc *m, sql_table *t, const char *cname)
{
	...
	if (!c)
		return NULL;
	TRC_DEBUG(SQL_MVC, "Bind column: %s.%s\n", t->base.name, cname);
	return c;
}
...
mvc_first_column(mvc *m, sql_table *t)
{
	...
	if (!c)
		return NULL;
	TRC_DEBUG(SQL_MVC, "First column: %s.%s\n", t->base.name, c->base.name);
	return c;
}
...
mvc_bind_key(mvc *m, sql_schema *s, const char *kname)
{
	...
	if (!n)
		return NULL;
	k = n->data;
	TRC_DEBUG(SQL_MVC, "Bind key: %s.%s\n", s->base.name, kname);
	return k;
}
...
mvc_bind_idx(mvc *m, sql_schema *s, const char *iname)
{
	...
	if (!n)
		return NULL;
	i = n->data;
	TRC_DEBUG(SQL_MVC, "Bind index: %s.%s\n", s->base.name, iname);
	return i;
}
...
mvc_bind_trigger(mvc *m, sql_schema *s, const char *tname)
{
	...
	if (!n)
		return NULL;
	trigger = n->data;
	TRC_DEBUG(SQL_MVC, "Bind trigger: %s.%s\n", s->base.name, tname);
	return trigger;
}
...
mvc_create_type(mvc *sql, sql_schema * s, const char *name, int digits, int scale, int radix, const char *impl)
{
	...
	TRC_DEBUG(SQL_MVC, "Create type: %s\n", name);
...
{
	TRC_DEBUG(SQL_MVC, "Drop type: %s %s\n", s->base.name, t->base.name);
	if (t)
...
mvc_create_func(mvc *sql, sql_allocator *sa, sql_schema *s, const char *name, list *args, list *res, sql_ftype type, sql_flang lang, const char *mod, const char *impl, const char *query, bit varres, bit vararg, bit system)
{
	...
	TRC_DEBUG(SQL_MVC, "Create function: %s\n", name);
	if (sa) {
...
{
	TRC_DEBUG(SQL_MVC, "Drop function: %s %s\n", s->base.name, f->base.name);
...
mvc_create_schema(mvc *m, const char *name, sqlid auth_id, sqlid owner)
{
	...
	TRC_DEBUG(SQL_MVC, "Create schema: %s %d %d\n", name, auth_id, owner);
...
{
	TRC_DEBUG(SQL_MVC, "Drop schema: %s\n", s->base.name);
...
{
	TRC_DEBUG(SQL_MVC, "Create ukey: %s %u\n", t->base.name, kt);
...
{
	TRC_DEBUG(SQL_MVC, "Create fkey: %s %u %p\n", t->base.name, kt, rkey);
	if (t->persistence == SQL_DECLARED_TABLE)
...
{
	TRC_DEBUG(SQL_MVC, "Drop key: %s %s\n", s->base.name, k->base.name);
	if (k->t->persistence == SQL_DECLARED_TABLE) {
...
mvc_create_idx(mvc *m, sql_table *t, const char *name, idx_type it)
{
	...
	TRC_DEBUG(SQL_MVC, "Create index: %s %u\n", t->base.name, it);
	if (t->persistence == SQL_DECLARED_TABLE)
...
{
	TRC_DEBUG(SQL_MVC, "Drop index: %s %s\n", s->base.name, i->base.name);
	if (i->t->persistence == SQL_DECLARED_TABLE) {
		/* declared tables should not end up in the catalog */
...
mvc_create_trigger(mvc *m, sql_table *t, const char *name, sht time, sht orientation, sht event, const char *old_name, const char *new_name, const char *condition, const char *statement )
{
	...
	TRC_DEBUG(SQL_MVC, "Create trigger: %s %d %d %d\n", t->base.name, time, orientation, event);
...
{
	TRC_DEBUG(SQL_MVC, "Drop trigger: %s %s\n", s->base.name, tri->base.name);
...
mvc_create_table(mvc *m, sql_schema *s, const char *name, int tt, bit system, int persistence, int commit_action, int sz, bit properties)
{
	...
	TRC_DEBUG(SQL_MVC, "Create table: %s %s %d %d %d %d %d\n", s->base.name, name, tt, system, persistence, commit_action, (int)properties);
	if (persistence == SQL_DECLARED_TABLE && (!s || strcmp(s->base.name, dt_schema))) {
		t = create_sql_table(m->sa, name, tt, system, persistence, commit_action, properties, 0, 0);
		t->s = s;
	} else {
		t = sql_trans_create_table(m->session->tr, s, name, NULL, tt, system, persistence, commit_action, sz, properties, 0, 0);
...
mvc_create_view(mvc *m, sql_schema *s, const char *name, int persistence, const char *sql, bit system)
{
	...
	TRC_DEBUG(SQL_MVC, "Create view: %s %s %s\n", s->base.name, name, sql);
	if (persistence == SQL_DECLARED_TABLE) {
		t = create_sql_table(m->sa, name, tt_view, system, persistence, 0, 0, 0, 0);
		t->s = s;
		t->query = sa_strdup(m->sa, sql);
	} else {
		t = sql_trans_create_table(m->session->tr, s, name, sql, tt_view, system, SQL_PERSIST, 0, 0, 0, 0, 0);
	}
	return t;
}
...
mvc_create_remote(mvc *m, sql_schema *s, const char *name, int persistence, const char *loc)
{
	...
	TRC_DEBUG(SQL_MVC, "Create remote: %s %s %s\n", s->base.name, name, loc);
	if (persistence == SQL_DECLARED_TABLE) {
		t = create_sql_table(m->sa, name, tt_remote, 0, persistence, 0, 0, 0, 0);
		t->s = s;
		t->query = sa_strdup(m->sa, loc);
	} else {
		t = sql_trans_create_table(m->session->tr, s, name, loc, tt_remote, 0, SQL_REMOTE, 0, 0, 0, 0, 0);
	}
	return t;
}
...
mvc_create_stream_table(mvc *m, sql_schema *s, const char *name, int tt, bit system, int persistence, int commit_action, int sz, int window, int stride)
{
	...
	if (persistence == SQL_DECLARED_TABLE && (!s || strcmp(s->base.name, dt_schema))) {
		t = create_sql_table(m->sa, name, tt, system, persistence, commit_action, 0, window, stride);
		t->s = s;
	} else {
		t = sql_trans_create_table(m->session->tr, s, name, NULL, tt, system, persistence, commit_action, sz, 0, window, stride);
	}
	return t;
}
...
{
	TRC_DEBUG(SQL_MVC, "Drop table: %s %s\n", s->base.name, t->base.name);
	if (isRemote(t)) {
		str AUTHres;
...
{
	TRC_DEBUG(SQL_MVC, "Drop column: %s %s\n", t->base.name, col->base.name);
...
{
	TRC_DEBUG(SQL_MVC, "Create dependency: %d %d %d\n", id, depend_id, (int) depend_type);
...
mvc_create_dependencies(mvc *m, list *id_l, sqlid depend_id, sql_dependency dep_type)
{
	...
	TRC_DEBUG(SQL_MVC, "Create dependencies on '%d' of type: %d\n", depend_id, (int) dep_type);
...
mvc_check_dependency(mvc * m, sqlid id, sql_dependency type, list *ignore_ids)
{
	...
	TRC_DEBUG(SQL_MVC, "Check dependency on: %d\n", id);
	switch (type) {
...
{
	TRC_DEBUG(SQL_MVC, "Drop default: %s\n", col->base.name);
	if (col->t->persistence == SQL_DECLARED_TABLE) {
		col->def = NULL;
		return col;
	} else {
