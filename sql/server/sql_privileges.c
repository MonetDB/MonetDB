...
sql_grant_global_privs( mvc *sql, char *grantee, int privs, int grant, sqlid grantor)
{
	...
	if (!allowed)
		throw(SQL,"sql.grant_global",SQLSTATE(0L000) "GRANT: Grantor '%s' is not allowed to grant global privileges", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"));
...
sql_grant_table_privs( mvc *sql, char *grantee, int privs, char *sname, char *tname, char *cname, int grant, sqlid grantor)
{
	...
	if (!cname) {
		...
		if (!allowed)
			throw(SQL,"sql.grant_table", SQLSTATE(0L000) "GRANT: Grantor '%s' is not allowed to grant privileges for table '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"), tname);
	}
	if (cname) {
		...
		if (!allowed)
			throw(SQL, "sql.grant_table", SQLSTATE(0L000) "GRANT: Grantor '%s' is not allowed to grant privilege %s for table '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), priv2string(privs), tname);
	}
...
sql_grant_func_privs( mvc *sql, char *grantee, int privs, char *sname, sqlid func_id, int grant, sqlid grantor)
{
	...
	if (!allowed)
		throw(SQL, "sql.grant_func", SQLSTATE(0L000) "GRANT: Grantor '%s' is not allowed to grant privileges for function '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"), f->base.name);
...
sql_revoke_global_privs( mvc *sql, char *grantee, int privs, int grant, sqlid grantor)
{
	...
	if (!allowed)
		throw(SQL, "sql.revoke_global", SQLSTATE(0L000) "REVOKE: Grantor '%s' is not allowed to revoke global privileges", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"));
...
sql_revoke_table_privs( mvc *sql, char *grantee, int privs, char *sname, char *tname, char *cname, int grant, sqlid grantor)
{
	...
	if (!allowed)
		throw(SQL, "sql.revoke_table", SQLSTATE(0L000) "REVOKE: Grantor '%s' is not allowed to revoke privileges for table '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"), tname);
	...
	if (cname) {
		...
		if (!allowed)
			throw(SQL, "sql.revoke_table", SQLSTATE(0L000) "REVOKE: Grantor '%s' is not allowed to revoke privilege %s for table '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), priv2string(privs), tname);
	}
...
sql_revoke_func_privs( mvc *sql, char *grantee, int privs, char *sname, sqlid func_id, int grant, sqlid grantor)
{
	...
	if (!allowed)
		throw(SQL, "sql.revoke_func", SQLSTATE(0L000) "REVOKE: Grantor '%s' is not allowed to revoke privileges for function '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"),"current_user"), f->base.name);
...
sql_alter_user(mvc *sql, char *user, char *passwd, char enc, char *schema, char *oldpasswd)
{
	...
	if (!admin_privs(sql->user_id) && !admin_privs(sql->role_id) && user != NULL && strcmp(user, stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user")) != 0)
