...
symbol_create(sql_allocator *sa, tokens token, char *data)
{
	...
	if (s) {
		...
		s->data.sval = data;
		DEBUG(SQL_SYMBOL, "%p = symbol_create_string(%s, %s)\n", s, token2string(s->token), s->data.sval);
	}
	return s;
}
...
symbol_create_list(sql_allocator *sa, tokens token, dlist *data)
{
	...
	if (s) {
		...
		s->data.lval = data;
		DEBUG(SQL_SYMBOL, "%p = symbol_create_list(%s, %p)\n", s, token2string(s->token), s->data.lval);
	}
	return s;
}
...
symbol_create_int(sql_allocator *sa, tokens token, int data)
{
	...
	if (s) {
		...
		s->data.i_val = data;
		DEBUG(SQL_SYMBOL, "%p = symbol_create_int(%s, %d)\n", s, token2string(s->token), data);
	}
	return s;
}
...
symbol_create_lng(sql_allocator *sa, tokens token, lng data)
{
	...
	if (s) {
		...
		s->data.l_val = data;
		DEBUG(SQL_SYMBOL, "%p = symbol_create_lng(%s, "LLFMT")\n", s, token2string(s->token), data);
	}
	return s;
}
