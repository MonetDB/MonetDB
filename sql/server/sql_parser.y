...
%nonassoc UNBOUNDED
%nonassoc IDENT PARTITION RANGE ROWS GROUPS PRECEDING FOLLOWING CUBE ROLLUP SET OBJECT VALUE WITH WITHOUT PATH
	...
	|	set sqlSESSION AUTHORIZATION opt_equal ident
		{
			...
			append_list(l, append_string(append_string(L(), "sys"), "current_role"));
	...
	|	set session_schema ident
		{
			...
			append_list(l, append_string(append_string(L(), "sys"), "current_schema"));
	...
	|	set session_user opt_equal ident
		{
			...
			append_list(l, append_string(append_string(L(), "sys"), "current_user"));
	...
	|	set session_timezone opt_equal LOCAL
		{
			...
			append_list(l, append_string(append_string(L(), "sys"), "current_timezone"));
...
schema_element_list:
	...
	|	schema_element_list schema_element { $$ = append_symbol($1, $2); }
	;
...
authid_list:
	...
	|	authid_list ',' authid { $$ = append_string($1, $3); }
	;
...
global_privileges:
	...
	|	global_privilege ',' global_privilege { $$ = append_int(append_int(L(), $1), $3); }
	;
...
operation_commalist:
	...
	|	operation_commalist ',' operation { $$ = append_symbol($1, $3); }
	;
...
grantee_commalist:
	...
	|	grantee_commalist ',' grantee { $$ = append_string($1, $3); }
	;
...
as_subquery_clause:
		opt_column_list	AS SelectStmt with_or_without_data
		{
			$$ = append_list(L(), $1);
			append_symbol($$, $3);
			append_int($$, $4);
...
table_element_list:
		table_element                        { $$ = append_symbol(L(), $1); }
	|	table_element_list ',' table_element { $$ = append_symbol( $1, $3 ); }
	;
...
column_def_opt_list:
		column_option                     { $$ = append_symbol(L(), $1); }
	|	column_def_opt_list column_option { $$ = append_symbol($1, $2); }
	;
...
column_option_list:
		column_option { $$ = append_symbol(L(), $1 ); }
	|	column_option_list ',' column_option { $$ = append_symbol($1, $3 ); }
	;
...
table_constraint_type:
		UNIQUE column_commalist_parens                    { $$ = _symbol_create_list( SQL_UNIQUE, $2); }
	|	UNIQUE NULLS DISTINCT column_commalist_parens     { $$ = _symbol_create_list( SQL_UNIQUE, $4); }
	|	UNIQUE NULLS NOT DISTINCT column_commalist_parens { $$ = _symbol_create_list( SQL_UNIQUE_NULLS_NOT_DISTINCT, $5); }
	|	PRIMARY KEY column_commalist_parens               { $$ = _symbol_create_list( SQL_PRIMARY_KEY, $3); }
...
ident_commalist:
	...
	|	ident_commalist ',' column_id { $$ = append_string( $1, $3 ); }
	;
...
func_def:
		create_or_replace func_def_type_no_proc qfunc '(' opt_paramlist ')' func_def_opt_returns func_def_opt_order_spec
	...
	|	create_or_replace PROCEDURE qfunc '(' opt_paramlist ')' EXTERNAL sqlNAME external_function_name
		{
			dlist *f = L();
			append_list(f, $3);
			append_list(f, $5);
	...
	|	create_or_replace func_def_type_no_proc qfunc '(' opt_paramlist ')' func_def_opt_returns routine_body
		{
			dlist *f = L();
			append_list(f, $3);
			append_list(f, $5);
	...
	|	create_or_replace PROCEDURE qfunc '(' opt_paramlist ')' procedure_body
		{
			dlist *f = L();
			append_list(f, $3);
			append_list(f, $5);
...
routine_body:
		return_statement                                                     { $$ = L(); append_symbol( $$, $1); }
	|	BEGIN procedure_statement_list procedure_statement SCOLON END        { $$ = append_symbol($2,$3); }
	|	BEGIN ATOMIC procedure_statement_list procedure_statement SCOLON END { $$ = append_symbol($3,$4); }
	;

procedure_body:
		procedure_statement                                                  { $$ = L(); append_symbol( $$, $1); }
	|	BEGIN procedure_statement_list procedure_statement SCOLON END        { $$ = append_symbol($2,$3); }
	|	BEGIN ATOMIC procedure_statement_list procedure_statement SCOLON END { $$ = append_symbol($3,$4); }
	;
...
procedure_statement_list:
		/* empty*/	                                        { $$ = L(); }
	|	procedure_statement_list procedure_statement SCOLON { $$ = append_symbol($1,$2);}
	;

trigger_procedure_statement_list:
		/* empty*/                                                          { $$ = L(); }
	|	trigger_procedure_statement_list trigger_procedure_statement SCOLON	{ $$ = append_symbol($1,$2);}
	;
...
return_value:
		select_no_parens
	|	scalar_exp
	|	TABLE select_with_parens { $$ = _symbol_create_symbol(SQL_TABLE, $2); }
	;

case_statement:
		CASE scalar_exp when_statements case_opt_else_statement END CASE
		{
			$$ = _symbol_create_list(SQL_CASE, append_list(append_list(append_symbol(L(), $2), $3), $4));
		}
	|	CASE when_statements case_opt_else_statement END CASE
		{
			$$ = _symbol_create_list(SQL_CASE, append_list(append_list(L(), $2), $3));
		}
	;
...
when_statement:
		...
		{
			$$ = _symbol_create_list( SQL_WHEN, append_list(append_symbol(L(), $2), $4));
		}
	;

when_statements:
		when_statement                 { $$ = append_symbol( L(), $1); }
	|	when_statements when_statement { $$ = append_symbol( $1, $2); }
	;
...
if_opt_else:
		/* empty */ { $$ = NULL; }
	|	ELSE procedure_statement_list { $$ = _symbol_create_list(SQL_ELSE, $2); }
...
while_statement:
		opt_begin_label WHILE scalar_exp DO procedure_statement_list END WHILE opt_end_label
		{
			dlist *l;
			char *label = $1?$1:$8;
			if ($1 && $8 && strcmp($1, $8) != 0) {
...
func_data_type:
		TABLE '(' table_function_column_list ')' { $$ = _symbol_create_list(SQL_TABLE, $3); }
	|	data_type                                { $$ = _symbol_create_list(SQL_TYPE, append_type(L(),&$1)); }
	;
...
old_or_new_values_alias_list:
	...
	|	old_or_new_values_alias_list old_or_new_values_alias { $$ = append_list($1, $2); }
	;
...
triggered_statement:
		trigger_procedure_statement                       { $$ = append_symbol(L(), $1); }
...
typelist:
		data_type              { dlist *l = L(); append_type(l, &$1 ); $$ = l; }
	|	data_type ',' typelist { append_type($3, &$1); $$ = $3; }
	;
...
transaction_statement:
		transaction_stmt { $$ = $1; m->type = Q_TRANS; }
	;
...
transaction_stmt:
		COMMIT opt_work opt_chain                           { $$ = _symbol_create_int( TR_COMMIT, $3); }
	|	SAVEPOINT column_id                                 { $$ = _symbol_create( TR_SAVEPOINT, $2); }
	|	RELEASE SAVEPOINT column_id                         { $$ = _symbol_create( TR_RELEASE, $3); }
	|	ROLLBACK opt_work opt_chain opt_to_savepoint        { $$ = _symbol_create_list( TR_ROLLBACK, append_string(append_int(L(), $3), $4 )); }
	|	start_transaction TRANSACTION transaction_mode_list { $$ = _symbol_create_int( TR_START, $3); }
	|	SET LOCAL TRANSACTION transaction_mode_list         { $$ = _symbol_create_int( TR_MODE, $4); }
	|	SET TRANSACTION transaction_mode_list               { $$ = _symbol_create_int( TR_MODE, $3); }
	;
...
_transaction_mode_list:
		transaction_mode                            { $$ = $1; }
	|	_transaction_mode_list ',' transaction_mode { $$ = ($1 | $3); }
	;
...
fwf_widthlist:
	...
	|	fwf_widthlist ',' poslng { $$ = append_lng($1, $3); }
	;
...
opt_nr:
	...
	|	poslng RECORDS OFFSET poslng { $$ = append_lng(append_lng(L(), $1), $4); }
	;
...
string_commalist_contents:
	...
	|	string_commalist_contents ',' string { $$ = append_string($1, $3); }
	;
...
merge_stmt:
		opt_with_clause MERGE INTO qname opt_alias_name USING table_ref ON scalar_exp merge_when_list
...
assignment_commalist:
	...
	|	assignment_commalist ',' assignment { $$ = append_symbol($1, $3 ); }
	;
...
join_spec:
		ON scalar_exp                 { $$ = $2; }
	|	USING column_commalist_parens { $$ = _symbol_create_list( SQL_USING, $2); }
	;
	...
	|   select_clause order_by_clause
		{
			...
			if ($2) {
				if ($1 != NULL && ($1->token == SQL_SELECT || $1->token == SQL_UNION  ||
								   $1->token == SQL_EXCEPT || $1->token == SQL_INTERSECT)) {
					if ($1->token == SQL_SELECT) {
						...
						$$ = newSelectNode(
							SA, 0,
							append_symbol(L(), _symbol_create_list(SQL_TABLE, append_string(append_string(L(),NULL),NULL))),
							NULL, _symbol_create_list(SQL_FROM, append_symbol(L(), $1)),
							NULL, NULL, NULL, $2,
							_symbol_create_list(SQL_NAME, append_list(append_string(L(),"inner"),NULL)),
							NULL, NULL, NULL, NULL, NULL, NULL);
					}
				} else {
				...
				if ($1 != NULL && ($1->token == SQL_SELECT || $1->token == SQL_UNION  ||
					...
					if ($1->token == SQL_SELECT) {
						...
						$$ = newSelectNode(
							...
							NULL, NULL, NULL, $2,
							_symbol_create_list(SQL_NAME, append_list(append_string(L(),"inner"),NULL)),
							$3?$3->h->data.sym:NULL, $3?$3->h->next->data.sym:NULL, $4, $5, NULL, NULL);
					}
				} else {
					yyerror(m, "missing SELECT operator");
					YYABORT;
	...
	|	with_clause select_clause order_by_clause
		{
			$$ = $1;
			if ($3) {
				if ($2 != NULL && ($2->token == SQL_SELECT || $2->token == SQL_UNION  ||
								   $2->token == SQL_EXCEPT || $2->token == SQL_INTERSECT)) {
					if ($2->token == SQL_SELECT) {
						...
						$2 = newSelectNode(
							SA, 0,
							append_symbol(L(), _symbol_create_list(SQL_TABLE, append_string(append_string(L(),NULL),NULL))),
							NULL, _symbol_create_list( SQL_FROM, append_symbol(L(), $2)),
							NULL, NULL, NULL, $3,
							_symbol_create_list(SQL_NAME, append_list(append_string(L(),"inner"),NULL)),
							NULL, NULL, NULL, NULL, NULL, NULL);
					}
				...
				if ($2 != NULL && ($2->token == SQL_SELECT || $2->token == SQL_UNION  ||
					...
					if ($2->token == SQL_SELECT) {
						...
						$2 = newSelectNode(
							...
							NULL, NULL, NULL, $3,
							_symbol_create_list(SQL_NAME, append_list(append_string(L(),"inner"),NULL)),
							$4?$4->h->data.sym:NULL, $4?$4->h->next->data.sym:NULL, $5, $6, NULL, NULL);
					}
				} else {
					yyerror(m, "missing SELECT operator");
					YYABORT;
...
opt_corresponding:
		/* empty */                                   { $$ = NULL; }
	|	CORRESPONDING                                 { $$ = L(); }
	|	CORRESPONDING BY '(' column_ref_commalist ')' { $$ = $4; }
	;
...
table_ref_commalist:
	...
	|	table_ref_commalist ',' table_ref { $$ = append_symbol($1, $3); }
	;
...
sort_specification_list:
	...
	|	sort_specification_list ',' ordering_spec { $$ = append_symbol($1, $3 ); }
	;
...
scalar_exp:
	...
	|	scalar_exp '+' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "sql_add"));
	...
	|	scalar_exp '-' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "sql_sub"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '*' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "sql_mul"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '/' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "sql_div"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '%' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "mod"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '^' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "bit_xor"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '&' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "bit_and"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp CONCATSTRING scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "concat"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_OVERLAP scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_overlap"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_OVERLAP_OR_LEFT scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_overlap_or_left"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp  GEOM_OVERLAP_OR_RIGHT scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_overlap_or_right"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_OVERLAP_OR_BELOW scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_overlap_or_below"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_BELOW scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_below"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_OVERLAP_OR_ABOVE scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_overlap_or_above"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_ABOVE scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_above"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_DIST scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_distance"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp AT scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_contained"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '|' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "bit_or"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp '~' scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_contains"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp GEOM_MBR_EQUAL scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(L(), "mbr_equal"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	'~'scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "bit_not"));
			append_int(l, FALSE); /* ignore distinct */
	...
	|	scalar_exp LEFT_SHIFT scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "left_shift"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp RIGHT_SHIFT scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "right_shift"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp LEFT_SHIFT_ASSIGN scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "left_shift_assign"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	scalar_exp RIGHT_SHIFT_ASSIGN scalar_exp
		{
			dlist *l = L();
			append_list(l, append_string(append_string(L(), "sys"), "right_shift_assign"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $1);
			append_symbol(args, $3);
			append_list(l, args);
	...
	|	'-'scalar_exp %prec UMINUS
		{
			...
			if (!$$) {
				dlist *l = L();
				append_list(l,
							append_string(append_string(L(), "sys"), "sql_neg"));
				append_int(l, FALSE); /* ignore distinct */
				append_list(l, append_symbol(L(), $2));
				$$ = _symbol_create_list( SQL_NOP, l );
			}
	...
	|	'-'scalar_exp_no_and %prec UMINUS
		{
			...
			if (!$$) {
				dlist *l = L();
				append_list(l, append_string(append_string(L(), "sys"), "sql_neg"));
				append_int(l, FALSE); /* ignore distinct */
				append_list(l, append_symbol(L(), $2));
				$$ = _symbol_create_list( SQL_NOP, l );
			}
...
window_partition_clause:
		/* empty */            { $$ = NULL; }
	|	PARTITION BY expr_list { $$ = _symbol_create_list( SQL_GROUPBY, $3 ); }
	;
...
window_frame_start:
	...
	|	scalar_exp PRECEDING
		{
			dlist *l2 = append_symbol(L(), $1);
			$$ = _symbol_create_list( SQL_PRECEDING, l2);
		}
	|	CURRENT ROW
		{
...
datetime_funcs:
	...
	|	CURRENT_DATE opt_brackets
		{
			dlist *l = L();
			append_list(l, append_string(L(), "current_date"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, NULL);
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	CURRENT_TIME opt_brackets
		{
			dlist *l = L();
			append_list(l, append_string(L(), "current_time"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, NULL);
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	CURRENT_TIMESTAMP opt_brackets
		{
			dlist *l = L();
			append_list(l, append_string(L(), "current_timestamp"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, NULL);
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	LOCALTIME opt_brackets
		{
			dlist *l = L();
			append_list(l, append_string(L(), "localtime"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, NULL);
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	LOCALTIMESTAMP opt_brackets
		{
			dlist *l = L();
			append_list(l, append_string(L(), "localtimestamp"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, NULL);
			$$ = _symbol_create_list( SQL_NOP, l );
		}
...
string_funcs:
	...
	|	SUBSTRING '(' scalar_exp FROM scalar_exp ')'
		{
			dlist *l = L();
			append_list(l, append_string(L(), "substring"));
			append_int(l, FALSE); /* ignore distinct */
	...
	|	POSITION '(' scalar_exp_no_and sqlIN scalar_exp_no_and ')'
		{
			dlist *l = L();
			append_list(l, append_string(L(), "locate"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $3);
			append_symbol(args, $5);
			append_list(l, args);
	...
	|	SPLIT_PART '(' scalar_exp ',' scalar_exp ',' scalar_exp ')'
		{
			dlist *l = L();
			dlist *ops = L();
			append_list(l, append_string(L(), "splitpart"));
			append_int(l, FALSE); /* ignore distinct */
			append_symbol(ops, $3);
			append_symbol(ops, $5);
			append_symbol(ops, $7);
...
column_exp_commalist:
	...
	|	column_exp_commalist ',' column_exp { $$ = append_symbol( $1, $3 ); }
	;
...
end_field:
		non_second_datetime_field  { $$ = append_int( append_int( L(), $1), 0); }
	|	SECOND timestamp_precision { $$ = append_int( append_int( L(), isec), $2-1);  }
	;

single_datetime_field:
		non_second_datetime_field time_precision { $$ = append_int( append_int( L(), $1), $2-1); }
	|	SECOND timestamp_precision               { $$ = append_int( append_int( L(), isec), $2-1); }
	;
...
case_exp:
		...
		{ $$ = _symbol_create_list(SQL_NULLIF, append_symbol(append_symbol(L(), $3), $5)); }
	|	IFNULL '(' scalar_exp ',' scalar_exp ')'
		{ $$ = _symbol_create_list(SQL_COALESCE, append_symbol(append_symbol(L(), $3), $5)); }
	|	COALESCE '(' expr_list ')'
...
when_value_list:
		when_value                 { $$ = append_symbol( L(), $1);}
	|	when_value_list when_value { $$ = append_symbol( $1, $2); }
	;

when_value:
		WHEN scalar_exp THEN scalar_exp
		{ $$ = _symbol_create_list( SQL_WHEN, append_symbol(append_symbol(L(), $2),$4)); }
	;
...
data_type:
	...
	|	CHARACTER '(' nonzero ')' { sql_find_subtype(&$$, "char", $3, 0); }
	|	varchar '(' nonzero ')'   { sql_find_subtype(&$$, "varchar", $3, 0); }
	|	clob '(' nonzero ')'
		{
	...
	|	aTYPE '(' nonzero ')'     { sql_find_subtype(&$$, $1, $3, 0); }
...
non_reserved_keyword:
		AUTHORIZATION { $$ = "authorization"; } /* sloppy: officially reserved */
	|	ANALYZE       { $$ = "analyze"; }       /* sloppy: officially reserve */
	|	CACHE         { $$ = "cache"; }
	|	CYCLE         { $$ = "cycle"; }         /* sloppy: officially reserved */
	...
	|	PREPARE       { $$ = "prepare"; }       /* sloppy: officially reserved */
	|	RELEASE       { $$ = "release"; }       /* sloppy: officially reserved */
	|	START         { $$ = "start"; }         /* sloppy: officially reserved */
	|	UESCAPE       { $$ = "uescape"; }       /* sloppy: officially reserved */
	|	VALUE         { $$ = "value"; }         /* sloppy: officially reserved */
	|	WITHOUT       { $$ = "without"; }       /* sloppy: officially reserved */
...
column_name_keyword:
	...
	|	NULLIF        { $$ = "nullif"; }
	|	POSITION      { $$ = "position"; }
	|	PRECISION     { $$ = "precision"; }     /* sloppy: officially reserved */
	|	sqlREAL
	|	ROW           { $$ = "row"; }           /* sloppy: officially reserved */
	|	SUBSTRING     { $$ = "substring"; }
	|	TIME          { $$ = "time"; }
	|	TIMESTAMP     { $$ = "timestamp"; }
	|	TRIM          { $$ = "trim"; }
	|	VALUES        { $$ = "values"; }
	|	XMLATTRIBUTES { $$ = "xmlatributes"; }
	|	XMLCONCAT     { $$ = "xmlconcat"; }
...
XML_value_expression_list:
		XML_value_expression                               { $$ = append_symbol(L(), $1); }
	|	XML_value_expression_list ',' XML_value_expression { $$ = append_symbol($1, $3); }
	;
...
forest_element_list:
		forest_element                         { $$ = append_list(L(), $1); }
	|	forest_element_list ',' forest_element { $$ = append_list($1, $3); }
	;
...
/* |XML_value_function */
XML_parse:
		XMLPARSE '(' document_or_content value_exp /* should be a string */ XML_whitespace_option ')'
		{
			dlist *l = L();
			append_int(l, $3 );
			append_symbol(l, $4);
...
opt_comma_string_value_expression:
		/* empty */                         { $$ = NULL; }
	| ','value_exp /* should be a string */ { $$ = $2; }
	;
...
XML_text:
		XMLTEXT '(' value_exp /* should be a string */ opt_XML_returning_clause ')'
		{
			dlist *l = L();
			append_symbol(l, $3);
			append_int(l, $4);
...
odbc_datetime_func:
		...
		{
			dlist *l = L();
			append_list( l, append_string(L(), "hour"));
			append_int(l, FALSE); /* ignore distinct */
	...
	|	MINUTE '(' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "minute"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, append_symbol(L(), $3));
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	SECOND '(' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "second"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, append_symbol(L(), $3));
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	MONTH '(' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "month"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, append_symbol(L(), $3));
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	YEAR '(' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "year"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, append_symbol(L(), $3));
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	ODBC_TIMESTAMPADD '(' odbc_tsi_qualifier ',' scalar_exp ',' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "timestampadd"));
			append_int(l, FALSE); /* ignore distinct */
			sql_subtype t;
			lng i = 0;
			...
			append_list(
			append_int(
			append_list(L(), append_string(L(), "sql_mul")),
	...
	|	ODBC_TIMESTAMPDIFF '(' odbc_tsi_qualifier ',' scalar_exp ',' scalar_exp ')'
		{
			...
			switch($3) {
			case iyear:
				append_list( l, append_string(L(), "timestampdiff_year"));
				break;
			case iquarter:
				append_list( l, append_string(L(), "timestampdiff_quarter"));
				break;
			case imonth:
				append_list( l, append_string(L(), "timestampdiff_month"));
				break;
			case iweek:
				append_list( l, append_string(L(), "timestampdiff_week"));
				break;
			case iday:
				append_list( l, append_string(L(), "timestampdiff_day"));
				break;
			case ihour:
				append_list( l, append_string(L(), "timestampdiff_hour"));
				break;
			case imin:
				append_list( l, append_string(L(), "timestampdiff_min"));
				break;
			case isec:
				append_list( l, append_string(L(), "timestampdiff_sec"));
				break;
			default:
				/* diff in ms */
				append_list( l, append_string(L(), "timestampdiff"));
			}
			append_int(l, FALSE); /* ignore distinct */
...
odbc_scalar_func:
	...
	|	USER '(' ')'
		{
			$$ = _symbol_create_list(SQL_NAME, append_string(append_string(L(), "sys"), "current_user"));
		}
	|	CHARACTER '(' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "code"));
			append_int(l, FALSE); /* ignore distinct */
			append_list(l, append_symbol(L(), $3));
			$$ = _symbol_create_list( SQL_NOP, l );
		}
	|	TRUNCATE '(' scalar_exp ',' scalar_exp ')'
		{
			dlist *l = L();
			append_list( l, append_string(L(), "ms_trunc"));
			append_int(l, FALSE); /* ignore distinct */
			dlist *args = append_symbol(L(), $3);
			append_symbol(args, $5);
			append_list(l, args);
...
odbc_data_type:
		SQL_BIGINT                    { sql_find_subtype(&$$, "bigint", 0, 0); }
	|	SQL_BINARY                    { sql_find_subtype(&$$, "blob", 0, 0); }
	|	SQL_BIT                       { sql_find_subtype(&$$, "boolean", 0, 0); }
	|	SQL_CHAR                      { sql_find_subtype(&$$, "char", 0, 0); }
	|	SQL_DATE                      { sql_find_subtype(&$$, "date", 0, 0); }
	|	SQL_DECIMAL                   { sql_find_subtype(&$$, "decimal", 0, 0); }
	|	SQL_DOUBLE                    { sql_find_subtype(&$$, "double", 0, 0); }
	|	SQL_FLOAT                     { sql_find_subtype(&$$, "double", 0, 0); }
	|	SQL_GUID
	{
		sql_type* t = NULL;
	...
	|	SQL_HUGEINT  /* Note: SQL_HUGEINT is not part of or defined in ODBC. This is a MonetDB extension. */ { sql_find_subtype(&$$, "hugeint", 0, 0); }
	|	SQL_INTEGER                   { sql_find_subtype(&$$, "int", 0, 0); }
	|	SQL_INTERVAL_YEAR             { sql_find_subtype(&$$, "month_interval", 1, 0); }
	|	SQL_INTERVAL_YEAR_TO_MONTH    { sql_find_subtype(&$$, "month_interval", 2, 0); }
	|	SQL_INTERVAL_MONTH            { sql_find_subtype(&$$, "month_interval", 3, 0); }
	|	SQL_INTERVAL_DAY              { sql_find_subtype(&$$, "day_interval", 4, 0); }
	|	SQL_INTERVAL_DAY_TO_HOUR      { sql_find_subtype(&$$, "sec_interval", 5, 0); }
	|	SQL_INTERVAL_DAY_TO_MINUTE    { sql_find_subtype(&$$, "sec_interval", 6, 0); }
	|	SQL_INTERVAL_DAY_TO_SECOND    { sql_find_subtype(&$$, "sec_interval", 7, 0); }
	|	SQL_INTERVAL_HOUR             { sql_find_subtype(&$$, "sec_interval", 8, 0); }
	|	SQL_INTERVAL_HOUR_TO_MINUTE   { sql_find_subtype(&$$, "sec_interval", 9, 0); }
	|	SQL_INTERVAL_HOUR_TO_SECOND   { sql_find_subtype(&$$, "sec_interval", 10, 0); }
	|	SQL_INTERVAL_MINUTE           { sql_find_subtype(&$$, "sec_interval", 11, 0); }
	|	SQL_INTERVAL_MINUTE_TO_SECOND { sql_find_subtype(&$$, "sec_interval", 12, 0); }
	|	SQL_INTERVAL_SECOND           { sql_find_subtype(&$$, "sec_interval", 13, 0); }
	|	SQL_LONGVARBINARY             { sql_find_subtype(&$$, "blob", 0, 0); }
	|	SQL_LONGVARCHAR               { sql_find_subtype(&$$, "varchar", 0, 0); }
	|	SQL_NUMERIC                   { sql_find_subtype(&$$, "decimal", 0, 0); }
	|	SQL_REAL                      { sql_find_subtype(&$$, "real", 0, 0); }
	|	SQL_SMALLINT                  { sql_find_subtype(&$$, "smallint", 0, 0); }
	|	SQL_TIME                      { sql_find_subtype(&$$, "time", 0, 0); }
	|	SQL_TIMESTAMP                 { sql_find_subtype(&$$, "timestamp", 6, 0); }
	|	SQL_TINYINT                   { sql_find_subtype(&$$, "tinyint", 0, 0); }
	|	SQL_VARBINARY                 { sql_find_subtype(&$$, "blob", 0, 0); }
	|	SQL_VARCHAR                   { sql_find_subtype(&$$, "varchar", 0, 0); }
	|	SQL_WCHAR                     { sql_find_subtype(&$$, "char", 0, 0); }
	|	SQL_WLONGVARCHAR              { sql_find_subtype(&$$, "varchar", 0, 0); }
	|	SQL_WVARCHAR                  { sql_find_subtype(&$$, "varchar", 0, 0); }
	;
