...
symbol2string(mvc *sql, symbol *se, int expression, char **err) /**/
{
	switch (se->token) {
	case SQL_NOP: {
		dnode *lst = se->data.lval->h, *ops = lst->next->data.lval->h, *aux;
	...
	case SQL_BINOP: {
		...
		if (!sname)
			sname = sql->session->schema->base.name;
		if (!(l = symbol2string(sql, lst->next->data.sym, expression, err)) || !(r = symbol2string(sql, lst->next->next->data.sym, expression, err))) {
			_DELETE(l);
			_DELETE(r);
			return NULL;
		}
	...
	case SQL_UNOP: {
		dnode *lst = se->data.lval->h;
		const char *op = qname_fname(lst->data.lval), *sname = qname_schema(lst->data.lval);
		char *l = symbol2string(sql, lst->next->data.sym, expression, err), *res;
