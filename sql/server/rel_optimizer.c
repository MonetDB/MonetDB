		...
		for(n=exps->h; n; n = n->next) {
			...
			*/
			if (exp_is_join_exp(e) == 0) {
				sql_rel *nr = NULL;
				if (e->flag == cmp_equal)
					nr = rel_push_join(v->sql, top->l, e->l, e->r, NULL, e);
				if (!nr)
		...
		for (n = exps->h; n; n = n->next) {
			...
			if (e->type == e_cmp && !e->f && !is_complex_exp(e->flag)) {
				...
				if (re->card >= CARD_AGGR) {
					nr = rel_push_join(v->sql, r, e->l, re, NULL, e);
				} else {
					nr = rel_push_select(v->sql, r, e->l, e);
				}
				if (nr)
					rel->l = nr;
				...
				if (t->members) {
					...
					for (nt = t->members->h; nt; nt = nt->next) {
						...
						for (n = rel->exps->h, j=0; n && (!skip || first); n = n->next, j++) {
							...
							if (e)
								ne = exps_bind_column2(prel->exps, e->l, e->r, NULL);
							if (!e || !ne) {
								v->changes--;
								assert(0);
								return rel;
