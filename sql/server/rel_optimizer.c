...
typedef sql_rel *(*rewrite_fptr)(mvc *sql, sql_rel *rel, int *changes);
		...
		if (is_basetable(rel->op) && rel->l) {
			...
			if (isMergeTable(t)) {
				...
				if (sel) {
					int changes = 0;
					sel->l = nrel;
					sel = rel_visitor_topdown(sql, sel, &rel_push_select_down_union, &changes);
					if (changes)
