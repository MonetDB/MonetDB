		...
		if (a->type == e_aggr) {
			...
			if (strcmp(af->aggr->base.name, name) == 0 &&
				...
				if (eae->type == e_column &&
					...
					return exp_column(sa, a->rname, exp_name(a), exp_subtype(a), a->card, has_nil(a), is_intern(a), NULL);
			}
		}
	}
	return NULL;
	...
	if (is_groupby(rel->op) && !rel_is_ref(rel)) {
		...
		for (m = rel->exps->h, n = avgs->h; m; m = m->next) {
			...
			if (e->type == e_column && !rel_find_exp(rel->l, e))  
				...
				append(pexps, exp_column(sql->sa, exp_find_rel_name(e), exp_name(e), exp_subtype(e), e->card, has_nil(e), is_intern(e), NULL));
		}
		rel->exps = nexps;
...
exp_rename_up(mvc *sql, sql_exp *e, list *aliases) 
{
	...
	case e_column:
		...
		if (ne)
			ne = exp_column(sql->sa, exp_relname(ne), exp_name(ne), exp_subtype(ne), ne->card, has_nil(ne), is_intern(ne), NULL);
		else
			return e;
		break;
	case e_cmp: 
...
rel_rename(mvc *sql, sql_rel *rel, list *aliases)
{
	...
	case op_table: {
		...
		for (n = rel->exps->h; n; n = n->next) {
			...
			ne = exp_column(sql->sa, e->l, e->r, exp_subtype(e), e->card, has_nil(e), is_intern(e), NULL);
			exp_setname(sql->sa, ne, nme, e->r);
			a = exp_column(sql->sa, exp_relname(e), exp_name(e), exp_subtype(e), e->card, has_nil(e), is_intern(e), NULL);
...
rel_apply_rewrite(int *changes, mvc *sql, sql_rel *rel) 
{
	...
	if (rel->flag == APPLY_EXISTS || rel->flag == APPLY_NOTEXISTS) { /* rewrite as semijoin (difference/join)*/ /* rewrite as antijoin (minus/join) */
		...
		rel->l = nrel;
		ident = exp_column(sql->sa, exp_relname(ident), exp_name(ident), exp_subtype(ident), ident->card, has_nil(ident), is_intern(ident), NULL);
			...
			for (n=r->exps->h; n; n = n->next) {
				...
				if (e->l && exp_uses_exps(e->l, rel->exps)) {
					...
					case e_column:
						...
						if (ne) {
							ne = exp_column(sql->sa, ne->l, ne->r, exp_subtype(ne), ne->card, has_nil(ne), is_intern(ne), NULL);
							e->l = ne;
						}
						break;
					case e_func:
	...
	if (r->op == op_groupby) { /* groupby */ 
		...
		for(n=r->exps->h; n; n = n->next) {
			...
			if (!has_gbe && e->type == e_aggr && strcmp(((sql_subaggr *)e->f)->aggr->base.name, "count") == 0 && !e->l) {
					...
					set_no_nil(e);
					e->l = l;
					c = exp_column(sql->sa, exp_relname(c), exp_name(c), exp_subtype(c), exp_card(c), has_nil(c), is_intern(c), NULL);
					append(l, c);
			}
			append(rel->exps, e);
		}
