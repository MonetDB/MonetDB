...
rel_get_statistics_(visitor *v, sql_rel *rel)
{
	...
	case op_basetable: {
		...
		if (isTable(t) && t->s && !isDeclaredTable(t)) /* count active rows only */
			set_count_prop(v->sql->sa, rel, (BUN)store->storage_api.count_col(v->sql->session->tr, ol_last_node(t->columns)->data, CNT_ACTIVE));
...
{
	const ATOMIC_BASE_TYPE oahash_mask = (1U<<19);
	bool oahash_enabled = (ATOMIC_GET(&GDKdebug) & oahash_mask);
	(void) v;
