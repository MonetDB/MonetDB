...
rel_create_func(sql_query *query, dlist *qname, dlist *params, symbol *res, dlist *ext_name, dlist *body, sql_ftype type, sql_flang lang, int replace, int order_spec)
{
	...
	if ((sf = sql_bind_func_(sql, s->base.name, fname, type_list, type==F_GROUPFILT?F_FILT:type, true, true, false)) != NULL && create) {
	...
	if (create && (type == F_FUNC || type == F_AGGR || type == F_FILT || type == F_GROUPFILT)) {
		...
		if ((found = sql_bind_func_(sql, s->base.name, fname, type_list, (type == F_FUNC || type == F_FILT || type == F_GROUPFILT) ? F_AGGR : F_FUNC, true, true, false))) {
			list_destroy(type_list);
