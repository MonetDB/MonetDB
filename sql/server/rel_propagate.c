	...
	for (node *n = t->members->h; n; n = n->next) {
		...
		if (isRangePartitionTable(t)) {
			...
			if (is_min_nil && is_max_nil) {
				...
				full_range = range = exp_compare(sql->sa, le, exp_atom(sql->sa, atom_general_ptr(sql->sa, &pt->tpe, pt->part.range.maxvalue)), cmp_lt);
			} else if (is_max_nil) {
				full_range = range = exp_compare(sql->sa, le, exp_atom(sql->sa, atom_general_ptr(sql->sa, &pt->tpe, pt->part.range.minvalue)), cmp_gte);
			} else {
				...
				full_range = range = max_equal_min ? 
					exp_compare(sql->sa, le, exp_atom(sql->sa, atom_general_ptr(sql->sa, &pt->tpe, pt->part.range.minvalue)), cmp_equal) :
					exp_compare2(sql->sa, le, exp_atom(sql->sa, atom_general_ptr(sql->sa, &pt->tpe, pt->part.range.minvalue)),
											  exp_atom(sql->sa, atom_general_ptr(sql->sa, &pt->tpe, pt->part.range.maxvalue)), cmp_gte|CMP_BETWEEN);
			}
