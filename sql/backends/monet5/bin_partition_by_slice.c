				...
				for(node *n = inputs->h; n; n = n->next) {
					...
					if (!exp_is_scalar(e)) {
						...
						if (need_distinct) { /* create shared bat, for hash table */
							lng estimate = exp_getcard(be->mvc, rel->l /* count before group by */, e);
							if (estimate<0) {
								assert(0);
								estimate = 85000000;
							}
...
rel2bin_groupby_pp(backend *be, sql_rel *rel, list *refs)
{
	...
	if (!sub && !rel->spb) {
		...
		assert(nrparts);
		int source = pp_counter(be, -1, nrparts, false);
