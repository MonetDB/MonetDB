	...
	if (is_groupby(rel->op) && list_empty(rel->r) && !list_empty(rel->exps)) { /* global aggregation */
		...
		*aggrresults = sa_list(be->mvc->sa);
		for( node *n = rel->exps->h; n; n = n->next ) {
			...
			if (need_distinct(e)) { /* create shared bat, for hash table */
				...
				InstrPtr q = stmt_hash_new(be, t->type->localtype, estimate, false, 0); /* pushed already */
				if (q == NULL)
					return NULL;
				assert(!e->shared);
				e->shared = q->argv[0];
		...
		*aggrresults = sa_list(be->mvc->sa);
		for(node *n = gbexps->h; n; n = n->next ) {
			...
			InstrPtr q = stmt_hash_new(be, t->type->localtype, card, false, curhash);
			if (q == NULL)
				return NULL;
			curhash = getArg(q,0);
		...
		for( node *n = rel->exps->h; n; n = n->next ) {
			...
			if (need_distinct(e)) { /* create shared bat, for hash table */
				...
				InstrPtr q = stmt_hash_new(be, t->type->localtype, estimate, false, curhash);
				if (q == NULL)
					return NULL;
				assert(!e->shared);
				e->shared = q->argv[0];
