		...
		if (left && right && !is_select &&
		   ((l->nrcols && (r->nrcols || (r2 && r2->nrcols))) || 
			...
			if (r2) {
				...
				s = stmt_join(be, r, l, NULL, is_anti(e), swap_compare((comp_type)e->flag));
			} else {
				s = stmt_join(be, l, r, NULL, is_anti(e), (comp_type)e->flag);
			}
		} else {
			if (r2) {
...
releqjoin( backend *be, list *l1, list *l2, int used_hash, comp_type cmp_op, int need_left )
{
	...
	if (used_hash) {
		...
		res = stmt_join(be, l, r, NULL, 0, cmp_op == cmp_equal ? cmp_equal_nil : cmp_op);
	}
	if (need_left) 
		res->flag = cmp_left;
...
rel2bin_join(backend *be, sql_rel *rel, list *refs)
{
	...
	if (rel->exps) {
		...
		if (list_length(lje) > 1) {
			...
			join = stmt_join(be, lje->h->data, rje->h->data, NULL, 0, cmp_equal);
			if (need_left)
				join->flag = cmp_left;
		}
	} else {
...
join_updated_pkey(backend *be, sql_key * k, stmt *tids, stmt **updates)
{
	...
	rows = stmt_join(be, rows, tids, NULL, 0, cmp_equal); /* join over the join index */
...
sql_update_cascade_Fkeys(backend *be, sql_key *k, stmt *utids, stmt **updates, int action)
{
	...
	rows = stmt_join(be, rows, utids, NULL, 0, cmp_equal); /* join over the join index */
...
sql_delete_ukey(backend *be, stmt *utids /* deleted tids from ukey table */, sql_key *k, list *l, char* which, int cascade)
{
	...
	if (uk->keys && list_length(uk->keys) > 0) {
		...
		for(n = uk->keys->h; n; n = n->next) {
			...
			s = stmt_join(be, s, utids, NULL, 0, cmp_equal); /* join over the join index */
			s = stmt_result(be, s, 0);
