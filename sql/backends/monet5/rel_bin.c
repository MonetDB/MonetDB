...
static vec_dim*
vec_values(backend *be, sql_exp *exp, stmt *left, stmt *sel, vec_dim *acc, unsigned int dim)
{
...
rel2bin_basetable(backend *be, sql_rel *rel)
{
	...
	stmt *dels = stmt_tid(be, t, !complex?rel->partition:false), *col = NULL;
	node *en, *cn = NULL;
...
insert_ms(backend *be, sql_table *st, sql_subtype *ct, stmt *ms)
{
	...
	if (st->s) /* only not declared tables, need this */
		pos = stmt_claim(be, st, cnt, 0 /* no sync */);
	...
	for (n = ol_first_node(st->columns); n; n = n->next) {
		...
		if (ins) {
			insert = stmt_append_col(be, c, pos, ins, &mvc_var, false, false);
			if (!insert)
				return NULL;
			append(l,insert);
		}
