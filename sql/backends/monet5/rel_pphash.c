...
rel2bin_oahash_groupjoin(backend *be, sql_rel *rel, list *refs)
{
	...
	if (!list_empty(sexps)) {
		...
		for (node *n = probe_side->h; n; n = n->next) {
			...
			sql_alias *ta = table_name(be->mvc->sa, c);
			int label = c->label;
			c = stmt_alias(be, c, label, ta, cname);
			append(lp, c);
		}
	...
	if (list_length(rel->attr)) {
		if (mark) {
			...
			stmt *s = stmt_alias(be, prb_mrk, e->alias.label, ra, nme);
...
rel2bin_oahash_rightouterjoin(backend *be, sql_rel *rel, list *refs)
{
	...
	if (!list_empty(shared_hp)) {
		for (node *n = shared_hp->h; n; n = n->next) {
			...
			sql_alias *ta = table_name(sql->sa, c);
			int label = c->label;
			c = stmt_alias(be, c, label, ta, cname);
			append(lh, c);
		}
	...
	for (node *n = probe_side->h; n; n = n->next) {
		...
		sql_alias *ta = table_name(sql->sa, n->data);
