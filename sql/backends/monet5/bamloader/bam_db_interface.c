...
create_schema_if_not_exists(Client cntxt, mvc *m, str schemaname, str descr, sql_schema **ret) {
    ...
    if((result = mvc_bind_schema(m, schemaname)) == NULL) {
        ...
        if((result = mvc_bind_schema(m, schemaname)) == NULL) {
            throw(MAL, "create_schema_if_not_exists", "Could not create bam schema");
        }
    }
    if(ret) *ret = result;
    return MAL_SUCCEED;
...
/**
 * Function tries to bind to a table with the given name. If it fails (== NULL), it attempts to create the table. 
 * The function fails if a binding to the table is impossible, even after creation.
 * If the function succeeds, it adjusts the optionally given pointer to point to the binded sql_table.
 */
str 
create_table_if_not_exists(Client cntxt, mvc *m, sql_schema *s, str tablename, str sql_creation, str descr, sql_table **ret) {
    sql_table *result;
    str msg;
    if((result = mvc_bind_table(m, s, tablename)) == NULL) {
        ...
        if(msg != MAL_SUCCEED) {
            REUSE_EXCEPTION(msg, MAL, "create_table_if_not_exists", "Could not create table '%s': %s", tablename, msg);
            return msg;
        }
        if((result = mvc_bind_table(m, s, tablename)) == NULL) {
            throw(MAL, "create_table_if_not_exists", "Could not create table '%s'", tablename);
        }
    }
    if(ret) *ret = result;
    return MAL_SUCCEED;
...
/**
 * Function returns the highest file id that currently exists in the bam.files table and stores this number + 1 in the next_file_id
 * variable. Function doesn't use a mutex to guarantee that the returned value is up to date at return time. If this is required,
...
next_file_id(mvc *m, sql_table *files, lng *next_file_id)
{
    ...
    if((c = mvc_bind_column(m, files, "file_id")) == NULL) {
        throw(MAL, "next_file_id", "Could not retrieve the next file id: Error binding file_id column of 'files' table");
    }
        
    /* Loop through all BATs for this column, i.e. the real BAT and the delta BATs and find the maximum file_id */
    for(i=0; i<3; ++i) {
...
create_alignment_storage_0(Client cntxt, str descr, bam_wrapper *bw) {
    ...
    str msg;
    snprintf(sql_create_storage, BUF_SIZE_CREATE_STORAGE_0, SQL_CREATE_STORAGE_0, 
        bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id);
    RUN_SQL(cntxt, &sql_create_storage, descr, msg);
    if(msg != MAL_SUCCEED) {
        REUSE_EXCEPTION(msg, MAL, "create_alignment_storage_0", 
                         "Could not create alignment storage for file '%s' (file id '"LLFMT"'): %s",
...
create_alignment_storage_1(Client cntxt, str descr, bam_wrapper *bw) {
    ...
    str msg;
    snprintf(sql_create_storage, BUF_SIZE_CREATE_STORAGE_1, SQL_CREATE_STORAGE_1,
        bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id,
        bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id,
        bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id, bw->file_id);
    RUN_SQL(cntxt, &sql_create_storage, descr, msg);
...
/* Macro only used by copy_into_db to increase len by step only if it is positive and throw an exception otherwise */
...
/**
 * Copies the binary files, of which the paths are stored in bam_wrapper, into the database using the Client context.
 */
str
copy_into_db(Client cntxt, bam_wrapper *bw)
{
    ...
    step = snprintf(sql_copy_into, BUF_SIZE_COPY_INTO, SQL_COPY_INTO_FILES, 
        bw->fp_files[0], bw->fp_files[1], bw->fp_files[2], bw->fp_files[3], bw->fp_files[4], bw->fp_files[5]);
    CHECK_STEP(len, step, "files");
    
    if(bw->cnt_sq > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_SQ,
            bw->fp_sq[0], bw->fp_sq[1], bw->fp_sq[2], bw->fp_sq[3], bw->fp_sq[4], bw->fp_sq[5], bw->fp_sq[6]);
        CHECK_STEP(len, step, "sq");
    }
    
    if(bw->cnt_rg > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_RG,
            bw->fp_rg[0], bw->fp_rg[1], bw->fp_rg[2], bw->fp_rg[3] , bw->fp_rg[4] , bw->fp_rg[5], bw->fp_rg[6], 
            bw->fp_rg[7], bw->fp_rg[8], bw->fp_rg[9], bw->fp_rg[10], bw->fp_rg[11], bw->fp_rg[12]);
        CHECK_STEP(len, step, "rg");
    }
    
    if(bw->cnt_pg > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_PG,
            bw->fp_pg[0], bw->fp_pg[1], bw->fp_pg[2], bw->fp_pg[3], bw->fp_pg[4], bw->fp_pg[5]);
    ...
    if(bw->cnt_alignments_paired_primary > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_PAIRED_ALIGNMENTS, "primary", bw->file_id,
            bw->fp_alignments_paired_primary[0], bw->fp_alignments_paired_primary[1], bw->fp_alignments_paired_primary[2], 
            bw->fp_alignments_paired_primary[3], bw->fp_alignments_paired_primary[4], bw->fp_alignments_paired_primary[5], 
            bw->fp_alignments_paired_primary[6], bw->fp_alignments_paired_primary[7], bw->fp_alignments_paired_primary[8], 
            bw->fp_alignments_paired_primary[9], bw->fp_alignments_paired_primary[10], bw->fp_alignments_paired_primary[11],
            bw->fp_alignments_paired_primary[12], bw->fp_alignments_paired_primary[13], bw->fp_alignments_paired_primary[14], 
            bw->fp_alignments_paired_primary[15], bw->fp_alignments_paired_primary[16], bw->fp_alignments_paired_primary[17], 
            bw->fp_alignments_paired_primary[18], bw->fp_alignments_paired_primary[19], bw->fp_alignments_paired_primary[20],
            bw->fp_alignments_paired_primary[21], bw->fp_alignments_paired_primary[22]);
        CHECK_STEP(len, step, "alignments_paired_primary");
    }
    
    if(bw->cnt_alignments_paired_secondary > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_PAIRED_ALIGNMENTS, "secondary", bw->file_id,
            bw->fp_alignments_paired_secondary[0], bw->fp_alignments_paired_secondary[1], bw->fp_alignments_paired_secondary[2], 
            bw->fp_alignments_paired_secondary[3], bw->fp_alignments_paired_secondary[4], bw->fp_alignments_paired_secondary[5], 
            bw->fp_alignments_paired_secondary[6], bw->fp_alignments_paired_secondary[7], bw->fp_alignments_paired_secondary[8], 
            bw->fp_alignments_paired_secondary[9], bw->fp_alignments_paired_secondary[10], bw->fp_alignments_paired_secondary[11],
            bw->fp_alignments_paired_secondary[12], bw->fp_alignments_paired_secondary[13], bw->fp_alignments_paired_secondary[14], 
            bw->fp_alignments_paired_secondary[15], bw->fp_alignments_paired_secondary[16], bw->fp_alignments_paired_secondary[17], 
            bw->fp_alignments_paired_secondary[18], bw->fp_alignments_paired_secondary[19], bw->fp_alignments_paired_secondary[20],
            bw->fp_alignments_paired_secondary[21], bw->fp_alignments_paired_secondary[22]);
        CHECK_STEP(len, step, "alignments_paired_secondary");
    }
    
    if(bw->cnt_alignments_extra > 0) {
        step = snprintf(sql_copy_into + len, BUF_SIZE_COPY_INTO - len, SQL_COPY_INTO_ALIGNMENTS_EXTRA, bw->file_id,
            bw->fp_alignments_extra[0], bw->fp_alignments_extra[1], bw->fp_alignments_extra[2], bw->fp_alignments_extra[3]);
    ...
    if(msg != MAL_SUCCEED) {
        REUSE_EXCEPTION(msg, MAL, "copy_into_db", 
                         "Could not copy data from binary files into the database for file '%s' (file id '"LLFMT"'): '%s'",
                         bw->file_location, bw->file_id, msg);
    }
    return msg;
}
...
drop_file(Client cntxt, str descr, lng file_id, sht dbschema) {
    ...
    /* Below is a first effort to automatically retrieve the dbschema for the given file_id, however due to limited time, this is not done yet.
    ...
    */
    ...
    if(dbschema == 0) {
        len = snprintf(sql_drop_file, BUF_SIZE_DROP_FILE, SQL_DROP_STORAGE_0, file_id, file_id);
    } else {
        len = snprintf(sql_drop_file, BUF_SIZE_DROP_FILE, SQL_DROP_STORAGE_1, file_id, file_id, file_id, file_id, file_id, file_id, file_id);
    }
    if(len < -1) {
        throw(MAL, "drop_file", "Could not construct SQL string for dropping file");
    }
    snprintf(sql_drop_file+len, BUF_SIZE_DROP_FILE-len, SQL_DROP_HEADER, file_id, file_id, file_id, file_id);
