...
sql_update_dec2023(Client c, mvc *sql, sql_schema *s)
{
	...
	if (info == NULL) {
		...
		"  cast(sys.ifthenelse(c.\"type\" IN ('varchar','clob','char','json','url','xml') AND c.\"type_digits\" > 0, sys.sql_min(4 * c.\"type_digits\", 2147483647), NULL) AS int) AS CHARACTER_OCTET_LENGTH,\n"
		...
		"  cast(sys.ifthenelse(a.\"type\" IN ('varchar','clob','char','json','url','xml') AND a.\"type_digits\" > 0, sys.sql_min(4 * a.\"type_digits\", 2147483647), NULL) AS int) AS CHARACTER_OCTET_LENGTH,\n"
		"  cast(NULL AS varchar(1)) AS CHARACTER_SET_CATALOG,\n"
		"  cast(NULL AS varchar(1)) AS CHARACTER_SET_SCHEMA,\n"
