...
sql_update_storagemodel(Client c, mvc *sql, const char *prev_schema)
{
	...
	pos += snprintf(buf + pos, bufsize - pos,
		...
		"	sum(compressed) as compressedsize\n"
		" from sys.\"storage\"\n"
		...
		"	case when (isacolumn and not sorted) then cast(8 * \"count\" as bigint) else 0 end as compressedxsize,\n"
		...
		"	sum(case when (isacolumn and not sorted) then cast(8 * \"count\" as bigint) else 0 end) as compressedsize\n"
		" from sys.storagemodelinput\n"
		"group by \"schema\", \"table\"\n"
		"order by \"schema\", \"table\";\n"
	);
