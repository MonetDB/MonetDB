...
find_func( mvc *sql, char *name, list *exps )
{
	...
	return sql_bind_func_(sql, "sys", name, l, F_FUNC, false, true, false);
}
...
rel_avg_rewrite(visitor *v, sql_rel *rel)
{
	...
	if (is_groupby(rel->op) && rel->r /*&& rel->parallel*/) {
		...
		for (m = avgs->h; m; m = m->next) {
			...
			/* create nsum/cnt exp */
			if (!cnt) {
				sql_subfunc *cf = sql_bind_func_(sql, "sys", "count", append(sa_list(sql->sa), avg_input_t), F_AGGR, false, true, false);
			...
			if (!sum) {
				sql_subfunc *sf = sql_bind_func_(sql, "sys", "sum", append(sa_list(sql->sa), avg_input_t), F_AGGR, false, true, false);
