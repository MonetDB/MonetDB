...
getErrMsg(SQLHANDLE stmt) {
	...
	ret = SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, state, &errnr, msg, (sizeof(msg) -1), &msglen);
...
odbc_query(int caller, mvc *sql, sql_subfunc *f, char *url, list *res_exps, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	{
		char name[1024];
		ret = SQLGetInfo(dbc, SQL_DBMS_NAME, (SQLPOINTER) &name, 1023, NULL);
		if (trace_enabled)
...
//						bufferLength = largestStringSize;
				...
				if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
					if (trace_enabled) {
						char * ODBCmsg = getErrMsg(stmt);
						printf("Failed to get C_type %d data for col %u of row %lu. ODBCmsg: %s\n",
							targetType, col+1, row, (ODBCmsg != NULL) ? ODBCmsg : "");
						if (ODBCmsg)
							GDKfree(ODBCmsg);
					}
