...
#define LOAD_NCDF_VAR(tpe,ncdftpe) \
	...
	if ( (retval = nc_get_var_##ncdftpe(ncid, varid, databuf)) ) \
		return createException(MAL, "netcdf.importvar", \
						   "SQLSTATE ----- !""Cannot read variable %d values: %s", \
...
NCDFtest(int *vars, str *fname)
{
	...
	if ((retval = nc_open(*fname, NC_NOWRITE, &ncid)))
	    return createException(MAL, "netcdf.test", "SQLSTATE ----- !""Cannot open NetCDF file %s: %s", *fname, nc_strerror(retval));

    if ((retval = nc_inq(ncid, &dims, vars, &ngatts, &unlimdim)))
	    return createException(MAL, "netcdf.test", "SQLSTATE ----- !""Cannot read NetCDF header: %s", nc_strerror(retval));

    if ((retval = nc_close(ncid)))
	    return createException(MAL, "netcdf.test", "SQLSTATE ----- !""Cannot close file %s: \
...
NCDFattach(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if ( !sch )
        return createException(MAL, "netcdf.attach", "SQLSTATE ----- !""Cannot get schema sys\n");
	...
	if (tfiles == NULL || tdims == NULL || tvars == NULL || 
        ...
        return createException(MAL, "netcdf.attach", "SQLSTATE ----- !""Catalog table missing\n");

	/* check if the file is already attached */
	col = mvc_bind_column(m, tfiles, "location");
	rid = table_funcs.column_find_row(m->session->tr, col, fname, NULL);
	if (rid != oid_nil) 
	    return createException(SQL, "netcdf.attach", "SQLSTATE ----- !""File %s is already attached\n", fname);

	/* Open NetCDF file  */
	if ((retval = nc_open(fname, NC_NOWRITE, &ncid)))
        return createException(MAL, "netcdf.test", "SQLSTATE ----- !""Cannot open NetCDF \
file %s: %s", fname, nc_strerror(retval));

	if ((retval = nc_inq(ncid, &ndims, &nvars, &ngatts, &unlimdim)))
        return createException(MAL, "netcdf.test", "SQLSTATE ----- !""Cannot read NetCDF \
header: %s", nc_strerror(retval));
	...
	for (didx = 0; didx < ndims; didx++){
		if ((retval = nc_inq_dim(ncid, didx, dname, &dlen)) != 0)
	        return createException(MAL, "netcdf.attach", "SQLSTATE ----- !""Cannot read dimension %d : %s", didx, nc_strerror(retval));
	...
	for (vidx = 0; vidx < nvars; vidx++){
	    if ( (retval = nc_inq_var(ncid, vidx, vname, &vtype, &vndims, vdims, &vnatts)))
	        return createException(MAL, "netcdf.attach", 
								   "SQLSTATE ----- !""Cannot read variable %d : %s",
    	...
    	if ( vnatts > 0 ) { /* fill in netcdf_attrs table */
            ...
            for (aidx = 0; aidx < vnatts; aidx++){
                if ((retval = nc_inq_attname(ncid,vidx,aidx,aname)))
                    return createException(MAL, "netcdf.attach",
										   "SQLSTATE ----- !""Cannot read attribute %d of variable %d: %s",
										   aidx, vidx, nc_strerror(retval));

				if ((retval = nc_inq_att(ncid,vidx,aname,&atype,&alen)))
                    return createException(MAL, "netcdf.attach",
										   "SQLSTATE ----- !""Cannot read attribute %s type and length: %s",
				...
				case NC_CHAR:
					...
					if ((retval = nc_get_att_text(ncid,vidx,aname,aval)))
						return createException(MAL, "netcdf.attach",
											   "SQLSTATE ----- !""Cannot read attribute %s value: %s",
											   aname, nc_strerror(retval));
					fix_quote(aval, alen);
					aval[alen] = '\0';
	...
	for (aidx = 0; aidx < ngatts; aidx++){
		if ((retval = nc_inq_attname(ncid,NC_GLOBAL,aidx,aname)) != 0)
	        return createException(MAL, "netcdf.attach",
								   "SQLSTATE ----- !""Cannot read global attribute %d: %s",
								   aidx, nc_strerror(retval));

		if ((retval = nc_inq_att(ncid,NC_GLOBAL,aname,&atype,&alen)) != 0)
	        return createException(MAL, "netcdf.attach",
								   "SQLSTATE ----- !""Cannot read global attribute %s type and length: %s",
								   aname, nc_strerror(retval));
		...
		case NC_CHAR:
			...
			if ((retval = nc_get_att_text(ncid,NC_GLOBAL,aname,aval)))
				return createException(MAL, "netcdf.attach",
									   "SQLSTATE ----- !""Cannot read global attribute %s value: %s",
									   aname, nc_strerror(retval));
		    fix_quote(aval, alen);
			aval[alen] = '\0';
		...
		case NC_INT:
			if ((retval = nc_get_att_int(ncid,NC_GLOBAL,aname,&avalint)))
				return createException(MAL, "netcdf.attach",
									   "SQLSTATE ----- !""Cannot read global attribute %s of type %s : %s",
...
NCDFimportVarStmt(str *sciqlstmt, str *fname, int *varid)
{
	...
	if ((retval = nc_open(*fname, NC_NOWRITE, &ncid)))
        return createException(MAL, "netcdf.importvar",
            "SQLSTATE ----- !""Cannot open NetCDF file %s: %s", *fname, nc_strerror(retval));

	if ( (retval = nc_inq_var(ncid, *varid, vname, &vtype, &vndims, vdims, &vnatts)))
	    return createException(MAL, "netcdf.attach",
		    "SQLSTATE ----- !""Cannot read variable %d : %s", *varid, nc_strerror(retval));
	...
	for (i = 0; i < vndims; i++){
	    if ((retval = nc_inq_dim(ncid, vdims[i], dname, &dlen)))
	        return createException(MAL, "netcdf.attach",
			    "SQLSTATE ----- !""Cannot read dimension %d : %s", vdims[i], nc_strerror(retval));
...
NCDFloadVar(bat **dim, bat *v, int ncid, int varid, nc_type vtype, int vndims, int *vdims)
{
	...
	if ( dim == NULL )
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""array of dimension bat is NULL");
	dim_bids = *dim;
	...
	for (i = 0; i < vndims; i++){
		if ((retval = nc_inq_dimlen(ncid, vdims[i], &dlen[i])))
			return createException(MAL, "netcdf.importvar",
								   "SQLSTATE ----- !""Cannot read dimension %d : %s",
	...
	default:
	  return createException(MAL, "netcdf.importvar",
			   "SQLSTATE ----- !""Type %s not supported yet",
	...
	for (i = 0; i < vndims; i++) {
		...
		if (sermsg != MAL_SUCCEED) {
			...
			GDKfree(val_rep);
			GDKfree(grp_rep);
			return createException(MAL, "netcdf.loadvar", "SQLSTATE ----- !""Failed to create a dimension of variable %d", varid);
		}
	}
...
NCDFimportVariable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	sch = mvc_bind_schema(m, "sys");
	if ( !sch )
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Cannot get schema sys\n");

	tfiles = mvc_bind_table(m, sch, "netcdf_files");
	if (tfiles == NULL)
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Catalog table missing\n");

	/* get the name of the attached NetCDF file */
	col = mvc_bind_column(m, tfiles, "file_id");
	if (col == NULL)
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Could not find \"netcdf_files\".\"file_id\"\n");
	rid = table_funcs.column_find_row(m->session->tr, col, (void *)&fid, NULL);
	if (rid == oid_nil)
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""File %d not in the NetCDF vault\n", fid);
	...
	/* Open NetCDF file  */
	if ((retval = nc_open(fname, NC_NOWRITE, &ncid))) {
		char *msg = createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Cannot open NetCDF file %s: %s", 
	...
	if ( (retval = nc_inq_varid(ncid, vname, &varid)) )
		return createException(MAL, "netcdf.importvar",
			   "SQLSTATE ----- !""Cannot read variable %s: %s",
...
/* load variable data */
	...
	if (arr_table == NULL)
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""netcdf table %s missing\n", aname_sys);

	col = mvc_bind_column(m, arr_table, "value");
	if (col == NULL)
		return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Cannot find column %s.value\n", aname_sys);
	...
	for (i = 0; i < vndims; i++){
		...
		if (col == NULL)
			return createException(MAL, "netcdf.importvar", "SQLSTATE ----- !""Cannot find column %s.%s\n", aname_sys, dname[i]);
