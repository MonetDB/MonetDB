...
//	int* res_id = *getArgReference_str(stk, pci, 1);
	...
	if (tbl == NULL) {
		msg = createException (MAL, "lidar.exporttable", "SQLSTATE ----- !""Table %s is missing.\n", tname);
		return msg;
	}
	...
	if (cols[0] == NULL || cols[1] == NULL || cols[2] == NULL) {
		msg = createException(MAL, "lidar.exporttable", "SQLSTATE ----- !""Could not locate a column with name 'x', 'y', or 'z'.");
		return msg;
	} 
...
return MAL_SUCCEED;
	...
	if (globbuf.gl_pathc == 0)
		throw(MAL, "listdir", "SQLSTATE ----- !""Couldn't open the directory or there are no files that match the pattern");
...
LIDARtest(int *res, str *fname)
{
	...
	if (LASError_GetErrorCount() != 0) {
		msg = createException(MAL, "lidar.test", "SQLSTATE ----- !""Error accessing LIDAR file %s (%s)", 
...
str LIDARattach(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (access(fname, F_OK) == -1) {
		msg = createException(MAL, "lidar.test", "SQLSTATE ----- !""File %s not found.", fname);
		return msg;
	}
	...
	if (rid != oid_nil) {
		MT_lock_set(&mt_lidar_lock);
                ...
                if (reader != NULL) LASReader_Destroy(reader);
		MT_lock_unset(&mt_lidar_lock);
		msg = createException(SQL, "lidar.attach", "SQLSTATE ----- !""File %s already attached\n", fname);
		return msg;
	}
	...
	if (rid != oid_nil || tbl) {
		MT_lock_set(&mt_lidar_lock);
                ...
                if (reader != NULL) LASReader_Destroy(reader);
		MT_lock_unset(&mt_lidar_lock);
		msg = createException(SQL, "lidar.attach", "SQLSTATE ----- !""Table %s already exists\n", tname_low);
		return msg;
	}
...
str LIDARloadTable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	lidar_tbl = mvc_bind_table(m, sch, "lidar_tables");
	if (lidar_tbl == NULL) {
		msg = createException(MAL, "lidar.loadtable", "SQLSTATE ----- !""LIDAR catalog is missing.\n");
		return msg;
	}

	tbl = mvc_bind_table(m, sch, tname);
	if (tbl == NULL) {
		msg = createException(MAL, "lidar.loadtable", "SQLSTATE ----- !""Could not find table %s.\n", tname);
		return msg;
	}
	...
	if (sz != 0) {
		msg = createException(MAL, "lidar.loadtable", "SQLSTATE ----- !""Table %s is not empty.\n", tname);
		return msg;
	}
	...
	if (rid == oid_nil) {
		msg = createException(MAL, "lidar.loadtable", "SQLSTATE ----- !""Table %s is unknown to the LIDAR catalog. Attach first the containing file\n", tname);
		return msg;
	}
	...
	if (lidar_cl == NULL) {
		msg = createException(MAL, "lidar.loadtable", "SQLSTATE ----- !""Could not find table lidar_columns.\n");
		return msg;
	}
	...
	MT_lock_unset(&mt_lidar_lock);
	if (LASError_GetErrorCount() != 0) {
		msg = createException(MAL, "lidar.lidarload", "SQLSTATE ----- !""Error accessing LIDAR file %s (%s)", 
		fname, LASError_GetLastErrorMsg());
		return msg;
	}
	...
	if ( x == NULL || y == NULL || z == NULL) {
		...
		if (reader != NULL) LASReader_Destroy(reader);
		MT_lock_unset(&mt_lidar_lock);
		msg = createException(MAL, "lidar.lidarload", "SQLSTATE ----- !"MAL_MALLOC_FAIL);
		return msg;
	}
