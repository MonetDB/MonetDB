...
static str
pqc_relation(mvc *sql, sql_subfunc *f, char *filename, list *in_exps, list *res_exps, char *tname, lng *est)
{
	(void)in_exps;
	pqc_file *pq = NULL;
...
static void *
pqc_load(void *BE, sql_subfunc *f, char *filename, list *in_list, sql_exp *topn)
{
	(void)in_list;
	backend *be = BE;
	lng nrows = -1;
	...
	sql_alias *ta = a_create(be->mvc->sa, "bla");
	for (i=0, n = f->res->h, m = f->colnames->h; n && m; i++, n = n->next, m = m->next) {
		...
		s->q = q;
		s->subtype = *st;
		s = stmt_alias(be, s, i+1, ta, column_name);
		list_append(l, s);
	}
