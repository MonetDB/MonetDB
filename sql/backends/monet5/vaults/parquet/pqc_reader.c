	...
	if (r->pse->precision == 0 && !output) {
		if (ssize) {
			*ssize = cr->dictsize;
			...
			if (cr->dictsize < 256)
				*dict = 1;
			else if (cr->dictsize < 64*1024)
				*dict = 2;
			else
				*dict = 4;
			//*dict = cr->nr_bits<=8?1:2; /* later include size of dict offsets */
			//handle case where pqc has indices <8 or < 16 bits while
			//monetdb needs large offsets as they are based on offsets over the chars not inside a (char*)table).
			//ie need to read chars, write as shorts
			//or read shorts write as ints etc
			assert(cr->dictsize);
			return 0;
		}
	}
...
static int
pqc_project( char *output, char *data, u_int64_t nrows, int w)
{
	if (w == 1) {
		int sh = 0, j = 0;
...
static int
pqc_add_nil( pqc_reader_t *r, pqc_creader_t *cr, char *output, char *data, u_int64_t nrows, u_int64_t nils, int w)
{
	int cur = cr->curdef, sdef = cr->cursubdef;
	int def = cr->first_definition;
	int pos = 0;
...
#endif
		default:
			for(u_int64_t i = 0, j = 0; i<nrows; i++, j+=w)
	...
	if (!sdef)
		def = !def;
	pos = nrows*w;
	for(u_int64_t i = nrows; i > 0 && cur >= 0; cur--){
		int len = cr->definition[cur];
		len -= sdef;
		sdef = 0;
		if (i < (u_int64_t)len)
