	   ...
	   sht dbschema, sht nr_threads)
{
	...
	if (dbschema != 0 && dbschema != 1) {
		msg = createException(MAL, "bam_loader",
					  "SQLSTATE ---- !""Wrong value for dbschema: '%d' (0=straightforward storage schema, 1=pairwise storage schema)",
					  dbschema);
		goto cleanup;
	}
	if (nr_threads <= 0) {
	...
	if ((bws =
		...
		NULL) {
		msg = createException(MAL, "bam_loader", "SQLSTATE ---- !"MAL_MALLOC_FAIL);
		goto cleanup;
	}
	...
	if (dbschema == 1) {
		for (i = 0; i < nr_files; ++i) {
			...
			if (bws[i].ord != ORDERING_QUERYNAME) {
				msg = createException(MAL, "bam_loader",
							  "SQLSTATE ---- !""Only BAM files that are sorted on queryname can be inserted into the pairwise storage schema; "
	...
	for (i = 0; i < nr_threads; ++i) {
		if ((errnr =
			 MT_create_thread(&reader_threads[i],
					  ...
					  MT_THR_JOINABLE)) != 0) {
			msg = createException(MAL, "bam_loader",
						  "SQLSTATE ---- !""Could not create thread to process alignments (errnr %d)",
						  errnr);
			goto cleanup;
		}
	}
	...
	for (i = 0; i < nr_threads; ++i) {
		if ((errnr = MT_join_thread(reader_threads[i])) != 0) {
			msg = createException(MAL, "bam_loader",
						  "SQLSTATE ---- !""Could not join alignment processing thread (errnr %d)",
						  errnr);
			goto cleanup;
		}
...
bam_loader_repos(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if ((d = opendir(bam_repos)) == NULL) {
		if (errno == ENOENT) {
			msg = createException(MAL, "bam_loader_repos",
						  "SQLSTATE ---- !""Could not find directory '%s'",
						  bam_repos);
		} else {
			msg = createException(MAL, "bam_loader_repos",
						  "SQLSTATE ---- !""Could not open directory '%s'",
						  bam_repos);
		}
		goto cleanup;
	}
	...
	if (filecount == 0) {
		msg = createException(MAL, "bam_loader_repos",
					  "SQLSTATE ---- !""No SAM or BAM files found in directory '%s'",
					  bam_repos);
		goto cleanup;
	}
		...
		if (IS_SAMORBAM(direntry->d_name, len)) {
			...
			if (snprintf
				(path, 4096, "%s/%s", bam_repos,
				 direntry->d_name) < 0) {
				msg = createException(MAL, "bam_loader_repos",
							  "SQLSTATE ---- !""Could not construct filepath for SAM/BAM file '%s'",
							  direntry->d_name);
				goto cleanup;
			}
			if (nr_files >= filecount) {
				 ...
				 * the meantime */
				msg = createException(MAL, "bam_loader_repos",
							  "SQLSTATE ---- !""An error occurred while reading directory '%s'",
							  bam_repos);
				goto cleanup;
			}
...
bam_loader_files(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (f == NULL) {
		msg = createException(MAL, "bam_loader_files",
					  "SQLSTATE ---- !""Error on opening file list '%s': %s",
	...
	if (ferror(f)) {
		msg = createException(MAL, "bam_loader_files",
					  "SQLSTATE ---- !""Error while processing file '%s'",
					  bam_files);
		goto cleanup;
	}
	...
	if (nr_files == 0) {
		msg = createException(MAL, "bam_loader_files",
					  "SQLSTATE ---- !""No valid entries found in file '%s'",
					  bam_files);
		goto cleanup;
	}
...
bam_drop_file(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (msg != MAL_SUCCEED) {
		str msg2 = createException(MAL, "bam_drop_file",
			  "SQLSTATE ---- !""Error when dropping file with file id '" LLFMT
			  "': %s\n", file_id, msg);
		freeException(msg);
		return msg2;
	}
