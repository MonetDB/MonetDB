...
sql_drop_statistics(mvc *m, sql_table *t)
{
	...
	if (!table_privs(m, t, PRIV_SELECT))
		throw(SQL, "sql_drop_statistics", SQLSTATE(42000) "DROP STATISTICS: access denied for %s to table '%s.%s'", stack_get_string(m, "current_user"), t->s->base.name, t->base.name);
		...
		for (ncol = (t)->columns.set->h; ncol; ncol = ncol->next) {
			...
			if (!column_privs(m, c, PRIV_SELECT))
				throw(SQL, "sql_drop_statistics", SQLSTATE(42000) "DROP STATISTICS: access denied for %s to column '%s' on table '%s.%s'", 
					  stack_get_string(m, "current_user"), c->base.name, t->s->base.name, t->base.name);
		}
	}
			...
			for (ntab = (s)->tables.set->h; ntab; ntab = ntab->next) {
				...
				if (!table_privs(m, t, PRIV_SELECT))
					throw(SQL, "analyze", SQLSTATE(42000) "ANALYZE: access denied for %s to table '%s.%s'", 
						  stack_get_string(m, "current_user"), t->s->base.name, t->base.name);
				if (isTable(t) && t->columns.set) {
					for (ncol = (t)->columns.set->h; ncol; ncol = ncol->next) {
						sql_column *c = (sql_column *) ncol->data;
