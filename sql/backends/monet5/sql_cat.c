...
#define initcontext() \
    ...
    if ((msg = checkSQLContext(cntxt)) != NULL)\
        return msg;\
    if (STORE_READONLY)\
        return sql_message("SQLSTATE 25006 !""Schema statements cannot be executed on a readonly database.");
...
rel_check_tables(sql_table *nt, sql_table *nnt)
{
	...
	if (cs_size(&nt->columns) != cs_size(&nnt->columns))
		return sql_message("SQLSTATE 3F000 !""ALTER MERGE TABLE: to be added table doesn't match MERGE TABLE definition");
	for (n = nt->columns.set->h, m = nnt->columns.set->h; n && m; n = n->next, m = m->next) {
		...
		if (subtype_cmp(&nc->type, &mc->type) != 0)
			return sql_message("SQLSTATE 3F000 !""ALTER MERGE TABLE: to be added table column type doesn't match MERGE TABLE definition");
	}
	if (cs_size(&nt->idxs) != cs_size(&nnt->idxs))
		return sql_message("SQLSTATE 3F000 !""ALTER MERGE TABLE: to be added table index doesn't match MERGE TABLE definition");
	if (cs_size(&nt->idxs))
		for (n = nt->idxs.set->h, m = nnt->idxs.set->h; n && m; n = n->next, m = m->next) {
			...
			if (ni->type != mi->type)
				return sql_message("SQLSTATE 3F000 !""ALTER MERGE TABLE: to be added table index type doesn't match MERGE TABLE definition");
		}
	return MAL_SUCCEED;
}
...
alter_table_add_table(mvc *sql, char *msname, char *mtname, char *psname, char *ptname)
{
	...
	if (mt && (mt->type != tt_merge_table && mt->type != tt_replica_table))
		return sql_message("SQLSTATE 42S02 !""ALTER TABLE: cannot add table '%s.%s' to table '%s.%s'", psname, ptname, msname, mtname);
	if (mt && pt) {
		char *msg;
		node *n = cs_find_id(&mt->tables, pt->base.id);

		if (n)
			return sql_message("SQLSTATE 42S02 !""ALTER TABLE: table '%s.%s' is already part of the MERGE TABLE '%s.%s'", psname, ptname, msname, mtname);
		if ((msg = rel_check_tables(mt, pt)) != NULL)
			return msg;
		sql_trans_add_table(sql->session->tr, mt, pt);
	} else if (mt) {
		return sql_message("SQLSTATE 42S02 !""ALTER TABLE: no such table '%s' in schema '%s'", ptname, psname);
	} else {
		return sql_message("SQLSTATE 42S02 !""ALTER TABLE: no such table '%s' in schema '%s'", mtname, msname);
	}
	return MAL_SUCCEED;
}
...
alter_table_del_table(mvc *sql, char *msname, char *mtname, char *psname, char *ptname, int drop_action)
{
	...
	if (mt && pt) {
		...
		if (!pt || (n = cs_find_id(&mt->tables, pt->base.id)) == NULL)
			return sql_message("SQLSTATE 42S02 !""ALTER TABLE: table '%s.%s' isn't part of the MERGE TABLE '%s.%s'", psname, ptname, msname, mtname);
...
alter_table_set_access(mvc *sql, char *sname, char *tname, int access)
{
	...
	if (t) {
		if (t->type == tt_merge_table)
			return sql_message("SQLSTATE 42S02 !""ALTER TABLE: read only MERGE TABLES are not supported");
		if (t->access != access) {
			if (access && table_has_updates(sql->session->tr, t))
				return sql_message("SQLSTATE 40000 !""ALTER TABLE: set READ or INSERT ONLY not possible with outstanding updates (wait until updates are flushed)\n");

			mvc_access(sql, t, access);
		}
	} else {
		return sql_message("SQLSTATE 42S02 !""ALTER TABLE: no such table '%s' in schema '%s'", tname, sname);
	}
	return MAL_SUCCEED;
}
...
create_trigger(mvc *sql, char *sname, char *tname, char *triggername, int time, int orientation, int event, char *old_name, char *new_name, char *condition, char *query)
{
	...
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""CREATE TRIGGER: no such schema '%s'", sname);
	if (!s)
		s = cur_schema(sql);
	if (!mvc_schema_privs(sql, s))
		return sql_message("SQLSTATE 3F000 !""CREATE TRIGGER: access denied for %s to schema ;'%s'", stack_get_string(sql, "current_user"), s->base.name);
	if (mvc_bind_trigger(sql, s, triggername) != NULL)
		return sql_message("SQLSTATE 3F000 !""CREATE TRIGGER: name '%s' already in use", triggername);

	if (!(t = mvc_bind_table(sql, s, tname)))
		return sql_message("SQLSTATE 3F000 !""CREATE TRIGGER: unknown table '%s'", tname);

	if (isView(t))
		return sql_message("SQLSTATE 3F000 !""CREATE TRIGGER: cannot create trigger on view '%s'", tname);
...
drop_trigger(mvc *sql, char *sname, char *tname)
{
	...
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""DROP TRIGGER: no such schema '%s'", sname);
	if (!s)
		s = cur_schema(sql);
	assert(s);
	if (!mvc_schema_privs(sql, s))
		return sql_message("SQLSTATE 3F000 !""DROP TRIGGER: access denied for %s to schema ;'%s'", stack_get_string(sql, "current_user"), s->base.name);

	if ((tri = mvc_bind_trigger(sql, s, tname)) == NULL)
		return sql_message("SQLSTATE 3F000 !""DROP TRIGGER: unknown trigger %s\n", tname);
...
drop_table(mvc *sql, char *sname, char *tname, int drop_action, int if_exists)
{
	...
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""DROP TABLE: no such schema '%s'", sname);
	if (!s)
		s = cur_schema(sql);
	t = mvc_bind_table(sql, s, tname);
	if (!t && !sname) {
	...
	if (!t) {
		if (if_exists)
			return MAL_SUCCEED;
		return sql_message("SQLSTATE 42S02 !""DROP TABLE: no such table '%s'", tname);
	} else if (isView(t)) {
		return sql_message("SQLSTATE 42000 !""DROP TABLE: cannot drop VIEW '%s'", tname);
	} else if (t->system) {
		return sql_message("SQLSTATE 42000 !""DROP TABLE: cannot drop system table '%s'", tname);
	} else if (!mvc_schema_privs(sql, s) && !(isTempSchema(s) && t->persistence == SQL_LOCAL_TEMP)) {
		return sql_message("SQLSTATE 42000 !""DROP TABLE: access denied for %s to schema ;'%s'", stack_get_string(sql, "current_user"), s->base.name);
	}
			...
			if (k->type == ukey || k->type == pkey) {
				...
				if (uk->keys && list_length(uk->keys)) {
					...
					for (; l; l = l->next) {
						...
						if (k->t != t)
							return sql_message("SQLSTATE 40000 !""DROP TABLE: FOREIGN KEY %s.%s depends on %s", k->t->base.name, k->base.name, tname);
					}
				}
			}
		}
	...
	if (!drop_action && mvc_check_dependency(sql, t->base.id, TABLE_DEPENDENCY, NULL))
		return sql_message("SQLSTATE 42000 !""DROP TABLE: unable to drop table %s (there are database objects which depend on it)\n", t->base.name);
...
drop_view(mvc *sql, char *sname, char *tname, int drop_action)
{
	...
	if (sname != NULL && (ss = mvc_bind_schema(sql, sname)) == NULL)
		return sql_message("SQLSTATE 3F000 !""DROP VIEW: no such schema '%s'", sname);
	...
	if (!mvc_schema_privs(sql, ss) && !(isTempSchema(ss) && t && t->persistence == SQL_LOCAL_TEMP)) {
		return sql_message("SQLSTATE 42000 !""DROP VIEW: access denied for %s to schema '%s'", stack_get_string(sql, "current_user"), ss->base.name);
	} else if (!t) {
		return sql_message("SQLSTATE 42S02 !""DROP VIEW: unknown view '%s'", tname);
	} else if (!isView(t)) {
		return sql_message("SQLSTATE 42000 !""DROP VIEW: unable to drop view '%s': is a table", tname);
	} else if (t->system) {
		return sql_message("SQLSTATE 42000 !""DROP VIEW: cannot drop system view '%s'", tname);
	} else if (!drop_action && mvc_check_dependency(sql, t->base.id, VIEW_DEPENDENCY, NULL)) {
		return sql_message("SQLSTATE 42000 !""DROP VIEW: cannot drop view '%s', there are database objects which depend on it", t->base.name);
	} else {
...
drop_key(mvc *sql, char *sname, char *kname, int drop_action)
{
	...
	if (sname != NULL && (ss = mvc_bind_schema(sql, sname)) == NULL)
		return sql_message("SQLSTATE 3F000 !""ALTER TABLE: no such schema '%s'", sname);

	if (ss == NULL)
		ss = cur_schema(sql);

	if ((key = mvc_bind_key(sql, ss, kname)) == NULL)
		return sql_message("SQLSTATE 42000 !""ALTER TABLE: no such constraint '%s'", kname);
	if (!drop_action && mvc_check_dependency(sql, key->base.id, KEY_DEPENDENCY, NULL))
		return sql_message("SQLSTATE 42000 !""ALTER TABLE: cannot drop constraint '%s': there are database objects which depend on it", key->base.name);
...
drop_index(Client cntxt, mvc *sql, char *sname, char *iname)
{
	...
	if (!(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""DROP INDEX: no such schema '%s'", sname);
	i = mvc_bind_idx(sql, s, iname);
	if (!i) {
		return sql_message("SQLSTATE 42S12 !""DROP INDEX: no such index '%s'", iname);
	} else if (!mvc_schema_privs(sql, s)) {
		return sql_message("SQLSTATE 42000 !""DROP INDEX: access denied for %s to schema ;'%s'", stack_get_string(sql, "current_user"), s->base.name);
	} else {
...
create_seq(mvc *sql, char *sname, char *seqname, sql_sequence *seq)
{
	...
	(void)seqname;
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""CREATE SEQUENCE: no such schema '%s'", sname);
	if (s == NULL)
		s = cur_schema(sql);
	if (find_sql_sequence(s, seq->base.name)) {
		return sql_message("SQLSTATE 42000 !""CREATE SEQUENCE: name '%s' already in use", seq->base.name);
	} else if (!mvc_schema_privs(sql, s)) {
		return sql_message("SQLSTATE 42000 !""CREATE SEQUENCE: insufficient privileges for '%s' in schema '%s'", stack_get_string(sql, "current_user"), s->base.name);
	}
...
alter_seq(mvc *sql, char *sname, char *seqname, sql_sequence *seq, lng *val)
{
	...
	(void)seqname;
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""ALTER SEQUENCE: no such schema '%s'", sname);
	if (s == NULL)
		s = cur_schema(sql);
	if (!(nseq = find_sql_sequence(s, seq->base.name))) {
		return sql_message("SQLSTATE 42000 !""ALTER SEQUENCE: no such sequence '%s'", seq->base.name);
	} else if (!mvc_schema_privs(sql, s)) {
		return sql_message("SQLSTATE 42000 !""ALTER SEQUENCE: insufficient privileges for '%s' in schema '%s'", stack_get_string(sql, "current_user"), s->base.name);
	}
...
drop_seq(mvc *sql, char *sname, char *name)
{
	...
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""DROP SEQUENCE: no such schema '%s'", sname);
	if (!s)
		s = cur_schema(sql);
	if (!(seq = find_sql_sequence(s, name))) {
		return sql_message("SQLSTATE 42M35 !""DROP SEQUENCE: no such sequence '%s'", name);
	} else if (!mvc_schema_privs(sql, s)) {
		return sql_message("SQLSTATE 42000 !""DROP SEQUENCE: insufficient privileges for '%s' in schema '%s'", stack_get_string(sql, "current_user"), s->base.name);
	}
	if (mvc_check_dependency(sql, seq->base.id, BEDROPPED_DEPENDENCY, NULL))
		return sql_message("SQLSTATE 2B000 !""DROP SEQUENCE: unable to drop sequence %s (there are database objects which depend on it)\n", seq->base.name);
...
drop_func(mvc *sql, char *sname, char *name, int fid, int type, int action)
{
	...
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""DROP %s%s: no such schema '%s'", KF, F, sname);
	if (!s)
		s = cur_schema(sql);
	if (fid >= 0) {
...
create_func(mvc *sql, char *sname, char *fname, sql_func *f)
{
	...
	(void)fname;
	if (sname && !(s = mvc_bind_schema(sql, sname)))
		return sql_message("SQLSTATE 3F000 !""CREATE %s%s: no such schema '%s'", KF, F, sname);
	if (!s)
		s = cur_schema(sql);
	...
	if (nf && nf->query && nf->lang <= FUNC_LANG_SQL) {
		...
		if (!backend_resolve_function(sql, nf))
			return sql_message("SQLSTATE 3F000 !""CREATE %s%s: external name %s.%s not bound", KF, F, nf->mod, nf->base.name);
	}
	return MAL_SUCCEED;
}
...
alter_table(Client cntxt, mvc *sql, char *sname, sql_table *t)
{
	...
	if ((nt = mvc_bind_table(sql, s, t->base.name)) == NULL) {
		return sql_message("SQLSTATE 42S02 !""ALTER TABLE: no such table '%s'", t->base.name);
	} else if (!mvc_schema_privs(sql, s) && !(isTempSchema(s) && t->persistence == SQL_LOCAL_TEMP)) {
		return sql_message("SQLSTATE 42000 !""ALTER TABLE: insufficient privileges for user '%s' in schema '%s'", stack_get_string(sql, "current_user"), s->base.name);
	}
		...
		if (nt->pkey) {
			for (n = t->idxs.nelm; n; n = n->next) {
				...
				if (i->key && i->key->type == pkey)
					return sql_message("SQLSTATE 40000 !""CONSTRAINT PRIMARY KEY: a table can have only one PRIMARY KEY\n");
			}
		}
	}
	...
	for (n = t->columns.set->h; n != t->columns.nelm; n = n->next) {
		...
		if (c->null != nc->null && isTable(nt)) {
			if (c->null && nt->pkey) { /* check for primary keys based on this column */
				...
				for(m = nt->pkey->k.columns->h; m; m = m->next) {
					...
					if (kc->c->base.id == c->base.id)
						return sql_message("SQLSTATE 40000 !""NOT NULL CONSTRAINT: cannot change NOT NULL CONSTRAINT for column '%s' as its part of the PRIMARY KEY\n", c->base.name);
				}
			}
			...
			if (c->null == 0) {
				...
				if (has_nils)
					return sql_message("SQLSTATE 40002 !""ALTER TABLE: NOT NULL constraint violated for column %s.%s", c->t->base.name, c->base.name);
			}
		}
		if (c->def != nc->def)
		...
		if (c->storage_type != nc->storage_type) {
			if (c->t->access == TABLE_WRITABLE)
				return sql_message("SQLSTATE 40002 !""ALTER TABLE: SET STORAGE for column %s.%s only allowed on READ or INSERT ONLY tables", c->t->base.name, c->base.name);
	...
	if (t->idxs.set) {
		...
		for (n = t->idxs.nelm; n; n = n->next) {
			...
			if (i->type == ordered_idx) {
				...
				BBPunfix(b->batCacheid);
				if (msg != MAL_SUCCEED) {
					char *smsg = sql_message("SQLSTATE 40002 !""CREATE ORDERED INDEX: %s", msg);
					freeException(msg);
					return smsg;
				}
			}
...
{	mvc *sql = NULL;
	...
	if (val == NULL || *val == lng_nil)
		msg = sql_message("SQLSTATE 42M36 !""ALTER SEQUENCE: cannot (re)start with NULL");
	else
...
{	mvc *sql = NULL;
	...
	if (name && (auth_id = sql_find_auth(sql, name)) < 0) {
		msg = sql_message("SQLSTATE 42M32 !""CREATE SCHEMA: no such authorization '%s'", name);
	}
	if (sql->user_id != USER_MONETDB && sql->role_id != ROLE_SYSADMIN) {
		msg = sql_message("SQLSTATE 42000 !""CREATE SCHEMA: insufficient privileges for user '%s'", stack_get_string(sql, "current_user"));
	}
	if (mvc_bind_schema(sql, sname)) {
		msg = sql_message("SQLSTATE 3F000 !""CREATE SCHEMA: name '%s' already in use", sname);
	} else {
...
{	mvc *sql = NULL;
	...
	if (!s) {
		if(!if_exists)
			msg = sql_message("SQLSTATE 3F000 !""DROP SCHEMA: name %s does not exist", sname);
	} else if (!mvc_schema_privs(sql, s)) {
		msg = sql_message("SQLSTATE 42000 !""DROP SCHEMA: access denied for %s to schema ;'%s'", stack_get_string(sql, "current_user"), s->base.name);
	} else if (s == cur_schema(sql)) {
		msg = sql_message("SQLSTATE 42000 !""DROP SCHEMA: cannot drop current schema");
	} else if (strcmp(sname, "sys") == 0 || strcmp(sname, "tmp") == 0) {
		msg = sql_message("SQLSTATE 42000 !""DROP SCHEMA: access denied for '%s'", sname);
	} else if (sql_schema_has_user(sql, s)) {
		msg = sql_message("SQLSTATE 2BM37 !""DROP SCHEMA: unable to drop schema '%s' (there are database objects which depend on it", sname);
	} else {
...
{	mvc *sql = NULL;
	...
	if (!mvc_schema_privs(sql, sql->session->schema))
		msg = sql_message("SQLSTATE 0D000 !""CREATE TYPE: not enough privileges to create type '%s'", sname);
	if (!mvc_create_type(sql, s, name, 0, 0, 0, impl))
		msg = sql_message("SQLSTATE 0D000 !""CREATE TYPE: unknown external type '%s'", impl);
	return msg;
}
...
{	mvc *sql = NULL;
	...
	if (!t)
		msg = sql_message("SQLSTATE 0D000 !""DROP TYPE: type '%s' does not exist", sname);
	else if (!mvc_schema_privs(sql, sql->session->schema))
		msg = sql_message("SQLSTATE 0D000 !""DROP TYPE: not enough privileges to drop type '%s'", sname);
	else if (!drop_action && mvc_check_dependency(sql, t->base.id, TYPE_DEPENDENCY, NULL))
		return sql_message("SQLSTATE 42000 !""DROP TYPE: unable to drop type %s (there are database objects which depend on it)\n", sname);
	else if (!mvc_drop_type(sql, sql->session->schema, t, drop_action))
		msg = sql_message("SQLSTATE 0D000 !""DROP TYPE: failed to drop type '%s'", sname);
	return msg;
}
