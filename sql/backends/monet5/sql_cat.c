							   ...
							   sql_table **mt, sql_table **pt, int update)
{
	...
	if (!mvc_schema_privs(sql, ms))
		throw(SQL,call,SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ms->base.name);
	if (!mvc_schema_privs(sql, ps))
		throw(SQL,call,SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ps->base.name);
...
alter_table_del_table(mvc *sql, char *msname, char *mtname, char *psname, char *ptname, int drop_action)
{
	...
	if (!mvc_schema_privs(sql, ms))
		throw(SQL,"sql.alter_table_del_table",SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ms->base.name);
	if (!mvc_schema_privs(sql, ps))
		throw(SQL,"sql.alter_table_del_table",SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ps->base.name);
...
alter_table_set_access(mvc *sql, char *sname, char *tname, int access)
{
	...
	if (s && !mvc_schema_privs(sql, s))
		throw(SQL,"sql.alter_table_set_access",SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
...
create_trigger(mvc *sql, char *sname, char *tname, char *triggername, int time, int orientation, int event, char *old_name, char *new_name, char *condition, char *query)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL,"sql.create_trigger",SQLSTATE(42000) "CREATE TRIGGER: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
...
drop_trigger(mvc *sql, char *sname, char *tname, int if_exists)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL,"sql.drop_trigger",SQLSTATE(42000) "DROP TRIGGER: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
...
drop_table(mvc *sql, char *sname, char *tname, int drop_action, int if_exists)
{
	...
	if (!mvc_schema_privs(sql, s) && !(isTempSchema(s) && t->persistence == SQL_LOCAL_TEMP))
		throw(SQL,"sql.drop_table", SQLSTATE(42000) "DROP TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
...
drop_view(mvc *sql, char *sname, char *tname, int drop_action, int if_exists)
{
	...
	if (!mvc_schema_privs(sql, ss) && !(isTempSchema(ss) && t && t->persistence == SQL_LOCAL_TEMP))
		throw(SQL,"sql.drop_view", SQLSTATE(42000) "DROP VIEW: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ss->base.name);
	if (!isView(t))
...
drop_key(mvc *sql, char *sname, char *kname, int drop_action)
{
	...
	if (!mvc_schema_privs(sql, ss))
		throw(SQL,"sql.drop_key", SQLSTATE(42000) "ALTER TABLE: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), ss->base.name);
...
drop_func(mvc *sql, char *sname, char *name, sqlid fid, sql_ftype type, int action)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL,"sql.drop_func", SQLSTATE(42000) "DROP %s: access denied for %s to schema '%s'", F, stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
	if (fid >= 0) {
...
create_func(mvc *sql, char *sname, char *fname, sql_func *f)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL,"sql.create_func", SQLSTATE(42000) "CREATE %s: access denied for %s to schema '%s'", F, stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
...
SQLrename_schema(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL, "sql.rename_schema", SQLSTATE(42000) "ALTER SCHEMA: access denied for %s to schema '%s'", stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), old_name);
	if (s->system)
