...
create_func(mvc *sql, char *sname, char *fname, sql_func *f)
{
	...
	if (!mvc_schema_privs(sql, s))
		throw(SQL,"sql.create_func", SQLSTATE(42000) "CREATE %s%s: access denied for %s to schema '%s'", KF, F, stack_get_string(sql, mvc_bind_schema(sql, "tmp"), "current_user"), s->base.name);
