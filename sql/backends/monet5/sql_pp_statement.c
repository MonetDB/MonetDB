...
InstrPtr
stmt_oahash_probe(backend *be, stmt *key, int hsh, int rhs_ht, stmt *pp)
{
...
InstrPtr
stmt_oahash_combined_probe(backend *be, stmt *key, int hsh, int sel, int rhs_ht, stmt *pp)
{
...
pipeline_leave(backend *be, stmt *pp)
{
	...
	if (nrparts >= 0) {
		...
		if (r == NULL)
			return NULL;
		getArg(r, 0) = getArg(pp->q, 1); /* counter variable */
		...
		r->barrier = LEAVEsymbol;
		getArg(r, 0) = pp->nr;
		r = pushArgument(be->mb, r, getArg(pp->q, 1)); /* current nr */
		r = pushInt(be->mb, r, 1);
		pushInstruction(be->mb, r);
	}
	if (r) {
...
stmt_pp_create_nrparts_or_dynamic(backend *be, int nrparts, int input)
{
	...
	if ((nrparts >= 0 || be->source) && pipeline_leave(be, s))
		return s;
	return s;
}
