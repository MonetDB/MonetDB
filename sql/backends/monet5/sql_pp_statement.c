...
stmt_oahash_hshmrk_init(backend *be, stmt *stmts_ht, bool moveup)
{
	...
	stmt *s = stmt_none(be);
	s->subtype = *sql_fetch_localtype(TYPE_bit);
	s->q = q;
	s->nr = getArg(q, 0);
	s->nrcols = 1;
	return s;
...
stmt_oahash_hash(backend *be, stmt *key, stmt *prev, stmt *ht)
{
	...
	stmt *s = stmt_none(be);
	if (s == NULL) return NULL;
	s->subtype = *sql_fetch_localtype(TYPE_lng);
	s->nr = getArg(q, 0);
	s->nrcols = 1;
	s->q = q;
...
stmt_oahash_probe(backend *be, stmt *key, stmt *prev, stmt *rhs_ht, stmt *freq, stmt *outer, bool single, bool semantics, bool eq, bool outerjoin, bool groupedjoin)
{
	...
	stmt *s = stmt_none(be);
	if (s == NULL) return NULL;
	s->subtype = *sql_fetch_localtype(TYPE_oid);
	s->nr = getArg(q, 0);
	s->nrcols = 1;
	s->q = q;
	return s;
...
stmt_no_slices(backend *be, stmt *col, bool hash)
{
	...
	ns->q = q;
	ns->nr = getArg(q, 0);
	ns->subtype = *sql_fetch_localtype(TYPE_int);
	return ns;
}
