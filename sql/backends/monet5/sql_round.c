	...
	if (v->T->nonil == TRUE) {
		...
		for (i = 0; i < cnt; i++) {
			if (src[i] == bte_nil) {
				...
				dst[i] = 
	bte_round_body_nonil ( src[i], *d, *s, *r );
			}
		}
	}
...
sht_bat_dec_round_wrap ( bat *_res, bat *_v, sht *r )
{
	...
	/* allocate result BAT */
	res = BATnew(TYPE_void, TYPE_sht, cnt);
	if (res == NULL) {
		BBPreleaseref(v->batCacheid);
		throw(MAL, "round", MAL_MALLOC_FAIL);
	}
	...
	int_bat_dec_round_wrap ( bat *_res, bat *_v, int *r )
{
	...
	/* allocate result BAT */
	res = BATnew(TYPE_void, TYPE_int, cnt);
	if (res == NULL) {
		BBPreleaseref(v->batCacheid);
		throw(MAL, "round", MAL_MALLOC_FAIL);
	}
...
wrd_bat_dec_round_wrap ( bat *_res, bat *_v, wrd *r )
{
	...
	/* allocate result BAT */
	res = BATnew(TYPE_void, TYPE_wrd, cnt);
	if (res == NULL) {
		BBPreleaseref(v->batCacheid);
		throw(MAL, "round", MAL_MALLOC_FAIL);
	}
	...
	if (v->T->nonil == TRUE) {
		...
		for (i = 0; i < cnt; i++) {
			if (src[i] == wrd_nil) {
				...
				dst[i] = 
	wrd_dec_round_body_nonil ( src[i], *r );
			}
		}
	}
...
wrd_bat_round_wrap ( bat *_res, bat *_v, int *d, int *s, bte *r )
{
	...
	if (v->T->nonil == TRUE) {
		for (i = 0; i < cnt; i++)
			dst[i] = 
	wrd_round_body_nonil ( src[i], *d, *s, *r );
	} else {
		for (i = 0; i < cnt; i++) {
			if (src[i] == wrd_nil) {
				nil = TRUE;
...
str lng_bat_dec_round_wrap ( bat *_res, bat *_v, lng *r )
{
	BAT *res, *v;
	lng *src, *dst;
	BUN i, cnt;
	...
	/* allocate result BAT */
	res = BATnew(TYPE_void, TYPE_lng, cnt);
	if (res == NULL) {
		BBPreleaseref(v->batCacheid);
		throw(MAL, "round", MAL_MALLOC_FAIL);
	}
	...
	if (v->T->nonil == TRUE) {
		...
		for (i = 0; i < cnt; i++) {
			if (src[i] == lng_nil) {
				...
				dst[i] = 
	lng_dec_round_body_nonil ( src[i], *r );
			}
		}
	}
