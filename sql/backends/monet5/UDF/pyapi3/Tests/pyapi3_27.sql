...
CREATE FUNCTION rvalfunc() RETURNS TABLE(groupcol INTEGER, intcol INTEGER, fltcol FLOAT, dblcol DOUBLE, strcol STRING) LANGUAGE PYTHON3 {
...
CREATE AGGREGATE aggrmedian_int(val integer) RETURNS integer LANGUAGE PYTHON3_MAP { return numpy.median(val) };
CREATE AGGREGATE aggrmedian_flt(val float) RETURNS integer LANGUAGE PYTHON3_MAP { return numpy.median(val) };
CREATE AGGREGATE aggrmedian_dbl(val double) RETURNS integer LANGUAGE PYTHON3_MAP { return numpy.median(val) };
CREATE AGGREGATE aggrmedian_str(val string) RETURNS integer LANGUAGE PYTHON3_MAP { return numpy.median([int(x) for x in val]) };
...
#CREATE FUNCTION rvalfunc() RETURNS TABLE(groupcol INTEGER, datacol INTEGER) LANGUAGE PYTHON3 {
#    return {'groupcol': [0,1,2,0,1,2,3], 
