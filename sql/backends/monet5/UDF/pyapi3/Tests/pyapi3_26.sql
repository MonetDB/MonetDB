...
CREATE FUNCTION average(i INTEGER) RETURNS INTEGER LANGUAGE PYTHON3 'pyapi26_test.py';

CREATE FUNCTION create_file() RETURNS TABLE(i INTEGER) LANGUAGE PYTHON3 {
    import os
...
CREATE FUNCTION delete_file() RETURNS TABLE(i INTEGER) LANGUAGE PYTHON3 {
    import os
    homedir = os.getenv('HOME');
...
CREATE FUNCTION integers() RETURNS TABLE(i INTEGER) LANGUAGE PYTHON3 { return numpy.arange(10000) + 1 };
