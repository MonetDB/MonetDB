...
CREATE FUNCTION pyapi14_create_functions() returns table(s string) LANGUAGE PYTHON3
{
    def function_to_string(fun):
        ...
        if sys.version_info.major < 3:
            ...
            return '@\\x' + '\\x'.join(hex(x)[2:] if x > 15 else '0' + hex(x)[2:] for x in marshal.dumps(fun.__code__))
    ...
    _conn.execute('CREATE FUNCTION myfun1(a INTEGER, b INTEGER) RETURNS INTEGER LANGUAGE PYTHON3YTHON {%s};' % function_to_string(myfun1))
    _conn.execute('CREATE FUNCTION myfun2(a INTEGER, b INTEGER) RETURNS INTEGER LANGUAGE PYTHON3YTHON {%s};' % function_to_string(myfun2))
    _conn.execute('CREATE FUNCTION myfun3(a INTEGER, b INTEGER) RETURNS INTEGER LANGUAGE PYTHON3YTHON {%s};' % function_to_string(myfun3))
    _conn.execute('CREATE FUNCTION myfun4(a INTEGER, b INTEGER) RETURNS INTEGER LANGUAGE PYTHON3YTHON {%s};' % function_to_string(myfun4))
    _conn.execute('CREATE FUNCTION myfun5(a INTEGER, b INTEGER) RETURNS INTEGER LANGUAGE PYTHON3YTHON {%s};' % function_to_string(myfun5))
