...
CREATE FUNCTION mapped_query() returns table (i integer) LANGUAGE PYTHON3_MAP
{
...
CREATE FUNCTION mapped_result_query() returns table (i integer) LANGUAGE PYTHON3_MAP
{
...
START TRANSACTION;
# try a bigger set
CREATE FUNCTION pyapi19_create_table() returns table (i integer) LANGUAGE PYTHON3
{
    return numpy.arange(100000)
};
CREATE FUNCTION pyapi19_load_table() returns table (i integer) LANGUAGE PYTHON3_MAP
{
    res = _conn.execute('SELECT * FROM pyapi19_integers;')
    return res['i']
};
...
CREATE FUNCTION mapped_result_query() returns table (i STRING) LANGUAGE PYTHON3_MAP
{
...
CREATE FUNCTION mapped_result_query() returns table (d DATE) LANGUAGE PYTHON3_MAP
{
...
CREATE FUNCTION pyapi19_create_table() returns table (i integer, j integer, k double, l float, m smallint, n bigint, o STRING, p DECIMAL) LANGUAGE PYTHON3
{
    result = dict();
...
CREATE FUNCTION pyapi19_load_table() returns table (i integer, j integer, k double, l float, m smallint, n bigint, o STRING, p DECIMAL) LANGUAGE PYTHON3_MAP
{
    res = _conn.execute('SELECT * FROM pyapi19_integers;')
    return res
};
