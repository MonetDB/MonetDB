...
/*
 * M. Raasveldt
 * This file contains a number of helper functions for converting between utf8
 and utf32. The reason this is necessary is
   because Numpy stores Unicode strings in utf32 format, and MonetDB stores them
 in utf8 format.
 */
...
//! Returns the amount of characters in the utf8 formatted string, returns -1 if
//! the string is not a valid utf8 string
/* Arguments:
	utf8_str: A pointer to a utf8 formatted string.
	ascii: a pointer to a boolean, this is set to true if the string is
...
//! Converts a utf32 string to a utf8 string, returns TRUE on success and FALSE
//! on failure
/* Arguments:
	offset: The offset in the utf32 array in amount of utf32 characters (so 4 *
   bytes)
	size: The maximum length of the utf32 string in amount of utf32 characters
   (note: if a \0 character is encountered, the parser will stop early)
	utf8_storage: The storage container to put the resulting utf8 formatted
   string in. To ensure the utf8 string fits this has to be [size * 4] bytes.
...
/* Arguments:
	...
	size: The maximum length of the utf8 string in bytes (note: if a \0
   character is encountered, the parser will stop early)
	utf32_storage: The storage container to put the resulting utf32 formatted
...
//! utf-8 character in bytes on success [1,2,3 or 4], and -1 on failure
/* Arguments:
	position:
	utf32_storage:
	offset:
