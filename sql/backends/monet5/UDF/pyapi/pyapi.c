...
static str PyAPIeval(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci, bit grouped, bit mapped) {
	...
	if (!pyapiInitialized) {
		throw(MAL, "pyapi.eval", "SQLSTATE ----- !""Embedded Python is enabled but an error was "
	...
	for (i = pci->retc + 2; i < argcount; i++) {
		...
		if (!isaBatType(getArgType(mb, pci, i))) {
			...
			if (b == NULL) {
				msg = createException(
					MAL, "pyapi.eval",
					"SQLSTATE ----- !""The BAT passed to the function (argument #%d) is NULL.\n",
					i - (pci->retc + 2) + 1);
				goto wrapup;
			}
			seqbase = b->hseqbase;
	...
	if (mapped) {
		...
		gstate = Python_ObtainGIL(); // we need the GIL before forking,
									 ...
									 // child
		if ((pid = fork()) < 0) {
			msg = createException(MAL, "pyapi.eval", "SQLSTATE ----- !""Failed to fork process");
			MT_lock_unset(&pyapiLock);
		...
		if (!child_process) {
			...
			while (true) {
				...
				if (retcode < 0) {
					...
					msg = createException(
						MAL, "waitpid",
						"SQLSTATE ----- !""Error calling waitpid(%llu, &status, WNOHANG): %s",
						pid, err);
					break;
				}
			}
			...
			if (!success) {
				...
				if (descr->bat_size == 0) {
					msg = createException(
						MAL, "pyapi.eval",
						"SQLSTATE ----- !""Failure in child process with unknown error.");
				} else if (GDKinitmmap(mmap_id + 3, descr->bat_size,
									   &mmap_ptrs[3], &mmap_sizes[3],
									   &msg) == GDK_SUCCEED) {
					msg = createException(MAL, "pyapi.eval", "SQLSTATE ----- !""%s",
										  (char *)mmap_ptrs[3]);
				}
				goto wrapup;
			}
	...
	if (sqlfun) {
		...
		if (exprStr[0] == '/' ||
			...
			if (stat(address, &buffer) < 0) {
				msg = createException(
					MAL, "pyapi.eval",
					"SQLSTATE ----- !""Could not find Python source file \"%s\".", address);
				goto wrapup;
			}
			fp = fopen(address, "r");
			if (fp == NULL) {
				msg = createException(
					MAL, "pyapi.eval",
					"SQLSTATE ----- !""Could not open Python source file \"%s\".", address);
				goto wrapup;
			}
			fseek(fp, 0, SEEK_END);
			length = ftell(fp);
			...
			if (fread(exprStr, 1, length, fp) != length) {
				msg = createException(MAL, "pyapi.eval",
									  "SQLSTATE ----- !""Failed to read from file \"%s\".",
									  address);
				goto wrapup;
			}
			fclose(fp);
	...
	for (i = pci->retc + 2; i < argcount; i++) {
		...
		if (result_array == NULL) {
			if (msg == MAL_SUCCEED) {
				msg = createException(MAL, "pyapi.eval",
									  "SQLSTATE ----- !""Failed to create Numpy Array from BAT.");
			}
			goto wrapup;
		}
		if (code_object == NULL) {
		...
		if (parallel_aggregation) {
			...
			for (i = 0; i < named_columns; i++) {
				...
				if (!input.scalar) {
					switch (input.bat_type) {
						...
						default:
							msg = createException(
								MAL, "pyapi.eval", "SQLSTATE ----- !""Unrecognized BAT type %s",
...

			{
				...
				for (thread_it = 0; thread_it < threads; thread_it++) {
					...
					if (res != 0) {
						msg = createException(MAL, "pyapi.eval",
											  "SQLSTATE ----- !""Failed to start thread.");
						goto aggrwrapup;
					}
				}
				for (thread_it = 0; thread_it < threads; thread_it++) {
		...
		if (PyDict_Check(pResult)) { // Handle dictionary returns
			...
			if (!varres) {
				if (sqlfun != NULL) {
					...
					msg = createException(MAL, "pyapi.eval",
										  "SQLSTATE ----- !""Return value is a dictionary, but "
				...
				for (i = 0; i < retcols; i++) {
					...
					if (!PyString_CheckExact(colname)) {
						msg = createException(MAL, "pyapi.eval",
											  "SQLSTATE ----- !""Expected a string key in the "
			...
			msg = createException(MAL, "pyapi.eval",
								  "SQLSTATE ----- !""Expected a variable number return values, "
	...
	if (mapped && child_process) {
		...
		for (i = 0; i < retcols; i++) {
			...
			if (ret->result_type == NPY_OBJECT) {
				...
				if (new_array == NULL) {
					msg = createException(MAL, "pyapi.eval",
										  "SQLSTATE ----- !""Could not convert the returned "
...
PYFUNCNAME(PyAPIprelude)(void *ret) {
	...
	if (!pyapiInitialized) {
		...
		if (marshal_module == NULL) {
			return createException(MAL, "pyapi.eval", "SQLSTATE ----- !""Failed to load Marshal module.");
		}
		marshal_loads = PyObject_GetAttrString(marshal_module, "loads");
		if (marshal_loads == NULL) {
			return createException(MAL, "pyapi.eval", "SQLSTATE ----- !""Failed to load function \"loads\" from Marshal module.");
		}
	...
	if (py_error_value) {
		...
		if (pycall != NULL && strlen(pycall) > 0) {
			...
			// display 5 lines around the error message
			{
				...
				return createException(MAL, "pyapi.eval",  "SQLSTATE ----- !" "%s\n%s\n%s",
