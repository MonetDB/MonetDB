...
str
sql_install_41_md5sum(Client c)
{
	size_t bufsize = 16384, pos = 0;
	char *buf = GDKmalloc(bufsize), *err = NULL;

	if (buf == NULL)
		throw(SQL, "sql.install_41_md5sum", SQLSTATE(HY001) MAL_MALLOC_FAIL);
	pos += snprintf(buf + pos, bufsize - pos, "  create function sys.md5(v string) returns string external name clients.md5sum; grant execute on function md5 to public;");
