...
sql_install_11_times(Client c)
{
	...
	if (buf == NULL)
		throw(SQL, "sql.install_11_times", SQLSTATE(HY001) MAL_MALLOC_FAIL);
	pos += snprintf(buf + pos, bufsize - pos, "  create procedure times() external name sql.times; grant execute on procedure times to public;");
