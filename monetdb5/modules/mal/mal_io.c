...
static const char niltext[] = "nil";

static str
IOprintf_(allocator *ma, str *res, const char *format, ...)
{
	va_list ap;
	int n;

	int prec = 0, dotseen = 0, escaped = 0, type, argc = 1;
	size_t size = 80, adds = 100, width = 0;
	char *add, *dst, *buf;
	...
	if (format == NULL) {
		...
		*res = ma_strdup(ma, format);
		if (*res == NULL)
			throw(MAL, "io.printf", SQLSTATE(HY013) MAL_MALLOC_FAIL);
		return MAL_SUCCEED;
	}
	buf = dst = ma_alloc(ma, size);
	if (buf == NULL)
		throw(MAL, "io.printf", SQLSTATE(HY013) MAL_MALLOC_FAIL);
	*res = NULL;

	add = ma_alloc(ma, adds);
	if (add == NULL) {
		throw(MAL, "io.printf", SQLSTATE(HY013) MAL_MALLOC_FAIL);
	}
	...
	for (const char *cur = format; *cur; cur++) {
		if (paramseen) {
			...
			if ((p = va_arg(ap, char *)) == NULL) {
				va_end(ap);
				throw(MAL, "io.printf", toofew_error, argc);
			}
			type = va_arg(ap, int);
			...
			if (ATOMeq(type, ATOMnilptr(type), p)) {
				...
				*(++ctrg) = 0;
				...
				if (dotseen) {
					...
					va_end(ap);
					throw(MAL, "io.printf", type_error, argc);
#endif
...
IOprintf(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	switch (pci->argc) {
	case 2:
		msg = IOprintf_(ta, &fmt2, *fmt);
		break;
	case 3:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2));
		break;
	case 4:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3));
		break;
	case 5:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4));
		break;
	case 6:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4), G(5));
		break;
	case 7:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4), G(5), G(6));
		break;
	case 8:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4), G(5), G(6), G(7));
		break;
	case 9:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4), G(5), G(6), G(7), G(8));
		break;
	case 10:
		msg = IOprintf_(ta, &fmt2, *fmt, G(2), G(3), G(4), G(5), G(6), G(7), G(8),
						G(9));
		break;
	default:
		msg = createException(MAL, "io.printf", "Too many arguments to io.printf");
		break;
	}
	if (msg == MAL_SUCCEED) {
...
IOprintfStream(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	case 3:
		...
		break;
	case 4:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3));
		break;
	case 5:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4));
		break;
	case 6:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5));
		break;
	case 7:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5), G(6));
		break;
	case 8:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5), G(6), G(7));
		break;
	case 9:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5), G(6), G(7), G(8));
		break;
	case 10:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5), G(6), G(7), G(8), G(9));
		break;
	case 11:
		msg = IOprintf_(ta, &fmt2, *fmt, G(3), G(4), G(5), G(6), G(7), G(8), G(9),
						G(10));
...
#include "mel.h"
static mel_func mal_io_init_funcs[] = {
 ...
 pattern("io", "print", IOtable, false, "BATs are printed with '#' for legend lines, and the BUNs on separate lines between brackets, containing each to comma separated values (head and tail). If multiple BATs are passed for printing, print() performs an implicit natural join on the void head, producing a multi attribute table.", args(1,2, arg("",void),batvarargany("b1",0))),
