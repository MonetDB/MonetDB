 ...
 * updates the heap */ \
static int pqueue_topreplace_##NAME(BAT *h, \
		 oid *idx, \
		 TYPE *el) \
{ \
  BATiter hi = bat_iterator(h); \
...
/*enqueue an element*/
static int pqueue_enqueue_anymin(BAT *h, oid *idx, ptr el, int tpe)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN ins,cur;
...
/* moves down the root element */
/* used by dequeue (see below) */
static int pqueue_movedowntop_anymin(BAT *h)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN swp,cur;
...
/* removes the root element, puts the last element as root and moves it down */
static int pqueue_dequeue_anymin(BAT *h)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN num_elements;
...
/* replaces the top element with the input if it is larger (smaller) and
 * updates the heap */
static int pqueue_topreplace_anymin(BAT *h, oid *idx, ptr el, int tpe)
{
  BATiter hi = bat_iterator(h);
  BUN hbase = BUNfirst(h);
...
static int pqueue_topn_voidanymin(BAT **H, BAT *t, wrd *N)
{
  BAT *h = NULL;
  BATiter ti = bat_iterator(t);
  BUN i, n = BATcount(t);
...
static int pqueue_topn_anymin(BAT **H, BAT *t, wrd *N)
{
  BAT *h = NULL;
  BATiter ti = bat_iterator(t);
  BUN i, n = BATcount(t);
  ...
  n = BATcount(t);
  for(; i<n; i++, p++) {
	pqueue_topreplace_anymin(h, (oid*)BUNhloc(ti,p), BUNtail(ti,p), tpe); 
  }
  *H = heap2bat_anymin(h);
  BBPunfix(h->batCacheid);
  return GDK_SUCCEED;
...
static int pqueue_utopn_voidanymin(BAT **H, BAT *t, wrd *N)
{
  BAT *duplicates = NULL, *b;
...
static int pqueue_utopn_anymin(BAT **H, BAT *t, wrd *N)
{
  BAT *duplicates = NULL, *b;
  BATiter hi, ti = bat_iterator(t);
  BUN i, j, n, cnt = BATcount(t), p;
...
/*enqueue an element*/
static int pqueue_enqueue_anymax(BAT *h, oid *idx, ptr el, int tpe)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN ins,cur;
...
/* moves down the root element */
/* used by dequeue (see below) */
static int pqueue_movedowntop_anymax(BAT *h)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN swp,cur;
...
/* removes the root element, puts the last element as root and moves it down */
static int pqueue_dequeue_anymax(BAT *h)
{
  BATiter hi = bat_iterator(h);
  BUN hbase;
  BUN num_elements;
...
/* replaces the top element with the input if it is larger (smaller) and
 * updates the heap */
static int pqueue_topreplace_anymax(BAT *h, oid *idx, ptr el, int tpe)
{
  BATiter hi = bat_iterator(h);
  BUN hbase = BUNfirst(h);
...
static int pqueue_topn_voidanymax(BAT **H, BAT *t, wrd *N)
{
  BAT *h = NULL;
  BATiter ti = bat_iterator(t);
  BUN i, n = BATcount(t);
...
static int pqueue_topn_anymax(BAT **H, BAT *t, wrd *N)
{
  BAT *h = NULL;
  BATiter ti = bat_iterator(t);
  BUN i, n = BATcount(t);
  ...
  n = BATcount(t);
  for(; i<n; i++, p++) {
	pqueue_topreplace_anymax(h, (oid*)BUNhloc(ti,p), BUNtail(ti,p), tpe); 
  }
  *H = heap2bat_anymax(h);
  BBPunfix(h->batCacheid);
  return GDK_SUCCEED;
...
static int pqueue_utopn_voidanymax(BAT **H, BAT *t, wrd *N)
{
  BAT *duplicates = NULL, *b;
  BATiter hi, ti = bat_iterator(t);
  BUN i, j, n, cnt = BATcount(t), p;
...
static int pqueue_utopn_anymax(BAT **H, BAT *t, wrd *N)
{
  BAT *duplicates = NULL, *b;
  BATiter hi, ti = bat_iterator(t);
  BUN i, j, n, cnt = BATcount(t), p;
