...
PPcounter_get(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (c->s.type != PIPELINE_IO_COUNTER) {
		BBPunfix(b->batCacheid);
		throw(MAL, "pipeline.counter_get", SQLSTATE(HY002) "Invalid source type %d, expected %d", c->s.type, PIPELINE_IO_COUNTER);
	}
...
PPconcat(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	pp_concat *pcat = (pp_concat*)GDKzalloc(sizeof(pp_concat) + (nr+1) * sizeof(struct pipeline_io*) );
