...
MANIFOLDjob(allocator *ma, MULTItask *mut)
{
	...
	switch (mut->pci->argc) {
	case 4:
		Manifoldbody(1, args[3]);
		break;
	case 5:
		Manifoldbody(2, args[3], args[4]);
		break;
	case 6:
		Manifoldbody(3, args[3], args[4], args[5]);
		break;
	case 7:
		Manifoldbody(4, args[3], args[4], args[5], args[6]);
		break;
	case 8:
		Manifoldbody(5, args[3], args[4], args[5], args[6], args[7]);
		break;
	default:
...
MANIFOLDevaluate(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	mat[0].b = COLnew(mat[mut.fvar].b->hseqbase,
					  getBatType(getArgType(mb, pci, 0)), cnt, TRANSIENT);
	if (mat[0].b == NULL) {
		msg = createException(MAL, "mal.manifold",
							  SQLSTATE(HY013) MAL_MALLOC_FAIL);
		goto wrapup;
