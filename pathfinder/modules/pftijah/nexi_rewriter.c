...
int COtoCPlan(int query_num, int type, struct_RMT *txt_retr_model, struct_RF *rel_feedback) {
  ...
  if (command_file_pre == NULL) {
    printf("Error: cannot open CO pre-command file.\n");
    return 0;
  }

  if (token_file_pre == NULL) {
    printf("Error: cannot open CO pre-token file.\n");
    return 0;
  }

  if (command_file == NULL) {
    printf("Error: cannot open CO command file.\n");
    return 0;
  }

  if (token_file == NULL) {
    printf("Error: cannot open CO token file.\n");
    return 0;
  }
  ...
  if (rel_feedback == NULL) {
	...
	if (txt_retr_model != NULL && (txt_retr_model->prior_type == LENGTH_PRIOR || txt_retr_model->prior_type == LOG_NORMAL_PRIOR)) {
...
/*
	...
	if (rel_feedback->rf_type == RF_JOURNAL || rel_feedback->rf_type == RF_SIZE || rel_feedback->rf_type == RF_JOURNAL_SIZE) {
	...
	else if (rel_feedback->rf_type == RF_ELEMENT || rel_feedback->rf_type == RF_JOURNAL_ELEM || rel_feedback->rf_type == RF_ELEMENT_SIZE || rel_feedback->rf_type == RF_ALL) {
	  
	  if (!strcmp(rel_feedback->elem2_name,"EMPTY")) {
	    fprintf(command_file_pre, "%d\n", SELECT_NODE);
	    fprintf(token_file_pre, "\"%s\"\n",rel_feedback->elem1_name);
	  }
	  else {
	    fprintf(command_file_pre, "%d\n", OB);
	    fprintf(command_file_pre, "%d\n", SELECT_NODE);
	    ...
	    fprintf(command_file_pre, "%d\n", STRUCT_OR);
	    fprintf(command_file_pre, "%d\n", SELECT_NODE);
	    fprintf(token_file_pre, "\"%s\"\n",rel_feedback->elem2_name);
     
	    if (strcmp(rel_feedback->elem3_name,"EMPTY")) {
	      fprintf(command_file_pre, "%d\n", STRUCT_OR);
	      fprintf(command_file_pre, "%d\n", SELECT_NODE);
	      fprintf(token_file_pre, "\"%s\"\n",rel_feedback->elem3_name);
	    }
	    fprintf(command_file_pre, "%d\n", CB);
	  }
