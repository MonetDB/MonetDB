 ...
 * with side effects.
 */
static PFmil_t *reduce_expressions (PFmil_t *root, PFbitset_t *used_vars, 
                                    PFbitset_t *dirty_vars) 
{
    ...
    for (unsigned int i = 0; i < MIL_MAXCHILD 
             && root->child[i] != NULL; i++) {
        PFmil_t *param_op = mil_dce_worker (root->child[i], used_vars,
                                            dirty_vars);
 ...
 */
static bool var_duplication (PFmil_t *root, PFmil_ident_t *var) {
    switch (root->kind) {
    case m_var:
        *var = root->sem.ident;
        return true;
...
mil_dce_worker (PFmil_t *root, PFbitset_t *used_vars, PFbitset_t *dirty_vars) 
{
    ...
    switch (root->kind) {
    case m_seq:
        /* descend in reverse pre-order, collect the used variables from the 
           end to beginning of the program. */
        for (int i = MIL_MAXCHILD; --i >= 0;)
            if (root->child[i] != NULL)
                root->child[i] = mil_dce_worker (root->child[i], used_vars, 
                                                 dirty_vars);
        return root;
    case m_assgn:
        {
            ...
            if (var_duplication (rvalue, &var)) {
                /* Assignment is of the form x := y. Neighter x nor y 
                   can be dirty */
                if (PFbitset_get (dirty_vars, var)
                    || (var_duplication (lvalue, &var) 
                        && PFbitset_get (dirty_vars, var)))
                    assert (false);
            } 
#endif            
            if (!used) 
    ...
    case m_if:
        {
            ...
            root->child[1] = mil_dce_worker (root->child[1], used_vars, 
                                             dirty_vars);
            /* invoke dce on else part */
            root->child[2] = mil_dce_worker (root->child[2], else_used_vars, 
                                             else_dirty_vars);
    ...
    case m_col_name:
        {
            ...
            if (var_duplication (root->child[0], &var)) {
                ...
                /* statement is e.g. append(a, b) and a is not used, but b 
