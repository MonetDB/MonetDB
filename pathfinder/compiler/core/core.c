...
PFcore_new_var (char *prefix)
{
    ...
    /* It might be that the variable name is truncated. This does not affect
       the correct core mapping but may confuse the user. */
    if (prefix)
        snprintf (vname, sizeof(vname), "%s_%04u", prefix, core_vars);
    else
