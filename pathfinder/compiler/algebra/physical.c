...
PFpa_nljoin (PFalg_att_t att1, PFalg_att_t att2,
             const PFpa_op_t *n1, const PFpa_op_t *n2)
{
    PFpa_op_t  *ret = wire2 (pa_nljoin, n1, n2);
    ...
    if (contains_att (n1->schema, att1) && contains_att (n2->schema, att2)) {
        ...
        PFoops (OOPS_FATAL, "problem with attributes in NLjoin");
    ...
    /*
     * We actually use a LeftNLJoin implementation that keeps the
     * ordering of the left operand.  The output ordering is even more
     * specific: we can refine it by the ordering of the right operand,
     * with its first attribute removed.  Furthermore, we may
     * interleave the output ordering with the join attribute from the
     * right operand everywhere, as it is constant for any value in
     * the join operand from the right side.
     */
    for (unsigned int i = 0; i < PFord_set_count (n1->orderings); i++)
        for (unsigned int j = 0; j < PFord_set_count (n2->orderings); j++) {

            PFord_ordering_t refined = PFord_set_at (n1->orderings, i);

            /* refine by right ordering, with first attribute removed */
            for (unsigned int k = 1;
                 k < PFord_count (PFord_set_at (n2->orderings, j));
                 k++)
                refined = PFord_refine (
                        refined,
                        PFord_order_at (PFord_set_at (n2->orderings, j), k));

            /* now interleave this with the join attribute from right */
            for (unsigned int k = 0; k < PFord_count (refined); k++) {

                PFord_ordering_t ord = PFordering ();

                for (unsigned int l = 0; l < PFord_count (refined); l++) {

                    if (k == l)
                        ord = PFord_refine (ord, att2);

                    ord = PFord_refine (ord, PFord_order_at (refined, l));
                }

                PFord_set_add (ret->orderings, ord);
            }

            /* add the ordering with right join attribute at the end */
            PFord_set_add (ret->orderings, PFord_refine (refined, att2));
        }
...
/**
 * MergeJoin: Join two tables based on ordered join columns.
 *
 * Requires @em both input relations to be sorted on their join
 * attribute.
 *
 * @note
 *   The implementation as MIL code does not necessarily lead to
 *   a merge join.  Instead, we use MonetDB's @c join operator
 *   that may choose optimized implementations instead.
 */
PFpa_op_t *
PFpa_merge_join (PFalg_att_t att1, PFalg_att_t att2,
                 const PFpa_op_t *n1, const PFpa_op_t *n2)
{
    PFpa_op_t  *ret = wire2 (pa_merge_join, n1, n2);
    bool        ordered;
    ...
    if (contains_att (n1->schema, att1) && contains_att (n2->schema, att2)) {
        ...
        PFoops (OOPS_FATAL, "problem with attributes in MergeJoin");

#ifndef NDEBUG
    /* plans need to have appropriate ordering */
    ordered = false;
    for (unsigned int i = 0; i < PFord_set_count (n1->orderings); i++)
        if (PFord_implies (
                    PFord_set_at (n1->orderings, i),
                    PFord_refine (PFordering (), ret->sem.eqjoin.att1))) {
            ordered = true;
            break;
        }

    if (!ordered)
        PFoops (OOPS_FATAL, "Required ordering for MergeJoin is missing!");

    /* plans need to have appropriate ordering */
    ordered = false;
    for (unsigned int i = 0; i < PFord_set_count (n2->orderings); i++)
        if (PFord_implies (
                    PFord_set_at (n2->orderings, i),
                    PFord_refine (PFordering (), ret->sem.eqjoin.att2))) {
            ordered = true;
            break;
        }
    ...
    /*
     * We actually use a LeftMergeJoin implementation that keeps the
     * ordering of the left operand.  The output ordering is even more
     * specific: we can refine it by the ordering of the right operand,
     * with its first attribute removed.  Furthermore, we may
     * interleave the output ordering with the join attribute from the
    ...
    for (unsigned int i = 0; i < PFord_set_count (n1->orderings); i++)
        for (unsigned int j = 0; j < PFord_set_count (n2->orderings); j++) {
            ...
            /* refine by right ordering, with join attribute removed */
            for (unsigned int k = 0;
                 k < PFord_count (PFord_set_at (n2->orderings, j));
                 k++)
                if (strcmp (PFord_order_at (PFord_set_at (n2->orderings, j), k),
                            ret->sem.eqjoin.att2))
                    refined = PFord_refine (
                            refined,
                            PFord_order_at (
                                PFord_set_at (n2->orderings, j), k));
            ...
            /* add the ordering with right join attribute at the end */
            PFord_set_add (ret->orderings,
                           PFord_refine (refined, ret->sem.eqjoin.att2));
        }
