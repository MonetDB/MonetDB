 ...
 *    |    \ /        o |X|_2.att2 == iter
 *   pi*_5  #
 *     \    |         Furthermore the number operator (#) forms
                ...
                unsigned int map_count)
{
    ...
    /* # */
    if (number->kind != la_number || number->sem.number.res != num)
        return NULL;
...
replace_pos_predicate (PFla_op_t *p)
{
    ...
    for (unsigned int i = 0; i < count; i++)
        if (map[i].old == op->sem.rownum.res)
            return;

    base = L(op);
    part = op->sem.rownum.part;
...
opt_complex (PFla_op_t *p)
{
    ...
    switch (p->kind) {
        ...
        case la_step:
            ...
            else if (R(p)->kind == la_number &&
                     p->sem.step.axis == alg_chld &&
                     p->sem.step.iter == R(p)->sem.number.res &&
